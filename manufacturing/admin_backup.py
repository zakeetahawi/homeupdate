...
class ManufacturingOrderAdmin(admin.ModelAdmin):
    ...
    def bulk_update_status(self, request, queryset):
        ...
        if "apply" in request.POST:
            ...
            if form.is_valid():
                ...
                self.message_user(
                    request,
                    f"تم تحديث حالة {count} أمر تصنيع إلى {dict(queryset.model.STATUS_CHOICES)[new_status]} بنجاح.",
                )
                return None
        else:
            form = StatusForm()
    ...
    def get_queryset(self, request):
        ...
        return (
            ...
            .select_related("order", "order__customer", "production_line", "created_by")
            .defer("notes", "rejection_reason", "rejection_reply")
        )
