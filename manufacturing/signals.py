...
@receiver(post_save, sender='manufacturing.ManufacturingOrderItem')
def auto_complete_fabric_receipt_for_auto_completed_permit(sender, instance, created, **kwargs):
    """
    تحديد الأقمشة كمستلمة تلقائياً عندما يكون رقم الإذن يحتوي على 'AUTO-COMPLETED'
    """
    from django.utils import timezone
    
    # التحقق من أن رقم الإذن يحتوي على AUTO-COMPLETED
    permit = instance.permit_number or ''
    if 'AUTO-COMPLETED' not in permit.upper():
        return
    
    # التحقق من أنه لم يتم الاستلام بعد
    if instance.fabric_received:
        return
    
    try:
        # تحديث حالة الاستلام
        instance.fabric_received = True
        instance.fabric_received_date = timezone.now()
        instance.fabric_notes = 'تم الاستلام تلقائياً - رقم إذن AUTO-COMPLETED'
        
        # استخدام update لتجنب استدعاء post_save مرة أخرى
        sender.objects.filter(pk=instance.pk).update(
            fabric_received=True,
            fabric_received_date=timezone.now(),
            fabric_notes='تم الاستلام تلقائياً - رقم إذن AUTO-COMPLETED'
        )
        
        # إنشاء سجل FabricReceipt
        FabricReceipt = apps.get_model('manufacturing', 'FabricReceipt')
        FabricReceiptItem = apps.get_model('manufacturing', 'FabricReceiptItem')
        
        # البحث عن سجل استلام موجود أو إنشاء جديد
        fabric_receipt, created_receipt = FabricReceipt.objects.get_or_create(
            manufacturing_order=instance.manufacturing_order,
            bag_number=instance.bag_number or 'AUTO',
            defaults={
                'receipt_type': 'manufacturing_order',
                'order': instance.manufacturing_order.order if instance.manufacturing_order else None,
                'permit_number': instance.permit_number,
                'received_by_name': 'نظام آلي',
                'receipt_date': timezone.now(),
                'notes': 'تم الاستلام تلقائياً - رقم إذن AUTO-COMPLETED'
            }
        )
        ...
        if not FabricReceiptItem.objects.filter(
            ...
            FabricReceiptItem.objects.create(
                fabric_receipt=fabric_receipt,
                order_item=instance.order_item,
                cutting_item=instance.cutting_item,
                product_name=instance.product_name,
                quantity_received=instance.quantity,
                item_notes='تم الاستلام تلقائياً'
            )
        
        logger.info(f'✅ تم استلام الأقمشة تلقائياً للعنصر {instance.pk} - رقم الإذن: {instance.permit_number}')
        
    except Exception as e:
        logger.error(f'❌ خطأ في الاستلام التلقائي للعنصر {instance.pk}: {str(e)}')
