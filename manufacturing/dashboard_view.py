...
class ImprovedDashboardView(LoginRequiredMixin, PermissionRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        monthly_orders = (
            ManufacturingOrder.objects.filter(order_date__range=(start_date, end_date))
        ...
        if last_month_orders > 0:
            month_change_percent = (
                (this_month_orders - last_month_orders) / last_month_orders
            ) * 100
        else:
        ...
        avg_stats = ManufacturingOrder.objects.filter(
            ...
            avg_days=Avg(
                ExpressionWrapper(
                    F("completion_date") - F("order_date"), output_field=DurationField()
                )
            )
        )
            ...
            .annotate(
                ...
                overdue_orders=Count(
                    ...
                    filter=Q(
                        ...
                        manufacturing_orders__order_type__in=[
                            "installation",
                            "custom",
                            "delivery",
                        ],
