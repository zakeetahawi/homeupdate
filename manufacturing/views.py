...
def update_order_status(request, pk):
    ...
    try:
        ...
        try:
            ...
            original_order_old_status = (
                order.order.tracking_status if order.order else "pending"
            )
        ...
        try:
            ...
            logger.error(
                f"[update_order_status] Error adding admin log entry: {str(e)}",
                exc_info=True,
            )
...
class OverdueOrdersListView(
    ...
    def get_context_data(self, **kwargs):
        ...
        # تجميع حسب الحالة
        status_counts = overdue_orders.values("status").annotate(count=Count("status"))
        status_data = {item["status"]: item["count"] for item in status_counts}
