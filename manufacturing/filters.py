...
class ManufacturingOrderFilter(django_filters.FilterSet):
    ...
    production_line = django_filters.ModelMultipleChoiceFilter(
        queryset=ProductionLine.objects.filter(is_active=True).order_by(
            "-priority", "name"
        ),
        widget=forms.CheckboxSelectMultiple(attrs={"class": "form-check-input"}),
        label="خط الإنتاج",
    ...
    search = django_filters.CharFilter(
        ...
        widget=forms.TextInput(
            attrs={"class": "form-control", "placeholder": "ابحث في أوامر التصنيع..."}
        ),
    )
    ...
    page_size = django_filters.NumberFilter(method="noop_filter", label="عدد الصفوف")
