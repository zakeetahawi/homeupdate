...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            for mfg_order in manufacturing_orders:
                try:
                            ...
                            if (
                                ...
                                try:
                                    ...
                                    InstallationSchedule.objects.filter(
                                        order=order
                                    ).update(status=new_installation_status)
                                except ImportError:
                                    pass
                    ...
                    self.stdout.write(
                        self.style.ERROR(
                            f"❌ خطأ في الطلب {order.order_number}: {str(e)}"
                        )
                    )
        ...
        if dry_run:
            self.stdout.write(
                self.style.WARNING("⚠️ هذا كان اختبار فقط - لم يتم تطبيق التغييرات")
            )
        else:
