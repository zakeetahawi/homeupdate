...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS('=== فحص وإصلاح أنواع أوامر التصنيع ===')
        )
                ...
                try:
                    ...
                    if correct_type and not dry_run:
                        with transaction.atomic():
                            ...
                            fixed_count += 1
                            self.stdout.write(
                                self.style.SUCCESS(f'    ✅ تم إصلاح الطلب {mfg.order.order_number}')
                            )
                    elif correct_type:
                    ...
                    self.stdout.write(
                        self.style.ERROR(f'    ❌ خطأ في معالجة الطلب {mfg.order.order_number}: {e}')
                    )
            ...
            for mfg, correct_type in mismatched_orders:
                ...
                if not dry_run:
                    with transaction.atomic():
                        ...
                        fixed_count += 1
                        self.stdout.write(
                            self.style.SUCCESS(f'    ✅ تم إصلاح النوع')
                        )
                else:
    ...
    def get_statistics(self, queryset):
        ...
        by_type = queryset.values('order_type').annotate(count=Count('id')).order_by('-count')
