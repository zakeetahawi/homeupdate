...
class Command(BaseCommand):
    help = 'مزامنة خطوط الإنتاج وربط أوامر التصنيع بناءً على قواعد الفروع والأنواع المحددة'
    ...
    def get_order_branch(self, order):
        ...
        if hasattr(order, 'order') and order.order:
            ...
            elif hasattr(order.order, 'customer') and order.order.customer and hasattr(order.order.customer, 'branch'):
    ...
    def display_detailed_analysis(self, analysis):
        ...
        self.stdout.write('\n🏢 توزيع الفروع (أهم 10):')
        sorted_branches = sorted(analysis['branch_totals'].items(), key=lambda x: x[1], reverse=True)
        for branch_name, count in sorted_branches[:10]:
            ...
            type_details = ', '.join([f'{otype}({count})' for otype, count in branch_types.items()])
    ...
    def apply_new_synchronization(self, rules, analysis):
        ...
        with transaction.atomic():
            ...
            if self.force:
                ...
                reset_count = ManufacturingOrder.objects.all().update(production_line=None)
            ...
            unassigned_count = ManufacturingOrder.objects.filter(production_line__isnull=True).count()
    ...
    def get_matching_orders(self, config, analysis):
        ...
        if config.get('special_rules') and 'هاني رمسيس' in str(config.get('description', '')):
    ...
    def get_hani_ramsis_orders(self, config):
        ...
        if branch_ids:
            ...
            installation_query = (
                models.Q(order_type='installation') &
                (models.Q(order__branch_id__in=branch_ids) |
                 models.Q(order__customer__branch_id__in=branch_ids))
            )
        ...
        for line_name, config in rules.items():
            ...
            if count > 0:
                ...
                for order in sample_orders:
                    branch_name = self.get_order_branch(order)
                    self.stdout.write(f'      • {order.manufacturing_code} - {branch_name} - {order.order_type}')
