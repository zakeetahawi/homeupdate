...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if dry_run:
            self.stdout.write(
                self.style.WARNING("⚠️  وضع المعاينة - لن يتم حفظ أي تغييرات\n")
            )
        ...
        for item in received_items:
            ...
            if key not in receipts_to_create:
                receipts_to_create[key] = {
                    ...
                    "cutting_order": (
                        item.cutting_item.cutting_order if item.cutting_item else None
                    ),
                }
        ...
        if dry_run:
            ...
            for idx, (key, data) in enumerate(receipts_to_create.items(), 1):
                ...
                customer_name = (
                    mfg_order.order.customer.name
                    if mfg_order and mfg_order.order
                    else "غير محدد"
                )
        ...
        with transaction.atomic():
            for key, data in receipts_to_create.items():
                ...
                existing_receipt = FabricReceipt.objects.filter(
                    manufacturing_order=mfg_order, bag_number=data["bag_number"]
                ).first()
