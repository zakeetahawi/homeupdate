...
class GoogleSyncConfig(models.Model):
    ...
    credentials_file = models.FileField(
        _("ملف بيانات الاعتماد"), upload_to="google_credentials/"
    )
    ...
    sync_manufacturing_orders = models.BooleanField(
        _("مزامنة أوامر التصنيع"), default=True
    )
    ...
    sync_comprehensive_customers = models.BooleanField(
        _("مزامنة العملاء الشاملة"), default=False
    )
    sync_comprehensive_users = models.BooleanField(
        _("مزامنة المستخدمين الشاملة"), default=False
    )
    sync_comprehensive_inventory = models.BooleanField(
        _("مزامنة المنتجات والمخزون الشاملة"), default=False
    )
    sync_comprehensive_system = models.BooleanField(
        _("مزامنة إعدادات النظام الشاملة"), default=False
    )
            ...
            try:
                ...
                if isinstance(credentials, str):
                    try:
                        ...
                        logger.error(
                            "ملف بيانات الاعتماد ليس بالتنسيق المتوقع (نص غير قابل للتحويل)"
                        )
                        return None
                ...
                if missing_fields:
                    logger.error(
                        f"حقول مفقودة في ملف بيانات الاعتماد: {', '.join(missing_fields)}"
                    )
                    return None
...
class GoogleSyncLog(models.Model):
    ...
    config = models.ForeignKey(
        GoogleSyncConfig, on_delete=models.CASCADE, related_name="logs"
    )
...
def sync_with_google_sheets(
    config_id=None, manual=False, full_backup=False, selected_tables=None
):
    """
    ...
    try:
        ...
        if not credentials:
            message = (
                "فشل قراءة بيانات الاعتماد. تأكد من تحميل ملف اعتماد حساب خدمة صالح."
            )
            logger.error(message)
            GoogleSyncLog.objects.create(config=config, status="error", message=message)
            return {"status": "error", "message": message}
        ...
        if (
            selected_tables
            and isinstance(selected_tables, list)
            ...
            if "inspections" in selected_tables:
                inspections_result = sync_inspections(
                    sheets_service, config.spreadsheet_id
                )
        ...
        GoogleSyncLog.objects.create(
            config=config, status="success", message=message, details=sync_results
        )
...
def sync_branches(service, spreadsheet_id):
    ...
    try:
        ...
        fields = [
            f.name
            for f in Branch._meta.get_fields()
        ...
        for f in fields:
            ...
            if (
                ...
                and hasattr(field_obj, "related_model")
                and not field_obj.many_to_one
                and not field_obj.one_to_one
            ):
                continue
...
def create_sheets_service(credentials):
    ...
    try:
        ...
        if isinstance(credentials, dict):
            ...
            if missing_fields:
                raise ValueError(
                    f"حقول مفقودة في بيانات الاعتماد: {', '.join(missing_fields)}"
                )
            ...
            try:
                ...
                raise ValueError(
                    f"فشل إنشاء بيانات الاعتماد من المعلومات المقدمة: {str(e)}"
                )
        ...
        try:
            ...
            if "not found" in error_msg:
                ...
                raise ValueError(
                    "تأكد من مشاركة جدول البيانات مع البريد الإلكتروني لحساب الخدمة"
                )
            else:
...
def sync_databases(service, spreadsheet_id):
    ...
    try:
        ...
        fields = [
            f.name
            for f in Database._meta.get_fields()
            if not f.many_to_many and not f.one_to_many
        ]
        header = []
        for f in fields:
        ...
        return {
            "status": "success",
            "message": f"تمت مزامنة {len(databases)} قاعدة بيانات",
        }
    except Exception as e:
...
def sync_inspections(service, spreadsheet_id):
    ...
    try:
        ...
        for inspection in inspections:
            file_link = (
                inspection.google_drive_file_url
                if getattr(inspection, "google_drive_file_url", None)
                else ""
            )
            ...
            request_date = (
                inspection.request_date.strftime("%Y-%m-%d")
                if getattr(inspection, "request_date", None)
                else ""
            )
            scheduled_date = (
                inspection.scheduled_date.strftime("%Y-%m-%d")
                if getattr(inspection, "scheduled_date", None)
                else ""
            )
            windows_count = (
                inspection.windows_count
                if getattr(inspection, "windows_count", None) is not None
                else ""
            )
            inspector_name = ""
            if hasattr(inspection, "inspector") and inspection.inspector:
                inspector_name = getattr(
                    inspection.inspector, "name", str(inspection.inspector)
                )
            ...
            if (
                ...
                responsible_employee_name = getattr(
                    inspection.responsible_employee,
                    "name",
                    str(inspection.responsible_employee),
                )
            inspection_file_url = (
                inspection.inspection_file.url
                if getattr(inspection, "inspection_file", None)
                else ""
            )
            google_drive_file_name = (
                inspection.google_drive_file_name
                if getattr(inspection, "google_drive_file_name", None)
                else ""
            )
...
def sync_orders(service, spreadsheet_id):
    ...
    try:
        ...
        header = [
            ...
            "رقم العقد 3",
            "ملف العقد",
            "رابط العقد في Google Drive",
            "أنواع الطلب المختارة",
            "أنواع الخدمات",
            "وضع الطلب",
            "حالة الطلب",
            "حالة التتبع",
        ...
        for order in orders:
            ...
            customer_phone2 = (
                ...
                if order.customer and hasattr(order.customer, "phone2")
                else ""
            )
            ...
            # تحضير أنواع الخدمات
            service_types_str = (
                ", ".join(order.service_types) if order.service_types else ""
            )
            ...
            if order.selected_types:
                ...
                for type_code in types_list:
                    if type_code and type_code.strip():  # تجاهل القيم الفارغة
                        selected_types_display.append(
                            type_mapping.get(type_code.strip(), type_code.strip())
                        )
            ...
            row = [
                ...
                (
                    order.last_notification_date.strftime("%Y-%m-%d %H:%M")
                    if order.last_notification_date
                    else ""
                ),
                order.notes or "",
        ...
        return {
            "status": "success",
            "message": f"تمت مزامنة {len(orders)} طلب مع ربطها بالعملاء",
        }
    except Exception as e:
...
def sync_settings(service, spreadsheet_id):
    ...
    try:
        ...
        if company_info:
            social_links = (
                json.dumps(company_info.social_links)
                if company_info.social_links
                else "{}"
            )
            data.append(
                [
                    company_info.version,
                    company_info.release_date,
                    company_info.developer,
                    company_info.working_hours,
                    company_info.name,
                    company_info.copyright_text,
                    company_info.address,
                    company_info.phone,
                    company_info.email,
                    company_info.tax_number,
                    company_info.commercial_register,
                    ...
                    company_info.primary_color,
                    company_info.secondary_color,
                    company_info.accent_color,
                ]
            )
        else:
            data.append([""] * 24)
        ...
        return {
            "status": "success",
            "message": "تمت مزامنة إعدادات الشركة والنظام بنجاح",
        }
    except Exception as e:
    ...
    try:
        ...
        for manufacturing_order in manufacturing_orders:
            ...
            customer_name = (
                ...
                if manufacturing_order.order and manufacturing_order.order.customer
                else ""
            )
            customer_phone = (
                manufacturing_order.order.customer.phone
                if manufacturing_order.order and manufacturing_order.order.customer
                else ""
            )
            order_number = (
                manufacturing_order.order.order_number
            ...
            invoice_number = (
                manufacturing_order.order.invoice_number
                if manufacturing_order.order
                else ""
            )
            invoice_number_2 = (
                manufacturing_order.order.invoice_number_2
                if manufacturing_order.order
                else ""
            )
            invoice_number_3 = (
                manufacturing_order.order.invoice_number_3
                if manufacturing_order.order
                else ""
            )
            contract_number = (
            ...
            contract_number_2 = (
                manufacturing_order.order.contract_number_2
                if manufacturing_order.order
                else ""
            )
            contract_number_3 = (
                manufacturing_order.order.contract_number_3
                if manufacturing_order.order
                else ""
            )
            ...
            row = [
                ...
                (
                    manufacturing_order.get_order_type_display()
                    if manufacturing_order.order_type
                    else ""
                ),
                manufacturing_order.get_status_display(),
                (
                    manufacturing_order.get_priority_display()
                    if hasattr(manufacturing_order, "priority")
                    else ""
                ),
                (
                    manufacturing_order.expected_start_date.strftime("%Y-%m-%d")
                    if manufacturing_order.expected_start_date
                    else ""
                ),
                (
                    manufacturing_order.expected_completion_date.strftime("%Y-%m-%d")
                    if manufacturing_order.expected_completion_date
                    else ""
                ),
                (
                    manufacturing_order.completion_date.strftime("%Y-%m-%d")
                    if manufacturing_order.completion_date
                    else ""
                ),
                manufacturing_order.manufacturing_notes or "",
                (
                    manufacturing_order.created_at.strftime("%Y-%m-%d %H:%M")
                    if manufacturing_order.created_at
                    else ""
                ),
                (
                    manufacturing_order.updated_at.strftime("%Y-%m-%d %H:%M")
        ...
        return {
            "status": "success",
            "message": f"تمت مزامنة {len(manufacturing_orders)} أمر تصنيع",
        }
    except Exception as e:
    ...
    try:
        ...
        for technician in technicians:
            row = [
                ...
                (
                    str(technician.salary)
                    if hasattr(technician, "salary") and technician.salary
                    else ""
                ),
                (
                    technician.hire_date.strftime("%Y-%m-%d")
                    if hasattr(technician, "hire_date") and technician.hire_date
                    else ""
                ),
                "نعم" if technician.is_active else "لا",
                technician.notes or "",
                (
                    technician.created_at.strftime("%Y-%m-%d %H:%M")
                    if technician.created_at
                    else ""
                ),
                (
                    technician.updated_at.strftime("%Y-%m-%d %H:%M")
                    if technician.updated_at
                    else ""
                ),
            ]
            data.append(row)
...
def sync_installation_teams(service, spreadsheet_id):
    ...
    try:
        ...
        teams = InstallationTeam.objects.prefetch_related(
            "technicians", "drivers"
        ).all()
    ...
    try:
        ...
        for salesperson in salespersons:
            row = [
                ...
                (
                    salesperson.created_at.strftime("%Y-%m-%d %H:%M")
                    if salesperson.created_at
                    else ""
                ),
                (
                    salesperson.updated_at.strftime("%Y-%m-%d %H:%M")
                    if salesperson.updated_at
                    else ""
                ),
            ]
            data.append(row)
    ...
    try:
        ...
        if sheet_id:
            try:
                ...
                logger.warning(
                    f"حدث خطأ أثناء تنسيق ورقة: {sheet_name} - {str(format_error)}"
                )
...
def sync_comprehensive_customers(service, spreadsheet_id):
    ...
    try:
        ...
        customers = Customer.objects.prefetch_related(
            "customer_orders", "inspections", "customer_orders__manufacturing_order"
        ).all()
        ...
        for customer in customers:
            ...
            manufacturing_orders_count = customer.customer_orders.filter(
                manufacturing_order__isnull=False
            ).count()

            # حساب إجمالي قيمة الطلبات
            total_orders_value = sum(
                [order.total_amount or 0 for order in customer.customer_orders.all()]
            )
            ...
            if last_order:
                ...
                invoice_numbers = [
                    num
                    for num in [
                        last_order.invoice_number,
                        last_order.invoice_number_2,
                        last_order.invoice_number_3,
                    ]
                    if num
                ]
                last_invoice_number = (
                    ", ".join(invoice_numbers) if invoice_numbers else ""
                )
                ...
                contract_numbers = [
                    num
                    for num in [
                        last_order.contract_number,
                        last_order.contract_number_2,
                        last_order.contract_number_3,
                    ]
                    if num
                ]
                last_contract_number = (
                    ", ".join(contract_numbers) if contract_numbers else ""
                )
        ...
        return {
            "status": "success",
            "message": f"تمت مزامنة {len(customers)} عميل مع بياناتهم الشاملة",
        }
    except Exception as e:
...
def sync_comprehensive_users(service, spreadsheet_id):
    ...
    try:
        ...
        return {
            "status": "success",
            "message": f"تمت مزامنة {len(users)} مستخدم مع بياناتهم الشاملة",
        }
    except Exception as e:
...
def sync_comprehensive_inventory(service, spreadsheet_id):
    ...
    try:
        ...
        return {
            "status": "success",
            "message": f"تمت مزامنة {len(products)} منتج مع بيانات المخزون الشاملة",
        }
    except Exception as e:
...
def sync_comprehensive_system_settings(service, spreadsheet_id):
    ...
    try:
        ...
        header = ["نوع البيانات", "العدد الإجمالي", "النشط", "غير النشط", "ملاحظات"]
...
def sync_complete_orders_lifecycle(service, spreadsheet_id):
    ...
    try:
        ...
        orders = (
            ...
            .prefetch_related("installation_schedules", "inspections")
            .all()
        )
        ...
        header = [
            ...
            "نوع أمر التصنيع",
            "تاريخ بدء التصنيع",
            "تاريخ انتهاء التصنيع المتوقع",
            "تاريخ إكمال التصنيع الفعلي",
            "رقم إذن الخروج",
            "ملاحظات التصنيع",
            # بيانات التركيب
            "حالة التركيب",
            "فريق التركيب",
            "تاريخ التركيب المجدول",
            "وقت التركيب",
            "نوع مكان التركيب",
            "عنوان التركيب",
            "تاريخ إكمال التركيب",
            ...
            "عدد الشبابيك",
            "تاريخ طلب المعاينة",
            "تاريخ تنفيذ المعاينة",
            "وقت المعاينة",
            "حالة السداد للمعاينة",
            "ملاحظات المعاينة",
            # بيانات التسليم
            "تم التسليم",
            "رقم إذن التسليم",
        ...
        for order in orders:
            ...
            mfg_code = (
                manufacturing_order.manufacturing_code if manufacturing_order else ""
            )
            mfg_status = (
                manufacturing_order.get_status_display() if manufacturing_order else ""
            )
            mfg_type = (
                manufacturing_order.get_order_type_display()
            ...
            mfg_start_date = (
                manufacturing_order.order_date.strftime("%Y-%m-%d")
                if manufacturing_order and manufacturing_order.order_date
                else ""
            )
            mfg_expected_end = (
                manufacturing_order.expected_delivery_date.strftime("%Y-%m-%d")
                if manufacturing_order and manufacturing_order.expected_delivery_date
                else ""
            )
            mfg_actual_completion = (
                manufacturing_order.completion_date.strftime("%Y-%m-%d")
                if manufacturing_order and manufacturing_order.completion_date
                else ""
            )
            mfg_exit_permit = (
                manufacturing_order.exit_permit_number if manufacturing_order else ""
            )
            mfg_notes = manufacturing_order.notes if manufacturing_order else ""

            # بيانات التركيب
            installation = (
                order.installation_schedules.first()
                if hasattr(order, "installation_schedules")
                else None
            )
            inst_status = installation.get_status_display() if installation else ""
            inst_team = (
                installation.team.name if installation and installation.team else ""
            )
            inst_scheduled_date = (
                installation.scheduled_date.strftime("%Y-%m-%d")
                if installation and installation.scheduled_date
                else ""
            )
            inst_scheduled_time = (
                installation.scheduled_time.strftime("%H:%M")
                if installation and installation.scheduled_time
                else ""
            )
            inst_location_type = (
                installation.get_location_type_display()
                if installation and installation.location_type
                else ""
            )
            inst_location_address = (
                installation.location_address if installation else ""
            )
            inst_completion_date = (
                installation.completion_date.strftime("%Y-%m-%d %H:%M")
                if installation and installation.completion_date
                else ""
            )
            ...
            # بيانات المعاينة
            inspection = (
                order.inspections.first() if hasattr(order, "inspections") else None
            )
            ...
            insp_result = (
                ...
                if inspection and inspection.result
                else ""
            )
            insp_inspector = (
            ...
            insp_windows_count = (
                str(inspection.windows_count)
                if inspection and inspection.windows_count
                else ""
            )
            insp_request_date = (
                inspection.request_date.strftime("%Y-%m-%d")
                if inspection and inspection.request_date
                else ""
            )
            insp_scheduled_date = (
                inspection.scheduled_date.strftime("%Y-%m-%d")
                if inspection and inspection.scheduled_date
                else ""
            )
            insp_scheduled_time = (
                inspection.scheduled_time.strftime("%H:%M")
                if inspection and inspection.scheduled_time
                else ""
            )
            insp_payment_status = (
                inspection.get_payment_status_display() if inspection else ""
            )
            ...
            is_delivered = (
                "نعم"
                if manufacturing_order and manufacturing_order.status == "delivered"
                else "لا"
            )
            delivery_permit = (
                manufacturing_order.delivery_permit_number
                if manufacturing_order
                else ""
            )
            delivery_recipient = (
                manufacturing_order.delivery_recipient_name
                if manufacturing_order
                else ""
            )
            delivery_date = (
                manufacturing_order.delivery_date.strftime("%Y-%m-%d %H:%M")
                if manufacturing_order and manufacturing_order.delivery_date
                else ""
            )
        ...
        return {
            "status": "success",
            "message": f"تمت مزامنة دورة حياة {len(orders)} طلب كاملة",
        }
    except Exception as e:
...
def reverse_sync_from_google_sheets(
    service, spreadsheet_id, admin_password, delete_old_data=False
):
    """
    ...
    try:
        ...
        if not admin_user:
            ...
            logger.warning(
                f"كلمة مرور خاطئة. المستخدمين المديرين المتاحين: {admin_usernames}"
            )
            return {
                "status": "error",
                "message": f'كلمة مرور المدير غير صحيحة. المستخدمين المديرين: {", ".join(admin_usernames)}',
            }
        ...
        try:
            ...
            if delete_old_data:
                Order = apps.get_model("orders", "Order")
                ManufacturingOrder = apps.get_model(
                    "manufacturing", "ManufacturingOrder"
                )
                InstallationSchedule = apps.get_model(
            ...
            return {
                "status": "error",
                "message": f"خطأ في قراءة البيانات من Google Sheets: {str(e)}",
            }
