...
class AdvancedSyncService:
    def _create_manufacturing_order(self, mapped_data: Dict[str, str], order: Order):
        ...
        try:
            ...
            manufacturing_data = {
                ...
                "expected_delivery_date": (order.order_date or timezone.now().date())
                + timedelta(days=14),
    ...
    def _create_inspection(
        self, mapped_data: Dict[str, str], order: Order
    ) -> Inspection:
        ...
        if existing_inspection:
            logger.info(
                f"معاينة موجودة مسبقاً للطلب {order.order_number}: {existing_inspection.id}"
            )
        ...
        try:
            ...
            logger.info(
                f"تم إنشاء معاينة جديدة للطلب {order.order_number}: {inspection.id}"
            )
            return inspection
        except Exception as e:
            import traceback
    ...
    def sync_from_sheets(
        self, task: GoogleSyncTask = None, use_fast_mode: bool = True
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if not task:
                task = GoogleSyncTask.objects.create(
                    mapping=self.mapping, task_type="import", created_by=None
                )
            ...
            return {"success": True, "stats": self.stats, "task_id": task.id}
    ...
    def _process_sheet_data(self, sheet_data: List[List[str]], task: GoogleSyncTask):
        ...
        for batch_start in range(0, total_rows, batch_size):
                ...
                for i, row in enumerate(batch_rows):
                    ...
                    try:
                        ...
                        if not customer:
                            ...
                            row_details["actions"].append(
                                f"❌ تخطي: لا يوجد اسم عميل صالح"
                            )
                        ...
                        has_valid_inspection_date = self._is_valid_inspection_date(
                            inspection_date
                        )
                        ...
                        main_order = None
                        if order_type and order_type in [
                            "installation",
                            "accessory",
                            "tailoring",
                        ]:
                            # التحقق من وجود طلب مماثل للعميل بنفس النوع
                            existing_main_order = self._find_existing_order(
                                customer, order_type, contract_number, invoice_number
                            )

                            if existing_main_order:
                                main_order = existing_main_order
                                row_details["actions"].append(
                                    f"🔄 طلب موجود {order_type}: {main_order.order_number}"
                                )

                                # تحديث الطلب الموجود إذا لزم الأمر
                                if self.mapping.update_existing:
                                    updated = self._update_order(
                                        main_order, mapped_data, customer
                                    )
                                    if updated:
                                        self.stats["orders_updated"] += 1
                                        row_details["actions"].append(
                                            f"✅ تم تحديث الطلب: {main_order.order_number}"
                                        )
                            else:
                                main_order = self._create_order(
                                    mapped_data, customer, order_type
                                )
                                row_details["actions"].append(
                                    f"✅ طلب جديد {order_type}: {main_order.order_number}"
                                )

                            # إنشاء أمر تصنيع للطلب الأساسي (إذا لم يكن موجوداً)
                            try:
                                manufacturing_order = self._create_manufacturing_order(
                                    mapped_data, main_order
                                )
                                if manufacturing_order:
                                    self.stats["manufacturing_orders_created"] += 1
                                    row_details["actions"].append(
                                        f"✅ أمر تصنيع للطلب: {main_order.order_number}"
                                    )
                            except Exception as e:
                                self.stats["manufacturing_orders_failed"] += 1
                                row_details["actions"].append(
                                    f"❌ فشل أمر التصنيع: {str(e)}"
                                )
                        else:
                            self.stats["orders_skipped"] += 1
                            if not order_type:
                                row_details["actions"].append(
                                    f"❌ نوع طلب غير معروف: '{raw_order_type}'"
                                )
                            else:
                                row_details["actions"].append(
                                    f"❌ نوع طلب غير مدعوم: '{order_type}'"
                                )
                        ...
                        if has_valid_inspection_date:
                            ...
                            if parsed_inspection_date:
                                ...
                                inspection_order = (
                                    self._find_or_create_inspection_order(
                                        mapped_data,
                                        customer,
                                        inspection_date,
                                        contract_number,
                                ...
                                if inspection_order:
                                    ...
                                    if not existing_inspection:
                                        row_details["actions"].append(
                                            f"✅ طلب معاينة: {inspection_order.order_number}"
                                        )

                                        # إنشاء المعاينة مرتبطة بطلب المعاينة
                                        try:
                                            inspection = self._create_inspection(
                                                mapped_data, inspection_order
                                            )
                                            row_details["actions"].append(
                                                f"✅ معاينة جديدة: {parsed_inspection_date.date()}"
                                            )
                                        except Exception as e:
                                            row_details["actions"].append(
                                                f"❌ فشل إنشاء المعاينة: {str(e)}"
                                            )
                                    else:
                                        row_details["actions"].append(
                                            f"🔄 طلب معاينة موجود: {inspection_order.order_number}"
                                        )
                                        row_details["actions"].append(
                                            f"⚠️ معاينة موجودة مسبقاً للطلب: {existing_inspection.id}"
                                        )
                        else:
                            if inspection_date:
                                row_details["actions"].append(
                                    f"❌ تاريخ معاينة غير صالح: '{inspection_date}'"
                                )
                        ...
                        error_details = {
                            "row": row_index,
                            "customer_name": (
                                mapped_data.get("customer_name", "")
                                if "mapped_data" in locals()
                                else ""
                            ),
                            "order_type": (
                                mapped_data.get("order_type", "")
                                if "mapped_data" in locals()
                                else ""
                            ),
            ...
            print(
                f"[SYNC] التقدم: {progress:.1f}% - تمت معالجة {batch_end} من {total_rows} صف"
            )
    ...
    def _print_detailed_report(self):
        ...
        print(
            f"⚡ معدل النجاح: {(self.stats['successful_rows']/self.stats['total_rows']*100):.1f}%"
        )
        ...
        print(
            f"  ⚠️ ملاحظة: أوامر التصنيع تُنشأ فقط للطلبات ��لأساسية، وليس لطلبات المعاينة"
        )
        ...
        if self.stats["detailed_errors"]:
            ...
            for i, row_detail in enumerate(self.stats["detailed_errors"][:10]):
                ...
                print(
                    f"     📅 تاريخ المعاينة: {row_detail.get('inspection_date', 'غير محدد')}"
                )
                print(f"     🔧 الإجراءات:")
    ...
    def _get_sheet_data(self) -> List[List[str]]:
        ...
        try:
            ...
            logger.error(
                f"خطأ في جلب البيانات من Google Sheets: {str(e)}", exc_info=True
            )
            raise
    ...
    def _process_customer(
        self, mapped_data: Dict[str, str], row_index: int, task: GoogleSyncTask
    ) -> Optional[Customer]:
        """
    ...
    def _parse_date(self, value):
        ...
        try:
            ...
            dt = parser.parse(
                str(value).strip(), dayfirst=True, yearfirst=False, fuzzy=True
            )
            if dt.tzinfo is None:
                dt = timezone.make_aware(dt)
            return dt
    ...
    def _create_customer(self, mapped_data: Dict[str, str]) -> Customer:
        ...
        if (
            hasattr(self.mapping, "default_customer_category")
            and self.mapping.default_customer_category
        ):
            customer_data["category"] = self.mapping.default_customer_category
        if (
            hasattr(self.mapping, "default_customer_type")
            and self.mapping.default_customer_type
        ):
        ...
        if not getattr(self.mapping, "use_current_date_as_created", False):
            ...
            if created_at_str:
                ...
                if created_at_dt:
                    ...
                    logger.info(
                        f"تم تعيين تاريخ إضافة العميل من تاريخ الطلب: {created_at_dt}"
                    )
    ...
    def _update_customer(self, customer: Customer, mapped_data: Dict[str, str]) -> bool:
        ...
        if updates:
            ...
            logger.info(
                f"تم تحديث بيانات العميل {customer.id} - التغييرات: {', '.join(updates.keys())}"
            )
            return True
    ...
    def _create_order(
        self, mapped_data: Dict[str, str], customer: Customer, order_type: str = None
    ) -> Order:
        ...
        order_data["invoice_number"] = (
            invoice_number if invoice_number else "لا يوجد رقم فاتورة"
        )
        ...
        if tracking_status:
            ...
            order_data["tracking_status"] = status_mapping.get(
        ...
        if salesperson_name:
            salesperson = Salesperson.objects.filter(
                name__icontains=salesperson_name
            ).first()
            if salesperson:
    ...
    def _find_existing_order(
        ...
        try:
            ...
            query = Order.objects.filter(
                customer=customer, selected_types__contains=[order_type]
            )
    ...
    def _find_or_create_inspection_order(
        self,
        ...
        try:
            ...
            inspection_order = self._create_order(
                inspection_mapped_data, customer, "inspection"
            )
    ...
    def _update_order(
        self, order: Order, mapped_data: Dict[str, str], customer: Customer
    ) -> bool:
        """
        ...
        if updates:
            ...
            logger.info(
                f"تم تحديث الطلب {order.id} - التغييرات: {', '.join(updates.keys())}"
            )
            return True
    ...
    def _process_inspection(
        ...
        try:
            ...
            if inspection_date:
                try:
                    parsed_date = parser.parse(
                        inspection_date, dayfirst=True, yearfirst=False, fuzzy=True
                    )
                    if isinstance(parsed_date, datetime):
                        inspection_data["scheduled_date"] = (
                            parsed_date.date()
                            if hasattr(parsed_date, "date")
                            else parsed_date
                        )
                        inspection_data["request_date"] = (
                            parsed_date.date()
                            if hasattr(parsed_date, "date")
                            else parsed_date
                        ) - timedelta(days=1)
                        ...
                        inspection_data["scheduled_date"] = parsed_date
                        inspection_data["request_date"] = parsed_date - timedelta(
                            days=1
                        )
                except (ValueError, ParserError):
                    inspection_data["notes"] = (
                        f"تاريخ المعاينة: {inspection_date}\n{inspection_data.get('notes', '')}"
                    )
            # نتيجة المعاينة
            ...
            if inspection_result:
                ...
                inspection_data["result"] = result_mapping.get(
                    inspection_result, "approved"
                )
            else:
                inspection_data["result"] = (
                    "approved"  # جميع المعاينات تعتبر مكتملة وناجحة
                )
    ...
    def _update_inspection(
        self, inspection: Inspection, mapped_data: Dict[str, str]
    ) -> bool:
        """
        ...
        if inspection_result:
            ...
            new_result = result_mapping.get(
                inspection_result,
                inspection.result if hasattr(inspection, "result") else "pending",
            )
        ...
        if windows_count:
            try:
                ...
                if (
                    hasattr(inspection, "windows_count")
                    and new_count != inspection.windows_count
                ):
        ...
        if inspection_date:
            try:
                ...
                if (
                    hasattr(inspection, "scheduled_date")
                    and parsed_date != inspection.scheduled_date
                ):
        ...
        if updates:
            ...
            logger.info(
                f"تم تحديث المعاينة {inspection.id} - التغييرات: {', '.join(updates.keys())}"
            )
            return True
    ...
    def _process_installation(
        ...
        try:
            ...
            if installation_status:
                ...
                installation_data["status"] = status_mapping.get(
            ...
            self.stats["errors"].append(
                f"خطأ في معالجة التركيب في الصف {row_index}: {str(e)}"
            )
            raise
...
class SyncScheduler:
    ...
    def run_scheduled_syncs(self):
        ...
        due_schedules = GoogleSyncSchedule.objects.filter(
            is_active=True, next_run__lte=timezone.now()
        )
