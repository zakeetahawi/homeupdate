...
from .models import BackupSchedule, Database, GoogleDriveConfig
...
class GoogleDriveConfigForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            "name": forms.TextInput(attrs={"class": "form-control"}),
            "inspections_folder_id": forms.TextInput(
                attrs={
                    "class": "form-control",
                    "placeholder": "مثال: 1BxiMVs0XRA5nFMdKvBdBZjgmUUqptlbs74OgvE2upms",
                }
            ),
            "inspections_folder_name": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "مثال: ملفات المعاينات"}
            ),
            "contracts_folder_id": forms.TextInput(
                attrs={
                    "class": "form-control",
                    "placeholder": "مثال: 1BxiMVs0XRA5nFMdKvBdBZjgmUUqptlbs74OgvE2upms",
                }
            ),
            "contracts_folder_name": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "مثال: ملفات العقود"}
            ),
            "credentials_file": forms.FileInput(
                attrs={"class": "form-control", "accept": ".json"}
            ),
            "filename_pattern": forms.TextInput(
...
class ImportSelectionForm(forms.Form):
    ...
    import_all = forms.BooleanField(
        label=_("استيراد كل البيانات"), required=False, initial=True
    )
    page_start = forms.IntegerField(label=_("من الصفحة"), required=False, min_value=1)
    page_end = forms.IntegerField(label=_("إلى الصفحة"), required=False, min_value=1)
        ...
        if not import_all:
            ...
            if page_start > page_end:
                raise forms.ValidationError(
                    _("يجب أن تكون الصفحة الأولى أقل من أو تساوي الصفحة الأخيرة")
                )
