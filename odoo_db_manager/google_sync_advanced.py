...
class GoogleSheetMapping(models.Model):
    ...
    sheet_name = models.CharField(
        max_length=200, verbose_name="اسم الورقة", default="Sheet1"
    )
    ...
    last_row_processed = models.PositiveIntegerField(
        null=True, blank=True, verbose_name="آخر صف تم معالجته"
    )
    ...
    auto_create_customers = models.BooleanField(
        default=True, verbose_name="إنشاء عملاء تلقائياً"
    )
    auto_create_orders = models.BooleanField(
        default=True, verbose_name="إنشاء طلبات تلقائياً"
    )
    auto_create_inspections = models.BooleanField(
    ...
    enable_reverse_sync = models.BooleanField(
    ...
    row_filter_conditions = models.JSONField(
        default=dict, blank=True, verbose_name="شروط فلترة الصفوف"
    )
    data_validation_rules = models.JSONField(
        default=dict, blank=True, verbose_name="قواعد التحقق من البيانات"
    )
    ...
    use_current_date_as_created = models.BooleanField(
        default=False, verbose_name="استخدام التاريخ الحالي كتاريخ الإضافة"
    )
    ...
    def has_valid_mappings(self):
        ...
        return bool(
            mappings and any(field_type != "ignore" for field_type in mappings.values())
        )
    ...
    def validate_mappings(self):
        ...
        if self.auto_create_customers:
            ...
            if not customer_fields:
                errors.append(
                    "يجب تعيين على الأقل حقل واحد للعميل عند تفعيل الإنشاء التلقائي للعملاء"
                )
        ...
        if self.auto_create_orders:
            ...
            if not order_fields:
                errors.append(
                    "يجب تعيين على الأقل حقل واحد للطلب عند تفعيل الإنشاء التلقائي للطلبات"
                )
...
class GoogleSyncTask(models.Model):
    ...
    task_type = models.CharField(
        max_length=20, choices=TASK_TYPES, verbose_name="نوع المهمة"
    )
    status = models.CharField(
        max_length=20, choices=TASK_STATUS, default="pending", verbose_name="الحالة"
    )
    ...
    rows_successful = models.PositiveIntegerField(
    ...
    task_parameters = models.JSONField(
        default=dict, blank=True, verbose_name="معاملات المهمة"
    )
...
class GoogleSyncConflict(models.Model):
    ...
    conflict_type = models.CharField(
        max_length=30, choices=CONFLICT_TYPES, verbose_name="نوع التعارض"
    )
    ...
    existing_data = models.JSONField(
        null=True, blank=True, verbose_name="البيانات الموجودة"
    )
    suggested_action = models.CharField(
        max_length=100, blank=True, verbose_name="الإجراء المقترح"
    )
    ...
    resolution_action = models.CharField(
        max_length=100, blank=True, verbose_name="إجراء الحل"
    )
    ...
    def ignore(self, notes=None):
        ...
        self.resolved_at = timezone.now()
        self.save(
            update_fields=["resolution_status", "resolution_notes", "resolved_at"]
        )
...
class GoogleSyncSchedule(models.Model):
    ...
    name = models.CharField(
        max_length=200, default="جدولة افتراضية", verbose_name="اسم الجدولة"
    )
    is_active = models.BooleanField(default=True, verbose_name="نشط")
    ...
    next_run = models.DateTimeField(
        null=True, blank=True, verbose_name="التنفيذ القادم"
    )
