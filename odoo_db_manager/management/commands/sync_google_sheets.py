...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--mapping-id", type=int, help="معرف التعيين المحدد للمزامنة"
        )

        parser.add_argument("--mapping-name", type=str, help="اسم التعيين للمزامنة")

        parser.add_argument(
            "--all", action="store_true", help="مزامنة جميع التعيينات النشطة"
        )

        parser.add_argument(
            "--scheduled", action="store_true", help="تشغيل المزامنة المجدولة فقط"
        )
        ...
        parser.add_argument(
            "--validate", action="store_true", help="التحقق من صحة التعيينات فقط"
        )
    ...
    def handle(self, *args, **options):
        try:
            if options["validate"]:
                ...
                self.sync_mapping_by_id(
                    options["mapping_id"], options["reverse"], options["dry_run"]
                )
            elif options["mapping_name"]:
                self.sync_mapping_by_name(
                    options["mapping_name"], options["reverse"], options["dry_run"]
                )
            else:
                self.stdout.write(
                    self.style.ERROR(
                        "يجب تحديد أحد الخيارات: --mapping-id, --mapping-name, --all, --scheduled, أو --validate"
                    )
                )
                return
        ...
        for mapping in mappings:
            try:
                ...
                if errors:
                    ...
                    self.stdout.write(self.style.SUCCESS(f"✅ {mapping.name}: صحيح"))
                    valid_count += 1
            except Exception as e:
                self.stdout.write(
    ...
    def sync_all_mappings(self, reverse=False, dry_run=False):
        ...
        self.stdout.write(
            f"جاري {action} جميع التعيينات النشطة - المزامنة {sync_type}..."
        )
        ...
        if not mappings.exists():
            message = (
                "لا توجد تعيينات نشطة مع المزامنة العكسية"
                if reverse
                else "لا توجد تعيينات نشطة"
            )
        ...
        for mapping in mappings:
            try:
                if dry_run:
                    self.stdout.write(
                        f"📋 سيتم مزامنة: {mapping.name} ({mapping.sheet_name})"
                    )
                    success_count += 1
                else:
    ...
    def sync_mapping_by_id(self, mapping_id, reverse=False, dry_run=False):
        ...
        try:
            ...
            if result["success"]:
                ...
                self.stdout.write(
                    self.style.ERROR(
                        f"❌ فشلت مزامنة {mapping.name}: {result.get('error')}"
                    )
                )
    ...
    def sync_single_mapping(self, mapping, reverse=False):
        ...
        task = GoogleSyncTask.objects.create(
            mapping=mapping, task_type=task_type, created_by=user
        )
