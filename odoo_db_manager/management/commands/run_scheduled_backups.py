...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS("🔄 بدء فحص النسخ الاحتياطية المجدولة..."))
        ...
        if not schedules:
            self.stdout.write(self.style.WARNING("⚠️ لا توجد جدولات نسخ احتياطية نشطة"))
            return
        ...
        for schedule in schedules:
            try:
                ...
                self.stdout.write(
                    f'   آخر تشغيل: {schedule.last_run or "لم يتم التشغيل بعد"}'
                )
                self.stdout.write(
                    f'   التشغيل القادم: {schedule.next_run or "غير محدد"}'
                )
                ...
                if should_run:
                    if dry_run:
                        self.stdout.write(
                            self.style.WARNING(
                                "   🔍 سيتم تشغيل هذه الجدولة (وضع المعاينة)"
                            )
                        )
                        executed_count += 1
                    else:
                        ...
                        if backup:
                            self.stdout.write(
                                self.style.SUCCESS(
                                    f"   ✅ تم إنشاء النسخة الاحتياطية: {backup.name}"
                                )
                            )
                            executed_count += 1
                        else:
                    ...
                    self.stdout.write(self.style.WARNING("   ⏭️ لا تحتاج للتشغيل الآن"))
                    skipped_count += 1
    ...
    def _should_run_now(self, schedule, now):
        ...
        if schedule.last_run:
            ...
            if schedule.frequency == "hourly" and time_since_last > timedelta(hours=2):
                ...
                return True
            elif schedule.frequency == "weekly" and time_since_last > timedelta(
                weeks=2
            ):
                return True
            elif schedule.frequency == "monthly" and time_since_last > timedelta(
                days=60
            ):
                return True
