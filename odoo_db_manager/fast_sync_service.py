...
class FastSyncService:
                ...
                for i, row in enumerate(batch_rows):
                    try:
                        ...
                        self.stats["errors"].append(
                            f"خطأ في الصف {batch_start + i + 2}: {str(e)}"
                        )

        print(
            f"[FAST_SYNC] اكتملت المعالجة السريعة - {self.stats['successful_rows']} صف ناجح"
        )
        self._print_fast_summary()

    def _find_or_create_customer_fast(
        self,
        mapped_data: Dict[str, str],
        existing_customers: Dict,
        existing_customers_by_name: Dict,
    ) -> Optional[Customer]:
        ...
        if self.mapping.auto_create_customers:
            try:
                ...
                if (
                    hasattr(self.mapping, "default_branch")
                    and self.mapping.default_branch
                ):
                ...
                if not getattr(self.mapping, "use_current_date_as_created", False):
                    ...
                    if created_at_str:
                        ...
                        if created_at_dt:
                            ...
                            logger.info(
                                f"[FAST_SYNC] تم تعيين تاريخ إضافة العميل من تاريخ الطلب: {created_at_dt}"
                            )

                logger.info(
                    f"[FAST_SYNC] تم إنشاء عميل جديد: {customer.name} (تصنيف: {customer.category}, نوع: {customer.customer_type})"
                )
    ...
    def _process_orders_fast(self, mapped_data: Dict[str, str], customer: Customer):
        ...
        if has_valid_inspection_date:
            ...
            inspection_order = self._create_order_fast(
    ...
    def _create_order_fast(
        self, mapped_data: Dict[str, str], customer: Customer, order_type: str
    ) -> Order:
    ...
    def _create_manufacturing_order_fast(
    ...
    def _create_inspection_fast(self, mapped_data: Dict[str, str], order: Order):
        ...
        try:
            ...
            logger.error(
                f"[FAST_SYNC] فشل إنشاء المعاينة للطلب {order.order_number}: {str(e)}"
            )
            raise
