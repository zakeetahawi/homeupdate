...
class DatabaseAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("name", "db_type", "is_active")}),
        (
            _("معلومات الاتصال"),
            {"fields": ("connection_info",), "classes": ("collapse",)},
        ),
        (
            _("معلومات النظام"),
...
class GoogleDriveConfigAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (
            _("ملف الاعتماد"),
            {"fields": ("credentials_file",), "classes": ("collapse",)},
        ),
        (
            _("إعدادات تسمية الملفات"),
            {"fields": ("filename_pattern",), "classes": ("collapse",)},
        ),
        (
            _("الإحصائيات"),
            {"fields": ("total_uploads", "last_upload"), "classes": ("collapse",)},
        ),
        (
            _("معلومات النظام"),
            {"fields": ("created_at", "updated_at"), "classes": ("collapse",)},
...
class GoogleSyncTaskAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("mapping", "task_type", "status", "error_log")}),
        (
            _("التوقيتات"),
            {
        ...
        (
            _("معاملات إضافية"),
            {"fields": ("task_parameters", "result"), "classes": ("collapse",)},
        ),
    )
    list_per_page = 20
    ...
    def get_success_rate(self, obj):
        return (
            f"{obj.get_success_rate()}%" if hasattr(obj, "get_success_rate") else "N/A"
        )
...
class GoogleSyncConflictAdmin(admin.ModelAdmin):
    ...
    def get_conflict_description(self, obj):
        return (
            obj.get_conflict_description()
            if hasattr(obj, "get_conflict_description")
            else "غير متوفر"
        )
    ...
    def mark_as_resolved(self, request, queryset):
        updated = queryset.update(
            resolution_status="resolved", resolved_at=timezone.now()
        )
    ...
    def mark_as_ignored(self, request, queryset):
        updated = queryset.update(
            resolution_status="ignored", resolved_at=timezone.now()
        )
...
class GoogleSyncScheduleAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("name", "mapping", "is_active")}),
        (
            _("إعدادات الجدولة"),
...
class GoogleSyncLogAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("config", "status", "message")}),
        (_("التفاصيل"), {"fields": ("details",), "classes": ("collapse",)}),
        (_("معلومات النظام"), {"fields": ("created_at",), "classes": ("collapse",)}),
    )
