...
class DatabaseService:
    ...
    def create_database(self, name, db_type, connection_info, force_create=False):
        ...
        try:
            ...
            if db_type == "postgresql":
                ...
                if not force_create and self._check_postgresql_db_exists(
                    ...
                    raise ValueError(
                        f"قاعدة البيانات '{pg_db_name}' موجودة بالفعل في PostgreSQL"
                    )
                ...
                self._create_sqlite_database(name=connection_info["NAME"])
        except Exception as e:
        ...
        database = Database.objects.create(
            name=name, db_type=db_type, connection_info=connection_info
        )
    ...
    def _update_settings_file(self, database):
        ...
        if database:
            ...
            db_settings = {"active_db": None, "databases": {}}
            ...
            for row in cursor.fetchall():
                ...
                if db_name and db_name not in ["postgres", "template0", "template1"]:
                    ...
                    is_registered = Database.objects.filter(
                        name=db_name, db_type="postgresql"
                    ).exists()
        ...
        for db_info in discovered_dbs:
            try:
                ...
                existing_db = Database.objects.filter(
                    name=db_info["name"], db_type="postgresql"
                ).first()
        ...
        try:
            ...
            for db_id, db_info in db_settings.get("databases", {}).items():
                ...
                try:
                    ...
                    database.name = (
                        os.path.basename(db_name) if db_type == "sqlite3" else db_name
                    )
        ...
        try:
            ...
            if database.db_type == "postgresql":
                ...
                self._delete_sqlite_database(
                    name=database.connection_info.get(
                        "NAME", f"{database.name}.sqlite3"
                    )
                )
        except Exception as e:
    ...
    def _delete_postgresql_database(self, name, user, password, host, port):
        ...
        drop_cmd = [
            ...
            user,
            "-c",
            f'DROP DATABASE IF EXISTS "{safe_name}";',
        ]
    ...
    def create_physical_database(self, connection_info, force_create=False):
        ...
        db_type = (
            connection_info.get("ENGINE", "").split(".")[-1]
            if "ENGINE" in connection_info
            else "postgresql"
        )
        ...
        try:
            if db_type == "postgresql":
                ...
                return (
                    True,
                    f"تم إنشاء قاعدة البيانات '{pg_db_name}' في PostgreSQL بنجاح",
                )
