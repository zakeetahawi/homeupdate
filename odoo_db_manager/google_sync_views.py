...
from .google_sync import (
    ...
    reverse_sync_from_google_sheets,
    sync_customers,
    sync_databases,
    sync_inspections,
    sync_orders,
...
def google_sync(request):
    ...
    connection_status_cache_key = (
        f'google_sync_connection_status_{config.id if config else "none"}'
    )
...
def google_sync_config(request):
    ...
    try:
        ...
        job_executions = DjangoJobExecution.objects.filter(
            job_id="google_sync_job"
        ).order_by("-run_time")
...
def google_sync_now(request):
    try:
        ...
        if result["status"] == "success":
            ...
            messages.error(
                request, f'فشلت المزامنة مع Google Sheets: {result["message"]}'
            )
...
def google_sync_test(request):
    try:
        ...
        if not sheets_service:
            messages.error(
                request,
                f"فشل إنشاء خدمة Google Sheets. تحقق من صحة بيانات الاعتماد ومشاركة جدول البيانات مع البريد: {credentials.get('client_email', 'غير معروف')}",
            )
            return redirect("odoo_db_manager:google_sync")
        spreadsheet = (
        ...
        messages.success(
            request, f'تم الاتصال بنجاح بجدول البيانات "{spreadsheet_title}"'
        )
    except Exception as e:
        logger.error(f"فشل اختبار الاتصال: {str(e)}")
...
def google_sync_advanced_settings(request):
    try:
        ...
        return JsonResponse(
            {"status": "success", "message": "تم حفظ الإعدادات المتقدمة بنجاح"}
        )
    except Exception as e:
...
def reverse_sync_view(request):
    ...
    try:
        ...
        if not admin_password:
            return JsonResponse(
                {"status": "error", "message": "يرجى إدخال كلمة مرور المدير"}
            )
        ...
        if not credentials:
            return JsonResponse(
                {"status": "error", "message": "لا يمكن قراءة بيانات الاعتماد"}
            )

        sheets_service = create_sheets_service(credentials)
        if not sheets_service:
            return JsonResponse(
                {"status": "error", "message": "فشل في الاتصال بـ Google Sheets"}
            )

        # تنفيذ المزامنة العكسية
        result = reverse_sync_from_google_sheets(
            sheets_service, config.spreadsheet_id, admin_password, delete_old_data
        )
        ...
        GoogleSyncLog.objects.create(
            ...
            details={
                ...
                "admin_user": (
                    admin_password[:3] + "***" if admin_password else "غير محدد"
                ),  # إخفاء كلمة المرور
            },
        )
        ...
        return JsonResponse(
            {"status": "error", "message": f"حدث خطأ غير متوقع: {str(e)}"}
        )
