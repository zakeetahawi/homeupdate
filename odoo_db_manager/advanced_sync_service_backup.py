...
class AdvancedSyncService:
        ...
        try:
            ...
            for i, row_data in enumerate(data_rows):
                ...
                try:
                    ...
                    if (
                        not mapped_data.get("customer_category")
                        and self.mapping.default_customer_category
                    ):
                        mapped_data["customer_category"] = (
                            self.mapping.default_customer_category.name
                        )

                    # معالجة العميل (دائمًا لكل صف)
                    customer_key = (
                        mapped_data.get("customer_name", "").strip(),
                        mapped_data.get("customer_phone", "").strip(),
                    )
                    customer = customer_cache.get(customer_key)
                    if not customer:
                        customer = self._process_customer(
                            mapped_data, sheet_row_number, task
                        )
                    ...
                    if not invoice_number:
                        self.stats["errors"].append(
                            f"الصف {sheet_row_number}: رقم الفاتورة مطلوب لجميع الطلبات"
                        )
                        continue
                    ...
                    if order_key in imported_orders:
                        self.stats["warnings"].append(
                            f"الصف {sheet_row_number}: طلب مكرر بنفس رقم الفاتورة {invoice_number}"
                        )
                        continue
                    ...
                    if not order:
                        ...
                        order = self._process_order(
                            mapped_data, customer, sheet_row_number, task
                        )
                        if order:
                            imported_orders.add(order_key)
                            self.stats["orders_created"] += 1
                            logger.info(
                                f"تم إنشاء طلب جديد: رقم الفاتورة {invoice_number}"
                            )
                    else:
                    ...
                    if (
                        inspection_date
                        and self.mapping.auto_create_inspections
                        and order
                    ):
                        ...
                        if inspection_key not in imported_inspections:
                            ...
                            try:
                                ...
                                existing_inspection = Inspection.objects.filter(
                                    order=order, scheduled_date=parsed_date
                                ).first()
                                ...
                                if not existing_inspection:
                                    ...
                                    if inspection:
                                        ...
                                        logger.info(
                                            f"تم إنشاء معاينة جديدة للطلب رقم {invoice_number} بتاريخ {inspection_date}"
                                        )
                            except ValueError:
                                self.stats["warnings"].append(
                                    f"الصف {sheet_row_number}: تنسيق تاريخ المعاينة غير صحيح {inspection_date}"
                                )
                    ...
                    self.stats["errors"].append(
                        f"خطأ في الصف {sheet_row_number}: {str(e)}"
                    )
            ...
            task.save(
                update_fields=["processed_rows", "successful_rows", "failed_rows"]
            )
    ...
    def sync_to_sheets(self, task: GoogleSyncTask = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if not task:
                task = GoogleSyncTask.objects.create(
                    mapping=self.mapping, task_type="reverse_sync", created_by=None
                )
            task.start_task()
        ...
        for col_index, value in enumerate(row_data):
            ...
            if field_type and field_type != "ignore":
                if field_type in mapped_data:
                    logger.warning(
                        f"Duplicate field_type '{field_type}' in row mapping. Previous value will be overwritten."
                    )
    ...
    def _process_customer(
        ...
        try:
            ...
            self.stats["errors"].append(
                f"خطأ في معالجة العميل في الصف {row_index}: {str(e)}"
            )
            raise
    ...
    def _create_customer(self, mapped_data: Dict[str, str]) -> Customer:
        ...
        if hasattr(Customer, "generate_code") and callable(
            getattr(Customer, "generate_code")
        ):
    ...
    def _update_customer(self, customer: Customer, mapped_data: Dict[str, str]):
        ...
        if mapped_data.get("customer_phone2"):
            ...
            if new_phone2 and new_phone2 != current_phone2:
                ...
                updated = True
        if (
            mapped_data.get("customer_email")
            and mapped_data["customer_email"].strip() != (customer.email or "").strip()
        ):
            customer.email = mapped_data["customer_email"].strip()
            updated = True
        if (
            mapped_data.get("customer_address")
            and mapped_data["customer_address"].strip()
    ...
    def _process_order(
        ...
        try:
            ...
            self.stats["errors"].append(
                f"خطأ في معالجة الطلب في الصف {row_index}: {str(e)}"
            )
            raise
    ...
    def _create_order(self, mapped_data: Dict[str, str], customer: Customer) -> Order:
        ...
        if hasattr(Order, "generate_order_number") and callable(
            getattr(Order, "generate_order_number")
        ):
    ...
    def _process_inspection(
        ...
        try:
            inspection_data = {
                ...
                "notes": mapped_data.get("notes", ""),
                "contract_number": mapped_data.get(
                    "contract_number", ""
                ),  # إضافة رقم العقد
            }
            ...
            if hasattr(Inspection, "generate_code") and callable(
                getattr(Inspection, "generate_code")
            ):
    ...
    def _process_installation(
        ...
        try:
            ...
            if customer.branch:
                team = InstallationTeam.objects.filter(
                    branch=customer.branch, is_active=True
                ).first()
                if team:
    ...
    def _get_system_data(self) -> List[Dict[str, Any]]:
        raise NotImplementedError(
            "Reverse sync system data retrieval is not yet implemented."
        )
        ...
        try:
            ...
            for schedule in due_schedules:
                try:
                    ...
                    logger.error(
                        f"خطأ في تشغيل المزامنة المجدولة {schedule.mapping.name}: {str(e)}"
                    )
