...
from .forms import BackupScheduleForm, DatabaseForm, GoogleDriveConfigForm
from .models import BackupSchedule, Database, GoogleDriveConfig, RestoreProgress
...
def database_discover(request):
    ...
    try:
        ...
        existing_dbs = Database.objects.filter(db_type="postgresql").values_list(
            "name", flat=True
        )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            try:  # حفظ قاعدة البيانات من النموذج (بدون إنشاء قاعدة البيانات الفعلية)
                ...
                create_actual_db = (
                    request.POST.get("create_actual_db") == "on"
                )  # حفظ السجل أولاً
                database.save()
                ...
                if create_actual_db and not ignore_db_errors:
                    ...
                    try:
                        ...
                        db_created, create_message = (
                            database_service.create_physical_database(
                        ...
                        if db_created:
                            ...
                            success, test_message = database_service.test_connection(
                            ...
                            if success:
                                ...
                                try:
                                    migrations_applied = _apply_migrations_to_database(
                                        database
                                    )
                                ...
                                if migrations_applied:
                                    ...
                                    try:
                                        default_user_created = _create_default_user(
                                            database
                                        )
                                    except Exception as user_error:
                                        print(
                                            f"خطأ في إنشاء المستخدم الافتراضي: {user_error}"
                                        )
                                        ...
                                        try:
                                            ...
                                            print(
                                                f"فشل في المحاولة الثانية لإنشاء المستخدم الافتراضي: {user_error2}"
                                            )
                                ...
                                request.session["default_user_created"] = (
                                    default_user_created
                                )
                                request.session["migrations_applied"] = (
                                    migrations_applied
                                )
                                ...
                                messages.warning(
                                    request,
                                    f"تم إنشاء قاعدة البيانات في PostgreSQL ولكن فشل اختبار الاتصال: {test_message}",
                                )
                        else:
                            database.status = False
                            ...
                            messages.error(
                                request,
                                f"فشل في إنشاء قاعدة البيانات: {create_message}",
                            )
                        ...
                        database.save()
                        messages.error(
                            request, f"حدث خطأ أثناء إنشاء قاعدة البيانات: {str(e)}"
                        )
                    ...
                    if success:
                        ...
                        messages.success(
                            request,
                            f"تم إنشاء سجل قاعدة البيانات وتم اختبار الاتصال بقاعدة البيانات الموجودة. {message}",
                        )
                    else:
                        database.status = False
                        ...
                        messages.warning(
                            request,
                            f"تم إنشاء سجل قاعدة البيانات ولكن فشل اختبار الاتصال: {message}",
                        )
                else:
                    # تجاهل اختبار الاتصال
                    messages.warning(
                        request,
                        "تم إنشاء سجل قاعدة البيانات دون اختبار الاتصال أو إنشاء قاعدة البيانات.",
                    )
                ...
                messages.error(
                    request, _(f"حدث خطأ أثناء إنشاء قاعدة البيانات: {str(e)}")
                )
        else:
...
def database_activate(request, pk):
    ...
    if request.method != "POST":
        return JsonResponse(
            {"success": False, "message": "طريقة الطلب غير صحيحة. يجب استخدام POST."}
        )
    ...
    try:
        ...
        if activation_result.get("success", False):
            ...
            error_message = activation_result.get(
                "message", f"حدث خطأ أثناء تنشيط قاعدة البيانات {database.name}."
            )
            print(f"فشل التنشيط: {error_message}")

            response = JsonResponse({"success": False, "message": error_message})
            response["Content-Type"] = "application/json; charset=utf-8"
            return response
...
def schedule_delete(request, pk):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            messages.error(
                request, _(f"حدث خطأ أثناء حذف جدولة النسخة الاحتياطية: {str(e)}")
            )
            return redirect("odoo_db_manager:schedule_detail", pk=schedule.pk)
...
def scheduler_status(request):
    ...
    from .services.scheduled_backup_service import (
        get_scheduler,
...
def _restore_json_simple(file_path, clear_existing=False):
    ...
    summary = {"total": 0, "success": 0, "errors": 0, "failed_items": []}
        ...
        if not isinstance(data, list):
            if isinstance(data, dict):
                if (
                    ...
                    raise ValueError(
                        "هذا الملف يبدو وكأنه ملف Google Service Account Credentials وليس نسخة احتياطية للنظام. يرجى رفع ملف نسخة احتياطية صالح بتنسيق Django fixture."
                    )
                elif "model" in data and "fields" in data:
                    data = [data]
                elif "version" in data or "created_at" in data or "database" in data:
                    raise ValueError(
                        "تنسيق النسخة الاحتياطية غير مدعوم. يرجى استخدام ملف بتنسيق Django fixture (JSON)."
                    )
                else:
                    raise ValueError(
                        "تنسيق الملف غير صالح. يجب أن يكون ملف JSON يحتوي على قائمة من البيانات أو بيانات Django fixture."
                    )
            else:
                raise ValueError(
                    f"تنسيق البيانات غير مدعوم: {type(data)}. يجب أن تكون البيانات عبارة عن قائمة أو قاموس."
                )
...
def _restore_json_simple_with_progress(
    file_path, clear_existing=False, progress_callback=None, session_id=None
):
    """
    ...
    def update_progress(
        current_step=None, processed_items=None, success_count=None, error_count=None
    ):
            ...
            if processed_items is not None and hasattr(update_progress, "total_items"):
                ...
                if total_items > 0:
                    progress_percentage = min(
                        100, int((processed_items / total_items) * 100)
                    )
    ...
    try:
            ...
            try:
                with gzip.open(file_path, "rt", encoding="utf-8") as f:
                    ...
                    while True:
                        ...
                        content += chunk
                        update_progress(
                            current_step=f"📖 قراءة الملف... ({len(content):,} حرف)"
                        )
        ...
        if not isinstance(data, list):
            if isinstance(data, dict) and "model" in data and "fields" in data:
                ...
                raise ValueError(
                    "تنسيق الملف غير صحيح. يجب أن يكون ملف Django fixture صالح."
                )

        total_items = len(data)
        update_progress.total_items = total_items  # حفظ العدد الإجمالي
        update_progress(
            current_step=f"📊 تم تحليل {total_items} عنصر", processed_items=0
        )
        ...
        update_progress(
            current_step=f"❌ فشلت العملية: {error_msg}", error_message=error_msg
        )
        raise
                ...
                try:
                    ...
                    try:
                        ...
                        database = Database.objects.get(
                            id=current_progress_data["database_id"]
                        )
                        ...
                        RestoreProgress.objects.create(
                            ...
                            progress_percentage=current_progress_data[
                                "progress_percentage"
                            ],
                        ...
                        print(
                            f"❌ فشل في إعادة إنشاء سجل التقدم: {str(recreate_error)}"
                        )
        ...
        # بدء عملية الاستعادة
        update_progress(
            current_step="🔄 بدء استعادة البيانات الشاملة...",
            processed_items=0,
            success_count=0,
            error_count=0,
        )
        ...
        for idx, item in enumerate(final_data):
            try:
                ...
                try:
                    ...
                    if error_count <= 10:
                        print(
                            f"❌ خطأ في العنصر {idx + 1} ({model_name}): {error_msg[:100]}..."
                        )
        ...
        if failed_items:
            ...
            for i, error in enumerate(failed_items[:10], 1):
                print(
                    f"  {i}. العنصر {error['index']} ({error['model']} - PK: {error['pk']})"
                )
    ...
    if request.method == "POST":
        try:
            ...
            if not drive_service:
                return JsonResponse(
                    {"success": False, "message": "فشل في تهيئة خدمة Google Drive"}
                )
...
def google_drive_create_test_folder(request):
    ...
    if request.method == "POST":
        try:
            ...
            if result:
                messages.success(
                    request, f'تم إنشاء مجلد تجريبي بنجاح: {result["name"]}'
                )
                return JsonResponse(
                    {
                        "success": True,
                ...
                return JsonResponse(
                    {"success": False, "message": "فشل في إنشاء المجلد التجريبي"}
                )
    ...
    if request.method == "POST":
        try:
            ...
            if result and result.get("success"):
                ...
                error_message = (
                    result.get("message") if result else "فشل في اختبار رفع الملف"
                )
                return JsonResponse({"success": False, "message": error_message})
    ...
    if request.method == "POST":
        try:
            from orders.services.google_drive_service import (
                test_contract_file_upload_to_folder,
            )
            ...
            if result and result.get("success"):
                ...
                error_message = (
                    result.get("message") if result else "فشل في اختبار رفع ملف العقد"
                )
                return JsonResponse({"success": False, "message": error_message})
...
def database_register(request):
    ...
    if request.method == "POST":
        try:
            ...
            if not db_name:
                return JsonResponse(
                    {"success": False, "message": "اسم قاعدة البيانات مطلوب"}
                )

            # التحقق من عدم وجود قاعدة البيانات مسبقاً
            if Database.objects.filter(name=db_name).exists():
                return JsonResponse(
                    {"success": False, "message": "قاعدة البيانات مسجلة بالفعل"}
                )
...
def _create_default_user(database):
    ...
    try:
        ...
        cursor.execute(
            "SELECT COUNT(*) FROM accounts_user WHERE username = %s", ("admin",)
        )
...
def _apply_migrations_to_database(database):
    try:
        ...
        migrate_cmd = ["python", "manage.py", "migrate", "--fake-initial"]
        ...
        if result.returncode == 0:
            ...
            if (
                "django_apscheduler" in error_msg
                and "column" in error_msg
...
def restore_progress_status(request, session_id):
    ...
    try:
        ...
        print(
            f"✅ [DEBUG] Progress status: {progress.status} - {progress.progress_percentage}%"
        )
...
def generate_temp_token(request):
    ...
    try:
        ...
        print(
            f"🔍 [DEBUG] User is_superuser: {getattr(request.user, 'is_superuser', False)}"
        )
        ...
        if not (request.user.is_staff or request.user.is_superuser):
            ...
            return JsonResponse(
                {"error": "ليس لديك صلاحية للقيام بهذا الإجراء"}, status=403
            )
...
def refresh_session(request):
    ...
    try:
        ...
        print(
            f"🔍 [DEBUG] temp_token received: {temp_token[:10] if temp_token else 'None'}..."
        )

        if not temp_token:
            return JsonResponse(
                {"success": False, "message": "الرمز المؤقت مفقود"}, status=400
            )
        ...
        cache.set(
            f"progress_api_token_{api_token}", user_id, timeout=300
        )  # صالح لمدة 5 دقائق
