...
def diagnose_mapping_status(mapping):
    ...
    diagnosis = {"is_valid": True, "issues": [], "warnings": [], "suggestions": []}
    ...
    if not column_mappings:
        ...
        if missing_fields:
            diagnosis["warnings"].append(
                f'حقول أساسية مفقودة: {", ".join(missing_fields)}'
            )
...
def advanced_sync_dashboard(request):
    ...
    try:
        ...
        # آخر المهام
        recent_tasks = GoogleSyncTask.objects.select_related("mapping").order_by(
            "-created_at"
        )[:10]

        # التعيينات النشطة
        active_mappings_list = GoogleSheetMapping.objects.filter(
            is_active=True
        ).order_by("-last_sync")[:5]

        # المزامنة المجدولة
        scheduled_syncs = GoogleSyncSchedule.objects.filter(
            is_active=True
...
def mapping_list(request):
    ...
    try:
        ...
        return render(
            request, "odoo_db_manager/advanced_sync/mapping_list.html", context
        )
...
def mapping_create(request):
    ...
    if request.method == "POST":
        try:
            ...
            if existing_mapping:
                ...
                return redirect(
                    "odoo_db_manager:mapping_detail", mapping_id=existing_mapping.id
                )

            with transaction.atomic():
                # حفظ القيم الافتراضية
                default_customer_category_id = request.POST.get(
    ...
    try:
        ...
        available_sheets = []
        messages.warning(
            request, "لا يمكن جلب قائمة الجداول. تأكد من إعدادات Google Sheets."
        )
    ...
    context = {
        ...
        "field_types_json": json.dumps(
            GoogleSheetMapping.FIELD_TYPES, ensure_ascii=False
        ),
...
def mapping_detail(request, mapping_id):
    ...
    try:
        ...
        try:
            ...
            headers = (
                sheet_data[mapping.header_row - 1]
        ...
        # آخر المهام
        recent_tasks = GoogleSyncTask.objects.filter(mapping=mapping).order_by(
            "-created_at"
        )[:10]

        # التعارضات المعلقة
        pending_conflicts = GoogleSyncConflict.objects.filter(
            task__mapping=mapping, resolution_status="pending"
        ...
        return render(
            request, "odoo_db_manager/advanced_sync/mapping_detail.html", context
        )
...
def mapping_edit(request, mapping_id):
    ...
    try:
        ...
        if request.method == "POST":
            ...
            mapping.spreadsheet_id = request.POST.get(
            ...
            mapping.auto_create_customers = (
                request.POST.get("auto_create_customers") == "on"
            )
            mapping.auto_create_orders = request.POST.get("auto_create_orders") == "on"
            mapping.auto_create_inspections = (
                request.POST.get("auto_create_inspections") == "on"
            )
            # Removed auto_create_installations assignment
            mapping.update_existing = request.POST.get("update_existing") == "on"
            mapping.enable_reverse_sync = (
                request.POST.get("enable_reverse_sync") == "on"
            )

            # حل التعارضات
            mapping.conflict_resolution = request.POST.get(
            ...
            if column_mappings:
                ...
                logger.info(
                    f"تم تحديث تعيينات الأعمدة للتعيين {mapping.name}: {len(column_mappings)} تعيين"
                )
            ...
            if default_customer_category_id:
                try:
                    ...
                    mapping.default_customer_category = CustomerCategory.objects.get(
                        id=default_customer_category_id
                    )
            ...
            mapping.default_customer_type = (
                request.POST.get("default_customer_type") or None
            )
            ...
            mapping.use_current_date_as_created = (
                request.POST.get("use_current_date_as_created") == "on"
            )
        ...
        return render(
            request, "odoo_db_manager/advanced_sync/mapping_edit.html", context
        )
...
def api_run_sync(request, mapping_id):
    ...
    try:
        ...
        task = GoogleSyncTask.objects.create(
            mapping=mapping, status="pending", created_by=request.user
        )
                ...
                return JsonResponse(
                    {
                        ...
                        "message": (
                            "تم بدء المزامنة بنجاح"
                            if result.get("success")
                            else result.get("error", "فشل")
                        ),
            ...
            try:
                ...
                try:
                    ...
                    task.result = json.dumps(
                        result.get("stats", {}), ensure_ascii=False
                    )
                    task.save()
                    mapping.last_sync = timezone.now()
                    mapping.save(update_fields=["last_sync"])
...
def get_sheets_by_id(request):
    ...
    try:
        ...
        if not spreadsheet_id:
            return JsonResponse({"success": False, "error": "معرف الجدول مطلوب"})
        ...
        return JsonResponse(
            {"success": False, "error": f"خطأ في جلب صفحات الجدول: {str(e)}"}
        )
...
def mapping_update_columns(request, mapping_id):
    ...
    try:
        ...
        if request.method == "GET":
            ...
            return render(
                request,
                "odoo_db_manager/advanced_sync/mapping_update_columns.html",
                context,
            )
...
def start_sync(request, mapping_id):
    ...
    logger.info(
        f"=== بدء المزامنة للتعيين {mapping_id} بواسطة المستخدم {request.user.username} ==="
    )
    ...
    try:
        ...
        logger.info(
            f"عدد تعيينات الأعمدة: {len(mapping.column_mappings) if mapping.column_mappings else 0}"
        )
        ...
        # إنشاء مهمة المزامنة
        task = GoogleSyncTask.objects.create(
            mapping=mapping, task_type="import", created_by=request.user
        )
...
def task_detail(request, task_id):
    ...
    try:
        ...
        return render(
            request, "odoo_db_manager/advanced_sync/task_detail.html", context
        )
...
def conflict_list(request):
    """قائمة التعارضات"""
    try:
        conflicts = GoogleSyncConflict.objects.select_related(
            "task", "task__mapping"
        ).order_by("-created_at")
        ...
        return render(
            request, "odoo_db_manager/advanced_sync/conflict_list.html", context
        )
...
def schedule_sync(request, mapping_id):
    ...
    try:
        ...
        return render(
            request, "odoo_db_manager/advanced_sync/schedule_sync.html", context
        )
...
def get_sheet_columns(request):
    ...
    try:
        ...
        if not spreadsheet_id or not sheet_name:
            return JsonResponse(
                {"success": False, "error": "معرف الجدول واسم الصفحة مطلوبان"}
            )
        ...
        try:
            ...
            headers = (
                ...
                if sheet_data and len(sheet_data) >= header_row
                else []
            )
            ...
            return JsonResponse({"success": True, "headers": translated_headers})
        finally:
            # استعادة المعرف الأصلي
            importer.config.spreadsheet_id = original_id
    ...
    try:
        ...
        try:
            ...
            if not sheet_data or len(sheet_data) < start_row:
                return JsonResponse(
                    {"success": False, "error": "لا توجد بيانات كافية في الجدول"}
                )

            headers = (
                sheet_data[header_row - 1] if len(sheet_data) >= header_row else []
            )
