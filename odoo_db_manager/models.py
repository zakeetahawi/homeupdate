...
class ImportLog(models.Model):
    ...
    def __str__(self):
        return (
            f"استيراد {self.sheet_name} - {self.created_at.strftime('%Y-%m-%d %H:%M')}"
        )
...
class Database(models.Model):
    ...
    def update_env_file(self):
        ...
        try:
            ...
            try:
                with open(env_file, "r", encoding="utf-8") as src, open(
            ...
            new_lines.append(
                f"# تم تحديث هذا الملف تلقائياً بواسطة نظام إدارة قواعد البيانات في {current_time}\n"
            )
            new_lines.append(
                f"# قاعدة البيانات النشطة: {self.name} ({connection_info.get('NAME')})\n\n"
            )
            for line in lines:
            ...
            for line in lines:
                if (
                    ...
                    and not line.startswith("DB_NAME=")
                    and not line.startswith("DB_USER=")
                    and not line.startswith("DB_PASSWORD=")
                    and not line.startswith("DB_HOST=")
                    and not line.startswith("DB_PORT=")
                    and not line.startswith("PGPASSWORD=")
                    and line.strip()
                    and "=" in line
            ...
            if env_updated and settings_updated:
                ...
                try:
                    ...
                    if current_db == connection_info.get("NAME"):
                        ...
                        print(
                            f"⚠️ لم يتم التبديل بنجاح. قاعدة البيانات الحالية: {current_db}, المطلوبة: {connection_info.get('NAME')}"
                        )
                        return {
                            "success": True,
...
class BackupSchedule(models.Model):
    ...
    frequency = models.CharField(
        _("التكرار"), max_length=20, choices=FREQUENCY_CHOICES, default="daily"
    )
    # وقت التنفيذ
    ...
    day_of_week = models.IntegerField(
        _("يوم الأسبوع"), choices=DAYS_OF_WEEK, default=0, null=True, blank=True
    )
    # يوم الشهر (للتكرار الشهري)
    day_of_month = models.IntegerField(
        _("يوم الشهر"), default=1, help_text=_("1-31"), null=True, blank=True
    )
    # الحد الأقصى لعدد النسخ الاحتياطية
    max_backups = models.IntegerField(
        _("الحد الأقصى لعدد النسخ"), default=24, help_text=_("الحد الأقصى هو 24 نسخة")
    )
...
class GoogleDriveConfig(models.Model):
    ...
    name = models.CharField(
        _("اسم الإعداد"), max_length=100, default="إعدادات Google Drive"
    )
    # إعدادات المجلد
    ...
    def save(self, *args, **kwargs):
        ...
        if self.is_active:
            GoogleDriveConfig.objects.filter(is_active=True).exclude(pk=self.pk).update(
                is_active=False
            )
...
class RestoreProgress(models.Model):
    ...
    status = models.CharField(
        max_length=20, choices=STATUS_CHOICES, default="starting", verbose_name="الحالة"
    )
    ...
    current_step = models.CharField(
        max_length=255, blank=True, verbose_name="الخطوة الحالية"
    )
