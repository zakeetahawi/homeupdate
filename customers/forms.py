    ...
    def clean_phone2(self):
        ...
        if phone2:
            ...
            if qs.exists():
                ...
                raise ValidationError(
                    _(
                        "رقم الهاتف الثاني مستخدم بالفعل للعميل: %(name)s (الفرع: %(branch)s)"
                    ),
...
class CustomerSearchForm(forms.Form):
    search = forms.CharField(
        required=False,
        widget=forms.TextInput(
            attrs={"class": "form-control", "placeholder": _("بحث عن عميل...")}
        ),
    )
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # Update customer type choices dynamically
        self.fields["customer_type"].choices = [("", _("كل الأنواع"))] + list(
            Customer.get_customer_types()
        )
    ...
    class Meta:
        ...
        widgets = {
            ...
            "position": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "المنصب (اختياري)"}
            ),
            "phone": forms.TextInput(
                attrs={
                    "class": "form-control",
