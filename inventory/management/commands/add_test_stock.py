...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if not products.exists():
            self.stdout.write(
                self.style.ERROR("No products found. Run create_test_products first.")
            )
            return
        ...
        for product in products:
            ...
            if current_stock < 10:
                ...
                self.stdout.write(
                    f"Added {quantity_to_add} units to {product.name}. New stock: {product.current_stock}"
                )
            else:
                self.stdout.write(
                    f"Product {product.name} already has sufficient stock: {current_stock}"
                )
