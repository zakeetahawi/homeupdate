    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            ...
            if not default_category and create_category:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Created default category: {default_category.name}"
                    )
                )
                ...
                if not default_category:
                    self.stdout.write(
                        self.style.ERROR(
                            "No categories found and create-category not specified"
                        )
                    )
                    return
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Using existing category: {default_category.name}"
                    )
                )
            else:
                self.stdout.write(
            ...
            if count == 0:
                self.stdout.write(
                    self.style.SUCCESS("No products with missing category found")
                )
                return
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Successfully updated {count} products with default category: {default_category.name}"
                )
            )
