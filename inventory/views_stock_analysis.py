...
from .models import (
    Product, Warehouse, StockTransaction, StockTransfer,
    StockTransferItem
)
...
def product_stock_movement(request, product_id):
    ...
    ).select_related('warehouse', 'created_by').order_by('-transaction_date')
    ...
    stats = {
        ...
        'total_adjustment': transactions.filter(transaction_type='adjustment').aggregate(
            total=Sum('quantity')
        )['total'] or 0,
    }
...
def warehouse_stock_analysis(request, warehouse_id):
    ...
    transactions = StockTransaction.objects.filter(
        warehouse=warehouse,
        transaction_date__gte=start_date
    ).select_related('product', 'created_by').order_by('-transaction_date')
    ...
    ).select_related('from_warehouse').prefetch_related('items__product')
    
    transfers_out = StockTransfer.objects.filter(
        from_warehouse=warehouse,
        created_at__gte=start_date
    ).select_related('to_warehouse').prefetch_related('items__product')
...
def stock_movement_summary(request):
    ...
    total_transfers = StockTransfer.objects.filter(
        created_at__gte=start_date
    ).count()
    
    # حركات المخزون حسب النوع
    transaction_by_type = StockTransaction.objects.filter(
        transaction_date__gte=start_date
    ...
    transfers_by_status = StockTransfer.objects.filter(
        created_at__gte=start_date
    ...
    top_warehouses = StockTransaction.objects.filter(
        transaction_date__gte=start_date
...
def stock_discrepancy_report(request):
    ...
    for product in Product.objects.all():
        ...
        total_out = StockTransaction.objects.filter(
            product=product,
            transaction_type__in=['out', 'transfer']
        ).aggregate(total=Sum('quantity'))['total'] or 0
