        ...
        with transaction.atomic():
            ...
            previous_balance = (
                StockTransaction.objects.filter(
                    product=self.product, transaction_date__lt=self.transaction_date
                )
            ...
            current_balance = (
                previous_balance.running_balance if previous_balance else 0
            )
            ...
            next_transactions = (
                StockTransaction.objects.filter(
                    product=self.product, transaction_date__gt=self.transaction_date
                )
                .order_by("transaction_date")
                .select_for_update()
            )
