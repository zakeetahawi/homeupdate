...
from .models import (
    Category, Supplier, Warehouse, WarehouseLocation, 
    PurchaseOrder, PurchaseOrderItem, InventoryAdjustment,
    ProductBatch, StockAlert, Product
)
...
class CategoryForm(forms.ModelForm):
    ...
    class Meta:
        ...
        help_texts = {
            'parent': _('اختياري - يمكن تصنيف هذه الفئة ضمن فئة أكبر')
        }
...
class WarehouseForm(forms.ModelForm):
    ...
    class Meta:
        model = Warehouse
        fields = ['name', 'code', 'branch', 'manager', 'address', 'is_active', 
                  'is_official_fabric_warehouse', 'notes']
        widgets = {
            'name': forms.TextInput(attrs={
                'class': 'form-control',
            ...
            'is_official_fabric_warehouse': forms.CheckboxInput(attrs={
                'class': 'form-check-input'
            }),
            'notes': forms.Textarea(attrs={
                'class': 'form-control',
...
class InventoryAdjustmentForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'quantity_after': forms.NumberInput(attrs={
                'class': 'form-control',
                'min': '0',
                'step': '0.01',
                'required': True
    ...
    def clean(self):
        ...
        if product and warehouse:
            ...
            quantity_before = last_transaction.running_balance if last_transaction else 0
...
class StockAlertForm(forms.ModelForm):
    ...
    class Meta:
        model = StockAlert
        fields = ['product', 'alert_type', 'message', 'description', 'priority', 
                  'is_urgent', 'is_pinned']
        widgets = {
            'product': forms.Select(attrs={
                'class': 'form-select',
