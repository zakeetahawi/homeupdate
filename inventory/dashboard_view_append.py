...
class InventoryDashboardView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["low_stock_count"] = sum(
            ...
            if p.current_stock <= p.minimum_stock and p.current_stock > 0
        )
        context["purchase_orders_count"] = PurchaseOrder.objects.filter(
            status="ordered"
        ).count()
        context["inventory_value"] = sum(
            p.current_stock * p.price for p in Product.objects.all()
        )
...
def optimized_product_detail(request, product_id):
    ...
    transactions = (
        ...
        .select_related("created_by", "reason")
        .order_by("-date")[:20]
    )
    ...
    products = (
        ...
        .annotate(
            stock_in=Sum(
                Case(
                    When(
                        stock_transactions__transaction_type="in",
                        then="stock_transactions__quantity",
                    ),
                    default=0,
            ...
            stock_out=Sum(
                Case(
                    When(
                        stock_transactions__transaction_type="out",
                        then="stock_transactions__quantity",
                    ),
                    default=0,
                    output_field=IntegerField(),
...
def stock_movement_report(request):
    ...
    transactions = (
        ...
        .select_related("product", "reason", "created_by")
        .order_by("-date")
    )

    # تحليل البيانات للحصول على ملخصات
    product_movements = (
        transactions.values("product__name")
        .annotate(
            stock_in=Sum(
                Case(
