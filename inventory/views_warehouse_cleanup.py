...
def warehouse_cleanup_view(request):
    ...
    if request.method == 'POST':
        ...
        if action == 'delete_all_empty':
            ...
            if result.get('deleted', 0) > 0:
                messages.success(
                    request,
                    _(f"✅ تم حذف {result['deleted']} مستودع فارغ: {', '.join(result['warehouses'])}")
                )
            else:
            ...
            if deleted_names:
                messages.success(
                    request,
                    _(f"✅ تم حذف {len(deleted_names)} مستودع: {', '.join(deleted_names)}")
                )
    ...
    for warehouse in Warehouse.objects.all():
        ...
        if total_stock == 0:
            can_delete = (
                active_cutting == 0 and 
                not warehouse.is_official_fabric_warehouse
            )
