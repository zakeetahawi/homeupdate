...
def dashboard_stats_api(request):
    ...
    try:
        ...
        user_notifications = (
            Notification.objects.filter(visible_to=request.user, is_read=False)
            .filter(
...
def product_stock_info_api(request, product_id):
    ...
    try:
        ...
        if current_stock <= 0:
            ...
            and current_stock > product.maximum_stock
        ):
            status = "overstock"
                ...
                "transactions": [
                    {
                        ...
                        "created_by": (
                            trans.created_by.get_full_name()
                            if trans.created_by
                            else "System"
                        ),
                        "notes": trans.notes,
...
def resolve_alert_api(request, alert_id):
    ...
    try:
        ...
        if alert.status != "active":
            return JsonResponse({"success": False, "message": "هذا التنبيه ليس نشطاً"})
            ...
            categories_data.append(
                {
                    ...
                    "health_status": (
                        "critical"
                        if out_of_stock_count > category.product_count * 0.3
                        else (
                            "warning"
                            if low_stock_count > category.product_count * 0.5
                            else "good"
                        )
                    ),
                }
...
def get_stock_notifications_count(request):
    ...
    try:
        ...
        stock_alerts = StockAlert.objects.filter(status="active")
...
def inventory_value_report_api(request):
    ...
    try:
            ...
            for product in products:
                try:
                    ...
                    if stock_level > 0 and product.price:
                        product_value = Decimal(str(product.price)) * Decimal(
                            str(stock_level)
                        )
                ...
                warehouse_breakdown.append(
                    {
                        ...
                        "name": warehouse.name,
                        "branch": (
                            warehouse.branch.name if warehouse.branch else "بدون فرع"
                        ),
                        "total_value": float(warehouse_value),
                        "total_stock": warehouse_stock,
                        "percentage": (
                            round((warehouse_value / total_inventory_value) * 100, 2)
                            if total_inventory_value > 0
                            else 0
                        ),
                    }
        ...
        products_with_stock = sum(
            1
            for p in Product.objects.all()[:100]  # محدود للأداء
        ...
        low_value_products = (
            products_with_values[-10:] if len(products_with_values) > 10 else []
        )
        ...
        return JsonResponse(
            {
                ...
                "category_breakdown": sorted(
                    category_breakdown, key=lambda x: x["total_value"], reverse=True
                ),
                "warehouse_breakdown": sorted(
                    warehouse_breakdown, key=lambda x: x["total_value"], reverse=True
                ),
    ...
    try:
        ...
        for product in products:
            try:
                ...
                avg_stock = (
                    (current_stock + total_in - total_out) / 2
                    if total_in > 0
                    else current_stock
                )
                ...
                if total_out > 0:
                    days_to_sell = (
                        (current_stock / (total_out / days)) if total_out > 0 else 999
                    )
                else:
                    days_to_sell = 999
                ...
                turnover_data.append(
                    {
                        ...
                        "days_to_sell": (
                            round(days_to_sell, 1) if days_to_sell < 999 else "N/A"
                        ),
                        "status": status,
                        "status_color": status_color,
                    }
        ...
        return JsonResponse(
            {
                ...
                "fast_moving_products": [
                    d for d in turnover_data if d["turnover_rate"] > 2
                ][:10],
                "stagnant_products": [
                    d for d in turnover_data if d["turnover_rate"] <= 0.5
                ][:10],
                "last_updated": timezone.now().isoformat(),
            }
        )
            ...
            transfers_data.append(
                {
                    ...
                    "expected_arrival_date": (
                        transfer.expected_arrival_date.isoformat()
...
def reorder_recommendations_api(request):
    ...
    try:
                    ...
                    recommendations.append(
                        {
                            ...
                            "estimated_cost": (
                                float(product.price * suggested_quantity)
                                if product.price
                                else 0
                            ),
                        }
        ...
        recommendations.sort(
            key=lambda x: (x["priority_level"], -x["current_stock"]), reverse=True
        )
        ...
        return JsonResponse(
            {
                ...
                "summary": {
                    ...
                    "urgent": len(
                        [r for r in recommendations if r["priority"] == "عاجل"]
                    ),
                    "high_priority": len(
                        [r for r in recommendations if r["priority"] == "عالي"]
                    ),
                    "medium_priority": len(
                        [r for r in recommendations if r["priority"] == "متوسط"]
                    ),
                    "total_estimated_cost": round(total_estimated_cost, 2),
                },
                "recommendations": recommendations,
                "urgent_items": [r for r in recommendations if r["priority"] == "عاجل"][
                    :20
                ],
                "last_updated": timezone.now().isoformat(),
            }
        ...
        return JsonResponse(
            {
                ...
                "is_completed": upload_log.status
                ...
                "completed_at": (
                    upload_log.completed_at.isoformat()
                    if upload_log.completed_at
                    else None
                ),
...
def manage_warehouse_api(request):
    ...
    try:
        ...
        if not warehouse_ids:
            return JsonResponse(
                {"success": False, "message": "لم يتم تحديد مستودعات"}, status=400
            )
        ...
        if action == "mark_as_fabric":
            ...
            if hasattr(Warehouse, "is_official_fabric_warehouse"):
                ...
                return JsonResponse(
                    {"success": False, "message": "الحقل غير متوفر في النموذج"},
                    status=400,
                )
            ...
            return JsonResponse(
                {"success": True, "message": f"تم تعطيل {count} مستودع", "count": count}
            )

        else:
            return JsonResponse(
                {"success": False, "message": "إجراء غير معروف"}, status=400
            )
