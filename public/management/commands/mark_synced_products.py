...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        unsynced = BaseProduct.objects.filter(is_active=True, cloudflare_synced=False)
        ...
        if total == 0:
            self.stdout.write(
                self.style.SUCCESS(
                    "✅ All active products are already marked as synced!"
                )
            )
            return
        ...
        now = timezone.now()
        updated = unsynced.update(cloudflare_synced=True, last_synced_at=now)
