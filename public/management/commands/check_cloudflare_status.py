...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            f'  API Key: {"✓ Set" if settings.CLOUDFLARE_SYNC_API_KEY else "✗ Not Set"}'
        )
        ...
        try:
            response = requests.get(
                f"{settings.CLOUDFLARE_WORKER_URL}/health", timeout=10
            )
            if response.status_code == 200:
                self.stdout.write(
                    self.style.SUCCESS("  ✓ Worker is online and responding")
                )
                if verbose:
                    data = response.json()
        ...
        self.stdout.write(
            f"  Synced: {synced} ({synced/total*100:.1f}%)"
            if total > 0
            else "  Synced: 0"
        )
        ...
        if synced > 0:
            recent = (
                BaseProduct.objects.filter(
                    cloudflare_synced=True, last_synced_at__isnull=False
                )
            ...
            if recent:
                ...
                self.stdout.write(
                    f"  Last Sync: {recent.last_synced_at} ({hours:.1f} hours ago)"
                )
        ...
        if not_synced > 0:
            ...
            self.stdout.write(
                "  • Use: python manage.py sync_to_cloudflare --batch-size=10"
            )

        if not settings.CLOUDFLARE_SYNC_ENABLED:
            self.stdout.write(
                self.style.ERROR("  • Sync is DISABLED - enable in settings")
            )
