...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if settings and not settings.is_enabled:
            self.stdout.write(
                self.style.WARNING(
                    "⚠️  المزامنة معطلة في الإعدادات. استخدم --force لتجاوز ذلك."
                )
            )
        ...
        if not sync.is_configured():
            self.stdout.write(self.style.ERROR("❌ إعدادات Cloudflare غير مكتملة!"))
        ...
        try:
            ...
            for i in range(0, total_base, batch_size):
                ...
                if sync._send_request(data):
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"  ✅ تمت مزامنة {progress}/{total_base} منتج أساسي"
                        ),
                        ending="\n",
                    )
                    self.stdout.flush()
            ...
            if total_orphans > 0:
                ...
                for i in range(0, total_orphans, batch_size):
                    ...
                    if sync._send_request(data):
                        ...
                        self.stdout.write(
                            self.style.SUCCESS(
                                f"  ✅ تمت مزامنة {progress}/{total_orphans} منتج مستقل"
                            ),
                            ending="\n",
                        )
                        self.stdout.flush()
                        ...
                        self.stdout.write(
                            self.style.ERROR(
                                f"  ❌ فشلت مزامنة الدفعة {i//batch_size + 1}"
                            )
                        )
