...
class CloudflareSettings(models.Model):
    ...
    last_full_sync = models.DateTimeField(_("آخر مزامنة كاملة"), null=True, blank=True)

    products_synced = models.PositiveIntegerField(_("عدد المنتجات المُزامَنة"), default=0)
...
class CloudflareSettingsAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (
            _("إعدادات الاتصال"),
            {
                ...
                "description": _(
                    "أدخل رابط Worker ونسخ مفتاح API لإعداده في Cloudflare"
                ),
            },
        ),
        (
    ...
    def sync_all_products(self, request, queryset):
        ...
        if not settings_obj.is_enabled:
            self.message_user(
                request, "المزامنة معطلة. قم بتفعيلها أولاً.", messages.WARNING
            )
            return
        ...
        try:
            ...
            settings_obj.save()
            self.message_user(
                request, f"تم مزامنة {count} منتج بنجاح ✅", messages.SUCCESS
            )
...
class QRDesignSettings(models.Model):
    ...
    logo_text_en = models.CharField(
        "نص الشعار (إنجليزي)", max_length=100, default="Elkhawaga", blank=True
    )

    show_logo = models.BooleanField("إظهار الشعار", default=True)

    logo_size = models.IntegerField(
        "حجم الشعار", default=200, help_text="الحجم الأقصى للشعار بالبكسل (50-400)"
    )

    # ====== الألوان / Colors ======
    color_primary = ColorField(
        "اللون الأساسي", default="#d4af37", help_text="اللون الذهبي الرئيسي"
    )

    color_secondary = ColorField(
        "اللون الثانوي", default="#b8860b", help_text="لون ذهبي داكن"
    )

    color_background = ColorField(
        "لون الخلفية", default="#1a1a2e", help_text="لون خلفية الصفحة"
    )

    color_surface = ColorField(
        "لون السطح", default="#16213e", help_text="لون بطاقة المحتوى"
    )

    color_text = ColorField("لون النص", default="#ffffff", help_text="لون النص الرئيسي")

    color_text_secondary = ColorField(
        "لون النص الثانوي", default="#c0c0c0", help_text="لون النص الفرعي"
    )

    # ====== ألوان إضافية / Additional Colors ======
    color_card = ColorField(
        "لون البطاقة", default="#16213e", help_text="لون خلفية البطاقة الرئيسية"
    )

    color_button = ColorField(
        "لون الأزرار", default="#d4af37", help_text="لون خلفية الأزرار"
    )

    color_button_text = ColorField(
        "لون نص الأزرار", default="#1a1a2e", help_text="لون النص داخل الأزرار"
    )
    ...
    color_badge_text = ColorField(
        "لون نص البادجات", default="#1a1a2e", help_text="لون النص داخل البادجات"
    )
    ...
    color_product_name = ColorField(
        "لون اسم المنتج", default="#d4af37", help_text="لون اسم المنتج الرئيسي"
    )
    ...
    show_website_button = models.BooleanField("إظهار زر الموقع", default=True)

    # ====== التواصل الاجتماعي / Social Media ======
    facebook_url = models.URLField(
        "فيسبوك", blank=True, default="", help_text="رابط صفحة فيسبوك"
    )

    instagram_url = models.URLField(
        "إنستجرام", blank=True, default="", help_text="رابط حساب إنستجرام"
    )
    ...
    twitter_url = models.URLField(
        "تويتر / X", blank=True, default="", help_text="رابط حساب تويتر"
    )

    youtube_url = models.URLField(
        "يوتيوب", blank=True, default="", help_text="رابط قناة يوتيوب"
    )

    tiktok_url = models.URLField(
        "تيك توك", blank=True, default="", help_text="رابط حساب تيك توك"
    )
    ...
    email = models.EmailField(
        "البريد الإلكتروني", blank=True, default="", help_text="بريد إلكتروني للتواصل"
    )
    ...
    complaint_button_text_en = models.CharField(
        "نص زر الشكوى (إنجليزي)", max_length=50, default="Create Complaint", blank=True
    )

    show_complaint_button = models.BooleanField("إظهار زر الشكوى", default=True)
    ...
    card_border_radius = models.IntegerField(
        "انحناء زوايا البطاقة", default=15, help_text="بالبكسل (0-50)"
    )

    enable_animations = models.BooleanField("تفعيل التأثيرات الحركية", default=True)
    ...
    font_size_base = models.IntegerField(
        "حجم الخط الأساسي", default=16, help_text="بالبكسل (12-24)"
    )
    ...
    font_weight_heading = models.CharField(
        "وزن خط العناوين", max_length=20, default="700", choices=FONT_WEIGHT_CHOICES
    )

    # ====== المسافات والأبعاد / Spacing & Sizing ======
    card_padding = models.IntegerField(
        "مسافة داخلية للبطاقة", default=30, help_text="بالبكسل (20-50)"
    )

    card_max_width = models.IntegerField(
        "أقصى عرض للبطاقة", default=450, help_text="بالبكسل (400-600)"
    )

    element_spacing = models.IntegerField(
        "المسافة بين العناصر", default=20, help_text="بالبكسل (10-40)"
    )
    ...
    card_shadow_intensity = models.CharField(
        "قوة ظل البطاقة", max_length=20, default="medium", choices=SHADOW_CHOICES
    )
    ...
    enable_hover_effects = models.BooleanField(
        "تفعيل تأثيرات التمرير", default=True, help_text="حركات عند التمرير على الأزرار"
    )
    ...
    button_style = models.CharField(
        "شكل الأزرار", max_length=20, default="rounded", choices=BUTTON_STYLE_CHOICES
    )
    ...
    button_size = models.CharField(
        "حجم الأزرار", max_length=20, default="medium", choices=BUTTON_SIZE_CHOICES
    )

    # ====== عرض السعر / Price Display ======
    price_font_size = models.IntegerField(
        "حجم خط السعر", default=48, help_text="بالبكسل (32-72)"
    )

    show_price_badge = models.BooleanField(
        "إظهار شارة السعر", default=True, help_text="خلفية مميزة لقسم السعر"
    )

    # ====== تحسينات بطاقة المنتج / Product Card ======
    show_product_icon = models.BooleanField("إظهار أيقونة المنتج", default=True)

    show_category_badge = models.BooleanField("إظهار شارة التصنيف", default=True)

    # ====== إعدادات متقدمة / Advanced ======
    custom_css = models.TextField(
        "CSS مخصص", blank=True, default="", help_text="كود CSS إضافي للتخصيص المتقدم"
    )

    custom_js = models.TextField(
        "JavaScript مخصص", blank=True, default="", help_text="كود JavaScript إضافي"
    )
    ...
    # ====== المزامنة / Sync ======
    last_synced_at = models.DateTimeField("آخر مزامنة", auto_now=True)

    cloudflare_synced = models.BooleanField("تمت المزامنة مع Cloudflare", default=False)
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.pk and QRDesignSettings.objects.exists():
            raise ValueError(
                "يمكن إنشاء إعدادات واحدة فقط. قم بتعديل الإعدادات الموجودة."
            )
        super().save(*args, **kwargs)
