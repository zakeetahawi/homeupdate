...
def register_devices_from_csv(csv_file_path):
    ...
    with open(csv_file_path, 'r', encoding='utf-8-sig') as csvfile:
        ...
        for row_num, row in enumerate(reader, start=1):
            ...
            temp_fingerprint = f"TEMP_{branch.code}_{device_name}_{row_num}".replace(" ", "_")
            
            # التحقق من عدم وجود جهاز بنفس الاسم في نفس الفرع
            if BranchDevice.objects.filter(branch=branch, device_name=device_name).exists():
                print(f"⚠️  سطر {row_num}: الجهاز '{device_name}' موجود بالفعل في فرع '{branch_name}'")
                devices_failed += 1
                continue
            ...
            try:
                ...
                print(f"✅ سطر {row_num}: تم إنشاء '{device_name}' للفرع '{branch_name}'")
