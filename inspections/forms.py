...
class InspectionEvaluationForm(forms.Form):
    ...
    notes = forms.CharField(
        label=_("ملاحظات التقييم"),
        required=False,
        widget=forms.Textarea(attrs={"rows": 4, "class": "form-control"}),
    )
    # إضافة حقل تقييم لكل معيار
    location = forms.ChoiceField(
        label=_("الموقع"), choices=RATING_CHOICES, widget=forms.RadioSelect
    )
    condition = forms.ChoiceField(
        label=_("الحالة"), choices=RATING_CHOICES, widget=forms.RadioSelect
    )
    suitability = forms.ChoiceField(
        label=_("الملاءمة"), choices=RATING_CHOICES, widget=forms.RadioSelect
    )
    safety = forms.ChoiceField(
        label=_("السلامة"), choices=RATING_CHOICES, widget=forms.RadioSelect
    )
    accessibility = forms.ChoiceField(
        label=_("سهولة الوصول"), choices=RATING_CHOICES, widget=forms.RadioSelect
    )
...
class InspectionFilterForm(forms.Form):
    search = forms.CharField(
        ...
        widget=forms.TextInput(
            attrs={"class": "form-control", "placeholder": _("بحث...")}
        ),
    )
...
class InspectionForm(forms.ModelForm):
    ...
    def __init__(self, *args, user=None, order=None, customer=None, **kwargs):
        ...
        self.fields["inspector"].queryset = User.objects.filter(
            is_active=True, is_inspection_technician=True
        )

        # Set responsible employee (salesperson) queryset
        from accounts.models import Salesperson

        self.fields["responsible_employee"].queryset = Salesperson.objects.filter(
            is_active=True
        )
        ...
        if order and order.salesperson:
            ...
            self.fields["responsible_employee"].help_text = _(
                "تم تعيين البائع تلقائياً من الطلب المرتبط"
            )

        # سيناريو 2: إذا كانت المعاينة مرتبطة بطلب وتم حفظها
        elif (
            self.instance.pk
            and hasattr(self.instance, "order")
            and self.instance.order
            and self.instance.order.salesperson
        ):
            # تعيين البائع الحالي من الطلب المرتبط
            self.initial["responsible_employee"] = self.instance.order.salesperson.id
            self.fields["responsible_employee"].initial = (
                self.instance.order.salesperson.id
            )
        ...
        # سيناريو 3: إذا كان البائع موجود بالفعل في المعاينة وهي من طلب
            ...
            if self.instance.responsible_employee:
                self.initial["responsible_employee"] = (
                    self.instance.responsible_employee.id
                )
                self.fields["responsible_employee"].initial = (
                    self.instance.responsible_employee.id
                )
            ...
            self.fields["responsible_employee"].widget.attrs["readonly"] = True
            self.fields["responsible_employee"].help_text = _(
                "لا يمكن تغيير البائع بعد إنشاء المعاينة من طلب"
            )
    ...
    def clean(self):
        ...
        if request_date and scheduled_date and scheduled_date < request_date:
            self.add_error(
                "scheduled_date",
                _("تاريخ التنفيذ يجب أن يكون بعد أو يساوي تاريخ الطلب"),
            )
    ...
    q = forms.CharField(
        ...
        widget=forms.TextInput(
            attrs={
                "class": "form-control",
                "placeholder": _(
                    "بحث بكود العميل أو اسم العميل أو رقم الطلب أو رقم العقد أو رقم الهاتف..."
                ),
            }
        ),
    )
