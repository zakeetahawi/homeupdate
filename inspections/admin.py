...
class InspectionAdmin(SoftDeleteAdminMixin, admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (_("ملاحظات"), {"fields": ("notes", "order_notes")}),
        (
            _("ملفات المعاينة"),
            {
                "fields": (
        ...
        for inspection in queryset:
            writer.writerow(
                [
                    ...
                    (
                        inspection.responsible_employee.name
    ...
    def inspection_by_code_view(self, request, inspection_code):
        ...
        try:
            ...
            self.message_user(
                request, f"المعاينة بكود {inspection_code} غير موجودة", level="error"
            )
            return HttpResponseRedirect(
                reverse("admin:inspections_inspection_changelist")
            )
    ...
    def inspection_code(self, obj):
        ...
        try:
            ...
            admin_url = reverse(
                "admin:inspections_inspection_by_code", kwargs={"inspection_code": code}
            )
    ...
    def get_queryset(self, request):
        return (
            ...
            .select_related("customer", "branch", "created_by", "inspector", "order")
            .prefetch_related("evaluations", "notifications")
        )
    ...
    def get_form(self, request, obj=None, **kwargs):
        ...
        if (
            ...
            if hasattr(form.base_fields, "inspector"):
                form.base_fields["inspector"].queryset = form.base_fields[
                    "inspector"
                ].queryset.filter(branch=request.user.branch, is_active=True)

            if hasattr(form.base_fields, "responsible_employee"):
                form.base_fields["responsible_employee"].queryset = form.base_fields[
                    "responsible_employee"
                ].queryset.filter(branch=request.user.branch, is_active=True)
...
class InspectionEvaluationAdmin(admin.ModelAdmin):
    list_display = ["inspection", "criteria", "rating", "created_by", "created_at"]
...
class InspectionReportAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (
            _("معلومات النظام"),
            {"classes": ("collapse",), "fields": ("created_by", "created_at")},
        ),
    )
...
class InspectionNotificationAdmin(admin.ModelAdmin):
    list_display = ["inspection", "type", "is_read", "scheduled_for", "created_at"]
    list_filter = ["type", "is_read", "created_at", "scheduled_for"]
    search_fields = ["inspection__id", "inspection__customer__name", "message"]
