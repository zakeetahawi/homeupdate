...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if force:
            ...
            inspections = Inspection.objects.filter(
                inspection_file__isnull=False, google_drive_file_name__isnull=True
            ).exclude(inspection_file="")

        total_count = inspections.count()

        if total_count == 0:
            self.stdout.write(self.style.WARNING("⚠️ لا توجد معاينات تحتاج إلى تحديث."))
            return

        self.stdout.write(f"📊 تم العثور على {total_count} معاينة تحتاج إلى تحديث.")
        ...
        for inspection in inspections:
            try:
                ...
                self.stdout.write(
                    f'   الملف الحالي: {inspection.inspection_file.name if inspection.inspection_file else "لا يوجد"}'
                )
                ...
                if not dry_run:
                    ...
                    inspection.save(
                        update_fields=["google_drive_file_name", "is_uploaded_to_drive"]
                    )
                    ...
                    self.stdout.write(
                        self.style.WARNING(
                            f"   🔍 سيتم تحديث المعاينة #{inspection.id}"
                        )
                    )
