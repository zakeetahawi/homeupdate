...
logs = OrderStatusLog.objects.filter(created_at__range=(start,end), notes__icontains="مزامنة حالة الطلب", changed_by__isnull=True).select_related('order')
...
with transaction.atomic():
    for l in logs:
        ...
        if cb:
            ...
            updated += 1
            print('UPDATED', before, '=>', l.changed_by_id, getattr(l.changed_by, 'username', None))
        else:
