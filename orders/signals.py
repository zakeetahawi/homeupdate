    ...
    try:
        ...
        if update_fields:
            ...
            try:
                ...
                old_st = (
                    order.tracker.previous("order_status")
                    or order.tracker.previous("tracking_status")
                    or ""
                )
...
def create_inspection_on_order_creation(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        if "inspection" in order_types:
            print(
                f"📋 الطلب {instance.order_number} من نوع معاينة - سيتم إنشاء معاينة تلقائية"
            )
            try:
                from django.db import transaction
                ...
                with transaction.atomic():
                    ...
                    print(
                        f"✅ تم إنشاء معاينة للطلب {instance.order_number} - معرف المعاينة: {inspection.id} (قيد الانتظار)"
                    )
                    Order.objects.filter(pk=instance.pk).update(
                ...
                error_msg = (
                    f"❌ خطأ في إنشاء معاينة للطلب {instance.order_number}: {str(e)}"
                )
            ...
            try:
                if (
                    ...
                    if hasattr(instance, "_modified_by") and instance._modified_by:
                        ...
                        hasattr(instance, "assigned_worker")
                        and instance.assigned_worker
                    ):
                        changed_by = instance.assigned_worker
                    elif hasattr(instance, "created_by") and instance.created_by:
                        changed_by = instance.created_by
                    elif (
                        hasattr(instance, "responsible_user")
                        and instance.responsible_user
                    ):
