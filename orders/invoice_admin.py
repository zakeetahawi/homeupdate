...
class InvoiceTemplateAdmin(admin.ModelAdmin):
    ...
    search_fields = ["name", "company_name", "company_address"]
    ...
    fieldsets = (
        ...
        (
            "النصوص المخصصة",
            {
                "fields": ("header_text", "footer_text", "terms_text"),
                "classes": ("collapse",),
            },
        ),
        (
    ...
    def is_default_display(self, obj):
        ...
        if obj.is_default:
            return format_html(
                '<span style="color: green; font-weight: bold;">{}</span>', "✓ افتراضي"
            )
    ...
    class Media:
        css = {"all": ("admin/css/invoice_template_admin.css",)}
...
class InvoicePrintLogAdmin(admin.ModelAdmin):
    ...
    list_filter = ["print_type", "printed_at", "template__name"]
    ...
    readonly_fields = ["order", "template", "printed_by", "printed_at", "print_type"]
