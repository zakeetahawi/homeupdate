...
def edit_tracking_report(request):
    ...
    edited_drafts = (
        DraftOrder.objects.filter(
            updated_at__gte=start_date, edit_history__isnull=False
        )
        .exclude(edit_history=[])
        .select_related("created_by", "last_modified_by", "customer")
        .prefetch_related("items__added_by", "items__modified_by")
    )
        ...
        for edit in draft.edit_history:
            ...
            if editor_name not in editors_stats:
                editors_stats[editor_name] = {
                    ...
                    "edit_count": 0,
                    "drafts_edited": set(),
                    "actions": {"add_item": 0, "remove_item": 0, "other": 0},
                }
        ...
        draft_stats.append(
            {
                ...
                "last_editor": (
                    draft.last_modified_by.get_full_name()
...
def draft_edit_detail(request, draft_id):
    ...
    draft = get_object_or_404(
        DraftOrder.objects.select_related(
            "created_by", "last_modified_by", "customer"
        ).prefetch_related("items__product", "items__added_by", "items__modified_by"),
        pk=draft_id,
    )
    ...
    for item in draft.items.all():
        if item.added_by and item.added_by != draft.created_by:
            modified_items.append(
                {"item": item, "action": "added", "by": item.added_by}
            )
        elif item.modified_by and item.modified_by != item.added_by:
            modified_items.append(
                {"item": item, "action": "modified", "by": item.modified_by}
            )
