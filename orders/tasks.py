...
def upload_contract_to_drive_async(self, order_id):
    ...
    try:
        ...
        if self.request.retries < self.max_retries:
            ...
            logger.error(
                f"فشل نهائي في رفع ملف العقد للطلب {order_id} بعد {self.max_retries} محاولات"
            )
    ...
    try:
        ...
        for order in failed_orders:
            try:
                ...
                logger.error(
                    f"خطأ في إعادة جدولة رفع ملف العقد للطلب {order.pk}: {str(e)}"
                )
        ...
        for inspection in failed_inspections:
            try:
                ...
                logger.error(
                    f"خطأ في إعادة جدولة رفع ملف المعاينة {inspection.pk}: {str(e)}"
                )
...
@shared_task
def update_order_status_async(
    order_id, old_status, new_status, changed_by_id=None, notes=""
):
    """
    ...
    try:
        ...
        logger.info(
            f"تم تحديث حالة الطلب {order.order_number} من {old_status} إلى {new_status}"
        )
...
def generate_contract_pdf_async(self, order_id, user_id=None):
    ...
    try:
        ...
        if contract_saved:
            logger.info(
                f"✅ Contract PDF generated for order {order.order_number} in {elapsed:.2f}s (background)"
            )
            return {
                "success": True,
            ...
            logger.error(
                f"❌ Failed to generate contract PDF for order {order.order_number}"
            )
        ...
        logger.error(
            f"❌ Error generating contract PDF for order {order_id}: {str(e)}",
            exc_info=True,
        )
