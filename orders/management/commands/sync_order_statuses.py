...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f'المعالَجة (دفعات): {total_pairs} أوامر تصنيع مختلفة عن حالة الطلب')
        ...
        for start in range(0, total_pairs, chunk):
            ...
            if not changed:
                self.stdout.write(f'دفعة {min(start+chunk, total_pairs)}/{total_pairs}: لا تغييرات')
                continue
            ...
            self.stdout.write(f'دفعة {min(start+chunk, total_pairs)}/{total_pairs}: محدثة {updated_from_mfg} إجمالاً')
        ...
        if InstallationSchedule:
            inst_order_ids_qs = InstallationSchedule.objects.filter(status='completed').values_list('order_id', flat=True).distinct()
            ...
            for start in range(0, total_inst_orders, chunk):
                ...
                with transaction.atomic():
                    ...
                    self.stdout.write(f'معالجة التركيبات: {min(start + chunk, total_inst_orders)}/{total_inst_orders} (محدثة حتى الآن {updated_by_install})')
        ...
        try:
            ...
            self.stdout.write(f'تم تحديث حالات المعاينات المكتملة إلى مكتمل: {completed_count}')
        ...
        try:
            remaining_qs = Order.objects.filter(manufacturing_order__isnull=True).exclude(order_status='completed')
            # قد نريد استثناء الطلبات التي للتصنيع أو أنواع أخرى، لكن هذا يحاكي السلوك الأصلي
            pending_updated = remaining_qs.exclude(inspections__status='completed', inspections__result='passed').distinct().update(order_status='pending')
            updated_count += pending_updated
            self.stdout.write(f'تم تحديث حالات الطلبات بدون تصنيع إلى pending: {pending_updated}')
        except Exception as e:
            self.stdout.write(self.style.ERROR(f'خطأ عند تحديث حالات الطلبات بدون تصنيع: {e}'))
