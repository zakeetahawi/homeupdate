...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if not branch:
            self.stdout.write(
                self.style.ERROR("No branches found. Please create a branch first.")
            )
            return
        ...
        if not customer:
            self.stdout.write(
                self.style.ERROR("No customers found. Please create a customer first.")
            )
            return
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(f"Created inspection order: {order.order_number}")
            )
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Inspection order created successfully. Total amount: {order.total_amount}"
                )
            )
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Inspection record created manually: {inspection.id}"
                )
            )

        except Exception as e:
            self.stdout.write(
                self.style.ERROR(f"Error creating inspection order: {str(e)}")
            )
