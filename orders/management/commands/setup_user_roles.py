...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--user-id',
            type=int,
            help='معرف المستخدم لتطبيق الدور عليه'
        )
        parser.add_argument(
            '--role',
            type=str,
            choices=['salesperson', 'branch_manager', 'region_manager', 'general_manager'],
            help='نوع الدور المراد تطبيقه'
        )
        parser.add_argument(
            '--branch-id',
            type=int,
            help='معرف الفرع (للمدير الفرع)'
        )
        parser.add_argument(
            '--managed-branches',
            nargs='+',
    ...
    def handle(self, *args, **options):
        ...
        if not user_id or not role:
            self.stdout.write(
                self.style.ERROR('يجب تحديد معرف المستخدم ونوع الدور')
            )
            return
        ...
        if role == 'salesperson':
            ...
            self.stdout.write(
                self.style.SUCCESS(f'تم تعيين {user.username} كبائع')
            )
            ...
            if branch_id:
                ...
                try:
                    ...
                    user.branch = branch
                    self.stdout.write(
                        self.style.SUCCESS(f'تم تعيين {user.username} كمدير فرع {branch.name}')
                    )
                ...
                self.stdout.write(
                    self.style.SUCCESS(f'تم تعيين {user.username} كمدير فرع (بدون فرع محدد)')
                )
            ...
            if managed_branches:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f'تم تعيين {user.username} كمدير منطقة (بدون فروع محددة)')
                )
        ...
        self.stdout.write(
            self.style.SUCCESS('تم إعداد الدور والصلاحيات بنجاح!')
        )
