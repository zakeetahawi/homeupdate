...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(f"Created order: {order.order_number}")
            )
            ...
            for product in products:
                ...
                item_type = (
                    "fabric"
                    if product.category and "قماش" in product.category.name.lower()
                    else "accessory"
                )
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Order created successfully with {len(products)} items. Total amount: {total_amount}"
                )
            )
