...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS('🧹 بدء تنظيف التخزين المؤقت...')
        )
        ...
        try:
            ...
            if options['all']:
                ...
                self.stdout.write(
                    self.style.SUCCESS('✅ تم مسح جميع البيانات المؤقتة')
                )
                ...
                try:
                    ...
                    self.stdout.write(
                        self.style.WARNING('⚠️ لا يمكن مسح بيانات العملاء بالنمط - يرجى استخدام --all')
                    )
                ...
                OrderCache.invalidate_order_stats_cache()
                self.stdout.write(
                    self.style.SUCCESS('✅ تم مسح الإحصائيات المؤقتة')
                )
...
def test_cache_functionality():
    ...
    try:
        ...
        end_time = time.time()
        results['performance'] = round((end_time - start_time) * 1000, 2)  # بالميلي ثانية
...
def generate_cache_report():
    ...
    try:
        ...
        if hasattr(cache, 'keys'):
            report['cache_keys'] = list(cache.keys('orders:*'))[:10]  # أول 10 مفاتيح فقط
    except:
    ...
    if report['functionality_test']['performance'] and report['functionality_test']['performance'] > 1000:
