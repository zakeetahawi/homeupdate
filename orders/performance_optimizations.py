...
def get_draft_items_with_usage(draft):
    ...
    """
    from orders.contract_models import CurtainFabric, CurtainAccessory
    from orders.wizard_models import DraftOrderItem
    from django.db.models import Value
    
    # Subquery لحساب الأقمشة المستخدمة
    fabric_usage = CurtainFabric.objects.filter(
        draft_order_item=OuterRef('pk'),
        curtain__draft_order=draft
    ).values('draft_order_item').annotate(
        total=Sum('meters')
    ).values('total')[:1]
    
    # Subquery لحساب الإكسسوارات المستخدمة
    accessory_usage = CurtainAccessory.objects.filter(
        draft_order_item=OuterRef('pk'),
        curtain__draft_order=draft
    ).values('draft_order_item').annotate(
        total=Sum('quantity')
...
def get_orders_list_optimized(user, filters=None, page_size=25):
    ...
    queryset = get_user_orders_queryset(user).select_related(
        'customer',
        'customer__branch',
        'salesperson',
        'salesperson__user',
        ...
        'tracking_status', 'total_amount', 'paid_amount', 'created_at',
        'contract_number', 'invoice_number', 'selected_types',
        'customer__id', 'customer__name', 'customer__phone', 'customer__code',
...
def get_cached_active_branches():
    ...
    if branches is None:
        ...
        branches = list(Branch.objects.filter(is_active=True).values(
...
def get_cached_active_salespersons(branch_id=None):
    ...
    if salespersons is None:
        ...
        queryset = Salesperson.objects.filter(is_active=True).select_related('user', 'branch')
        
        if branch_id:
            queryset = queryset.filter(branch_id=branch_id)
        
        salespersons = list(queryset.values(
            'id', 'name', 'employee_number', 'branch_id', 'user_id'
        ))
...
def get_cached_wizard_options(field_type):
    ...
    if options is None:
        ...
        options = list(WizardFieldOption.get_active_options(field_type).values(
...
def bulk_update_order_items(order, items_data):
    ...
    for data in items_data:
        if data.get('id') and data['id'] in items:
            ...
            item.discount_percentage = data.get('discount_percentage', item.discount_percentage)
...
def bulk_create_draft_items(draft, products_data):
    ...
    for data in products_data:
        ...
        if product_id and product_id in products:
            ...
            items_to_create.append(DraftOrderItem(
                draft_order=draft,
                product=product,
