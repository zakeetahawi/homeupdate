...
from .models import Order, OrderInvoiceImage, OrderItem, Payment
...
def get_total_steps(draft):
    ...
    if draft and draft.selected_type:
        ...
        needs_contract = draft.selected_type in [
            "installation",
            "tailoring",
            "accessory",
        ]
...
def wizard_start(request):
    ...
    user_drafts = get_user_drafts_optimized(
        request.user, is_completed=False, limit=max_drafts
    )
    ...
    if customer_id:
        ...
        if customer_draft:
            ...
            messages.info(
                request, f"لديك مسودة طلب غير مكتملة لهذا العميل. سيتم إكمالها."
            )
    ...
    if user_drafts.exists():
        ...
        if customer_id:
            customer = (
                Customer.objects.select_related("branch").filter(pk=customer_id).first()
            )
        elif customer_code:
            customer = (
...
def wizard_start_new(request):
    ...
    existing_drafts = (
        ...
        .only("id", "current_step", "customer__id", "customer__name", "branch__id")
    )
    ...
    if customer_code:
        customer = (
            Customer.objects.select_related("branch").filter(code=customer_code).first()
        )
    elif customer_id:
        customer = (
            Customer.objects.select_related("branch").filter(pk=customer_id).first()
    ...
    if customer:
        ...
        if customer_draft:
            ...
            messages.info(
                request,
                f'لديك مسودة طلب غير مكتملة للعميل "{customer.name}". سيتم إكمالها.',
            )
            return redirect("orders:wizard_step", step=customer_draft.current_step)
    ...
    if current_drafts_count >= max_drafts:
        messages.error(
            request,
            f"لقد وصلت للحد الأقصى المسموح من المسودات ({max_drafts}). يرجى إكمال أو حذف بعض المسودات أولاً.",
        )
        return redirect("orders:wizard_drafts_list")
...
def wizard_delete_and_create_new(request):
    ...
    DraftOrder.objects.filter(created_by=request.user, is_completed=False).delete()
...
def wizard_drafts_list(request):
    ...
    if (
        ...
        or request.user.groups.filter(name="مدير نظام").exists()
    ):
        # مدير النظام - عرض جميع المسودات
        drafts = DraftOrder.objects.filter(is_completed=False).select_related(
            "created_by", "customer", "branch"
        )
    elif request.user.groups.filter(name="مدير عام").exists() or (
        hasattr(request.user, "is_sales_manager") and request.user.is_sales_manager
    ):
        # مدير عام - عرض جميع المسودات
        drafts = DraftOrder.objects.filter(is_completed=False).select_related(
            "created_by", "customer", "branch"
        )
    elif request.user.groups.filter(name="مدير منطقة").exists() or (
        hasattr(request.user, "is_region_manager") and request.user.is_region_manager
    ):
        # مدير منطقة - عرض مسودات الفروع المُدارة
        managed_branches = (
            request.user.managed_branches.all()
            if hasattr(request.user, "managed_branches")
            else []
        )
        if managed_branches:
...
def wizard_step(request, step):
    ...
    try:
        draft = DraftOrder.objects.select_related(
            "created_by", "last_modified_by", "customer", "branch"
        ).get(pk=draft_id)
        ...
        if form.is_valid():
            ...
            if draft.created_by != request.user:
                ...
                if old_customer != draft.customer:
                    changes.append(
                        f'تغيير العميل من {old_customer.name if old_customer else "غير محدد"} إلى {draft.customer.name if draft.customer else "غير محدد"}'
                    )
                if old_branch != draft.branch:
                    changes.append(
                        f'تغيير الفرع من {old_branch.name if old_branch else "غير محدد"} إلى {draft.branch.name if draft.branch else "غير محدد"}'
                    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if (
                ...
                and old_selected_type != draft.selected_type
            ):
                type_names = {
                    "accessory": "إكسسوار",
                    "installation": "تركيب",
                ...
                old_name = (
                    type_names.get(old_selected_type, old_selected_type)
                    if old_selected_type
                    else "غير محدد"
                )
                new_name = (
                    type_names.get(draft.selected_type, draft.selected_type)
                    if draft.selected_type
                    else "غير محدد"
                )
...
def wizard_step_3_order_items(request, draft):
    ...
    items = draft.items.select_related(
        "product", "product__category", "added_by", "modified_by"
    ).all()
...
def wizard_add_item(request):
    ...
    try:
        ...
        logger.info(
            f"wizard_add_item: Session key: {request.session.session_key if hasattr(request, 'session') else 'No session'}"
        )

        # التحقق من وجود مسودة محددة في الجلسة (للتعديل)
        draft_id = (
            request.session.get("wizard_draft_id")
        ...
        if not product_id:
            return JsonResponse(
                {"success": False, "message": "لم يتم تحديد المنتج"}, status=400
            )
...
def wizard_complete_step_3(request):
    ...
    try:
        ...
        if not draft.items.exists():
            return JsonResponse(
                {"success": False, "message": "يجب إضافة عنصر واحد على الأقل"},
                status=400,
            )
        ...
        return JsonResponse(
            {"success": True, "next_step": 4, "message": "تم حفظ العناصر بنجاح"}
        )
...
def wizard_step_4_invoice_payment(request, draft):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if draft.created_by != request.user:
                ...
                if old_invoice_number != draft.invoice_number:
                    changes.append(
                        f'تغيير رقم الفاتورة من {old_invoice_number or "غير محدد"} إلى {draft.invoice_number or "غير محدد"}'
                    )
                if old_paid_amount != draft.paid_amount:
                    changes.append(
                        f"تغيير المبلغ المدفوع من {old_paid_amount} إلى {draft.paid_amount}"
                    )
        ...
        form = Step4InvoicePaymentForm(
            instance=draft, draft_order=draft, request=request
        )
...
def delete_draft_invoice_image(request, image_id):
    ...
    try:
        ...
        if image.draft_order.created_by != request.user:
            return JsonResponse(
                {"success": False, "error": "غير مصرح لك بحذف هذه الصورة"}, status=403
            )
        ...
        return JsonResponse(
            {"success": False, "error": "الصورة غير موجودة"}, status=404
        )
    except Exception as e:
...
def delete_main_draft_invoice_image(request):
    ...
    try:
        ...
        if not draft_id:
            return JsonResponse(
                {"success": False, "error": "لا توجد مسودة نشطة"}, status=404
            )
        ...
        if draft.invoice_image:
            ...
            return JsonResponse(
                {"success": False, "error": "لا توجد صورة رئيسية لحذفها"}, status=404
            )

    except DraftOrder.DoesNotExist:
        return JsonResponse(
            {"success": False, "error": "المسودة غير موجودة"}, status=404
        )
    except Exception as e:
        return JsonResponse({"success": False, "error": str(e)}, status=500)
...
def wizard_step_5_contract(request, draft):
    ...
    if request.method == "POST":
        ...
        # الرد برسالة نجاح
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
            return JsonResponse({"success": True, "message": "تم حفظ بيانات العقد"})
        else:
    ...
    fabric_usage = dict(
        CurtainFabric.objects.filter(
            curtain__draft_order=draft, draft_order_item__isnull=False
        )
        .values("draft_order_item_id")
        .annotate(total=models.Sum("meters"))
        .values_list("draft_order_item_id", "total")
    )

    accessory_usage = dict(
    ...
    try:
        ...
        if draft_id:
            draft = (
                ...
                .prefetch_related("items__product", "invoice_images_new")
                .filter(pk=draft_id)
                .first()
            )
        else:
            draft = (
                ...
                .prefetch_related("items__product", "invoice_images_new")
                .filter(created_by=request.user, is_completed=False)
                .order_by("-updated_at")
                .first()
            )
        ...
        draft_items_list = list(
            draft.items.all()
        )  # تم تحميلها مسبقاً من prefetch_related
        ...
        if is_editing and order:
            ...
            try:
                ...
                order.selected_types = (
                    [draft.selected_type] if draft.selected_type else []
                )
                ...
                for draft_item in draft_items_list:
                    if (
                        draft_item.original_item_id
                        and draft_item.original_item_id in current_order_items
                    ):
                        # تحديث عنصر موجود
                        ...
                        if (
                            ...
                            original_item.discount_amount = total_price * (
                                draft_item.discount_percentage / Decimal("100.0")
                            )
                        else:
                ...
                current_curtains = {
                    c.sequence: c for c in order.contract_curtains.all()
                }
                draft_curtains = list(
                    draft.contract_curtains.all().order_by("sequence")
                )
                ...
                for draft_curtain in draft_curtains:
                    ...
                    if seq in current_curtains:
                        ...
                        existing_curtain.installation_type = (
                            draft_curtain.installation_type
                        )
                        existing_curtain.curtain_box_width = (
                            draft_curtain.curtain_box_width
                        )
                        existing_curtain.curtain_box_depth = (
                            draft_curtain.curtain_box_depth
                        )
                        ...
                        logger.info(
                            f"🔄 تم تحديث الستارة {seq} للطلب {order.order_number}"
                        )
                    else:
                        # إنشاء ستارة جديدة
                        ...
                        logger.info(
                            f"➕ تم إنشاء ستارة جديدة {seq} للطلب {order.order_number}"
                        )
                ...
                curtains_to_delete = [
                    seq
                    for seq in current_curtains.keys()
                    if seq not in processed_sequences
                ]
                if curtains_to_delete:
                    deleted_count = ContractCurtain.objects.filter(
                        order=order, sequence__in=curtains_to_delete
                    ).delete()[0]
                    logger.info(
                        f"🗑️ تم حذف {deleted_count} ستارة قديمة للطلب {order.order_number}"
                    )
                ...
                if draft.invoice_images_new.exists():
                    OrderInvoiceImage.objects.filter(
                        order=order
                    ).delete()  # حذف القديم فقط إذا فيه جديد
                    new_images = [
                ...
                if should_regenerate:
                    logger.info(
                        f"🔄 إعادة توليد العقد في وضع التعديل للطلب {order.order_number}"
                    )
                    ...
                    def regenerate_contract():
                        try:
                            ...
                            task = generate_contract_pdf_async.delay(
                                order.id, request.user.id
                            )
                            logger.info(
                                f"⚡ تم بدء إعادة توليد العقد (المهمة: {task.id}) للطلب #{order.id}"
                            )
                        except Exception as e:
                            logger.error(
                                f"❌ خطأ في إعادة توليد العقد: {e}", exc_info=True
                            )
        ...
        if not order:
            ...
            if curtains:
                for c in curtains:
                    c.order = order
                    c.draft_order = None
                ContractCurtain.objects.bulk_update(
        ...
        logger.info(
            f"🔍 فحص توليد العقد - نوع الطلب: {selected_type}, يحتاج عقد: {should_generate_contract}, رقم الطلب: {order.order_number}"
        )
            ...
            def trigger_contract_generation():
                try:
                    ...
                    if (
                        ...
                        logger.info(
                            f"⚡ تم بدء توليد العقد (المهمة: {task.id}) للطلب #{order_pk}"
                        )
                    else:
                        # توليد مباشر إذا لم يكن Celery متاحاً
                        logger.info(
                            f"⚡ توليد العقد مباشرةً (Celery غير متاح) للطلب #{order_pk}"
                        )
                        ...
                        if result:
                            logger.info(
                                f"✅ تم توليد العقد مباشرةً للطلب {order_obj.order_number}"
                            )
                        else:
                            logger.error(
                                f"❌ فشل توليد العقد مباشرةً للطلب {order_obj.order_number}"
                            )
                except Exception as e:
        ...
        return JsonResponse(
            {
                "success": True,
                "message": (
                    "تم حفظ الطلب بنجاح" if is_editing else "تم إنشاء الطلب بنجاح"
                ),
                "order_id": order.pk,
                "order_number": order.order_number,
...
def wizard_delete_draft(request, draft_id):
    ...
    try:
        ...
        if not (
            ...
            or request.user.groups.filter(name__in=["مدير نظام", "مدير عام"]).exists()
        ):
    ...
    try:
        ...
        if request.method == "POST":
            ...
            if draft.created_by == request.user:
                ...
                messages.info(
                    request,
                    f"تم الخروج من المسودة (المسودة تابعة لـ {draft.created_by.get_full_name()})",
                )
...
def wizard_add_curtain(request):
    ...
    try:
        ...
        try:
            ...
            return JsonResponse(
                {"success": False, "message": f"خطأ في تنسيق البيانات: {str(e)}"},
                status=400,
            )
        ...
        logger.info(
            f"Adding curtain - Room: {room_name}, Width: {width}, Height: {height}"
        )
        logger.info(
            f"Fabrics: {len(fabrics_data)}, Accessories: {len(accessories_data)}"
        )
        ...
        if not room_name:
            return JsonResponse(
                {"success": False, "message": "يرجى إدخال اسم الغرفة"}, status=400
            )

        if not width or not height:
            return JsonResponse(
                {"success": False, "message": "يرجى إدخال العرض والارتفاع"}, status=400
            )
        ...
        try:
            ...
            if width <= 0 or height <= 0:
                return JsonResponse(
                    {"success": False, "message": "يجب أن تكون القيم أكبر من صفر"},
                    status=400,
                )
        except (ValueError, TypeError):
            return JsonResponse(
                {"success": False, "message": "قيم غير صحيحة للعرض أو الارتفاع"},
                status=400,
            )
            ...
            try:
                ...
                if item_id:
                    try:
                        ...
                        logger.info(
                            f"Found draft item: {draft_order_item.product.name}"
                        )
                    except Exception as e:
                ...
                fabric = CurtainFabric(
                    ...
                    meters=Decimal(
                        str(round(float(fabric_data.get("meters", 0)), 3))
                    ),  # تقريب لتجنب مشاكل دقة الفاصلة العائمة
                ...
                try:
                    ...
                    fabric.save()
                    logger.info(
                        f"Saved fabric: {fabric.fabric_name} - {fabric.meters}m"
                    )
                ...
                return JsonResponse(
                    {"success": False, "message": f"خطأ في إضافة القماش: {str(e)}"},
                    status=400,
                )
            ...
            try:
                ...
                if item_id:
                    try:
                        draft_order_item = draft.items.get(id=item_id)
                        logger.info(
                            f"Found draft item for accessory: {draft_order_item.product.name}"
                        )
                    except Exception as e:
                        logger.warning(f"Could not find draft item {item_id}: {e}")
                ...
                if (
                    quantity_val
                    and quantity_val != ""
                    and quantity_val != "null"
                    and quantity_val != "None"
                ):
                    try:
                        # تقريب القيمة إلى 3 منازل عشرية لتجنب مشاكل دقة الفاصلة العائمة
                ...
                try:
                    ...
                    accessory.save()
                    logger.info(
                        f"Saved accessory: {accessory.accessory_name} - count: {count} × size: {size} = quantity: {quantity}"
                    )
                except ValidationError as ve:
                    # إرجاع رسالة خطأ واضحة للمستخدم
                    ...
                    return JsonResponse(
                        {
                            "success": False,
                            "message": "خطأ في بيانات الإكسسوار: "
                            + " | ".join(error_msgs),
                        },
                        status=400,
                    )
                ...
                return JsonResponse(
                    {"success": False, "message": f"خطأ في إضافة الإكسسوار: {str(e)}"},
                    status=400,
                )
        ...
        logger.error(
            f"Request data: {request.body[:500] if request.body else 'No body'}"
        )
        return JsonResponse(
            {"success": False, "message": f"حدث خطأ أثناء حفظ الستارة: {str(e)}"},
            status=500,
        )
...
def wizard_edit_curtain(request, curtain_id):
    ...
    try:
        ...
        if request.method == "GET":
            ...
            for fabric in curtain.fabrics.all():
                ...
                if fabric.draft_order_item_id:
                    ...
                    matching_item = draft.items.filter(
                        product__name=fabric.fabric_name
                    ).first()
                    if matching_item:
                ...
                fabrics_data.append(
                    {
                        ...
                        "source": (
                            "belt"
                            if is_belt
                            else ("invoice" if item_id else "external")
                        ),
            ...
            for accessory in curtain.accessories.all():
                ...
                if accessory.draft_order_item_id:
                    ...
                    # البحث عن العنصر بالاسم في المسودة
                    matching_item = draft.items.filter(
                        product__name=accessory.accessory_name
                    ).first()
                    if matching_item:
                        item_id = str(matching_item.id)
                        source = "invoice"
                ...
                accessories_data.append(
                    {
                        ...
                        "display_type": accessory.accessory_type
                        or "",  # نوع الإكسسوار الشائع
            ...
            return JsonResponse(
                {
                    ...
                    "curtain": {
                        ...
                        "curtain_box_width": (
                            float(curtain.curtain_box_width)
                            if curtain.curtain_box_width
                            else None
                        ),
                        "curtain_box_depth": (
                            float(curtain.curtain_box_depth)
                            if curtain.curtain_box_depth
                            else None
                        ),
            ...
            if not width or not height:
                return JsonResponse(
                    {"success": False, "message": "يرجى إدخال العرض والارتفاع"},
                    status=400,
                )

            try:
                width = Decimal(
                    str(round(float(width), 3))
                )  # تقريب لتجنب مشاكل دقة الفاصلة العائمة
                height = Decimal(
            ...
            curtain.curtain_box_width = (
                Decimal(str(round(float(curtain_box_width), 3)))
                if curtain_box_width
                else None
            )
            curtain.curtain_box_depth = (
                Decimal(str(round(float(curtain_box_depth), 3)))
                if curtain_box_depth
                else None
            )
            ...
            for accessory_data in accessories_data:
                try:
                    ...
                    accessory = CurtainAccessory(
                        ...
                        draft_order_item=draft_order_item,
                        accessory_name=accessory_data.get("name", ""),
                        accessory_type=accessory_data.get(
                            "display_type", ""
                        ),  # نوع الإكسسوار الشائع
                        count=count,
                        size=size,
                        quantity=quantity,
                        color=accessory_data.get("color", ""),
        ...
        return JsonResponse(
            {"success": False, "message": f"حدث خطأ أثناء تعديل الستارة: {str(e)}"},
            status=500,
        )
...
def wizard_remove_curtain(request, curtain_id):
    ...
    try:
        ...
        return JsonResponse(
            {"success": False, "message": f"حدث خطأ أثناء حذف الستارة: {str(e)}"},
            status=500,
        )
...
def wizard_upload_contract(request):
    ...
    try:
        ...
        if "contract_file" not in request.FILES:
            return JsonResponse(
                {"success": False, "message": "لم يتم رفع أي ملف"}, status=400
            )
        ...
        if not contract_file.name.lower().endswith(".pdf"):
            return JsonResponse(
                {"success": False, "message": "يجب أن يكون الملف بصيغة PDF"}, status=400
            )
        ...
        return JsonResponse(
            {"success": False, "message": f"حدث خطأ أثناء رفع الملف: {str(e)}"},
            status=500,
        )
...
def wizard_remove_contract_file(request):
    ...
    try:
        ...
        return JsonResponse({"success": True, "message": "تم حذف ملف العقد بنجاح"})

    except Exception as e:
        logger.error(f"Error removing contract file: {e}")
        return JsonResponse(
            {"success": False, "message": f"حدث خطأ أثناء حذف الملف: {str(e)}"},
            status=500,
        )
...
def wizard_edit_order(request, order_pk):
    ...
    if not request.user.has_perm("orders.change_order"):
        if order.created_by != request.user:
            ...
            return redirect(
                "orders:order_detail_by_number", order_number=order.order_number
            )
...
def _create_draft_from_order(order, user):
    ...
    draft = DraftOrder.objects.create(
        ...
        status=order.status,
        selected_type=(
            order.get_selected_types_list()[0]
            if order.get_selected_types_list()
            else None
        ),
        notes=order.notes,
    ...
    for curtain in order.contract_curtains.all():
        ...
        for accessory in curtain.accessories.all():
            CurtainAccessory.objects.create(
                curtain=new_curtain,
                draft_order_item=(
                    item_mapping.get(accessory.draft_order_item_id)
                    if accessory.draft_order_item_id
                    else None
                ),
                accessory_name=accessory.accessory_name,
...
def wizard_edit_options(request, order_pk):
    ...
    has_contract = order.selected_types and any(
        t in ["installation", "tailoring", "accessory"] for t in order.selected_types
    )
    ...
    if request.method == "POST":
        ...
        if new_type and new_type != old_type:
            ...
            old_option = WizardFieldOption.objects.filter(
                field_type="service_type", value=old_type, is_active=True
            ).first()

            new_option = WizardFieldOption.objects.filter(
                field_type="service_type", value=new_type, is_active=True
            ).first()
            ...
            logger.info(
                f"Order type and service changed: {order.order_number} from {old_type} to {new_type}"
            )
            messages.success(
                request,
                f'تم تغيير نوع الطلب والخدمة من "{old_display}" إلى "{new_display}" بنجاح',
            )
