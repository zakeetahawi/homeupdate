...
from .models import Order, OrderStatusLog, OrderItem
    ...
    for field_name, field_display in TRACKED_FIELDS.items():
        try:
            ...
            if hasattr(old_value_raw, 'strftime'):
                old_value_display = old_value_raw.strftime('%Y-%m-%d') if old_value_raw else 'غير محدد'
            if hasattr(new_value_raw, 'strftime'):
                new_value_display = new_value_raw.strftime('%Y-%m-%d') if new_value_raw else 'غير محدد'
            ...
            if field_name in ['salesperson', 'customer', 'branch']:
                ...
                old_compare = str(old_value_raw) if old_value_raw not in [None, ''] else ''
                new_compare = str(new_value_raw) if new_value_raw not in [None, ''] else ''
            ...
            if old_compare != new_compare:
                ...
                # إنشاء رسالة واضحة
                if field_name in ['contract_number', 'contract_number_2', 'contract_number_3']:
                    notes = f'تم تحديث {field_display}'
                elif field_name in ['invoice_number', 'invoice_number_2', 'invoice_number_3']:
                    notes = f'تم تحديث {field_display}'
            ...
            logger.error(f"خطأ في تتبع تغيير الحقل {field_name} للطلب {instance.pk}: {e}")
