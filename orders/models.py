...
class Order(models.Model):
        ...
        try:
            ...
            if not self.pk:
                raise models.ValidationError(
                    "فشل في حفظ الطلب: لم يتم إنشاء مفتاح أساسي"
                )
            ...
            if self.contract_file and not self.is_contract_uploaded_to_drive:
                try:
                    ...
                    logger.error(
                        f"خطأ في جدولة رفع ملف العقد للطلب {self.order_number}: {str(e)}"
                    )
                    ...
                    try:
                        ...
                        if success:
                            logger.info(
                                f"تم رفع ملف العقد للطلب {self.order_number} بنجاح (مباشر)"
                            )
                        else:
                            logger.warning(
                                f"فشل في رفع ملف العقد للطلب {self.order_number}: {message}"
                            )
                    except Exception as e2:
                        logger.error(
                            f"خطأ في رفع ملف العقد للطلب {self.order_number}: {str(e2)}"
                        )
            ...
            if is_new or "final_price" not in kwargs.get("update_fields", []):
                try:
                    ...
                    try:
                        ...
                        logger.error(
                            f"خطأ في حساب السعر النهائي للطلب {self.order_number}: {str(calc_error)}"
                        )
                        self.final_price = 0
    ...
    def get_smart_delivery_date(self):
        ...
        if self.order_status in ["completed", "ready_install", "delivered"]:
            ...
            if (
                hasattr(self, "manufacturing_order")
                and self.manufacturing_order.completion_date
            ):
            ...
            # إذا كان تم التسليم، التحقق من تاريخ التسليم الفعلي
                ...
                and hasattr(self, "manufacturing_order")
                and self.manufacturing_order.delivery_date
            ):
    ...
    def is_manufacturing_order(self):
        ...
        return (
            hasattr(self, "manufacturing_order")
            and self.manufacturing_order is not None
        )
...
class Payment(models.Model):
    ...
    reference_number = models.CharField(
        max_length=100, blank=True, verbose_name="رقم المرجع"
    )
    notes = models.TextField(blank=True, verbose_name="ملاحظات")
    created_by = models.ForeignKey(
        settings.AUTH_USER_MODEL,
