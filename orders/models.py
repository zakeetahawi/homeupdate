...
class Order(SoftDeleteMixin, models.Model):
    ...
    order_date = models.DateTimeField(
        default=timezone.now, editable=True, verbose_name="تاريخ الطلب"
    )
    ...
    service_types = models.JSONField(
        default=list, blank=True, verbose_name="أنواع الخدمات"
    )
    ...
    last_notification_date = models.DateTimeField(
        null=True, blank=True, verbose_name="تاريخ آخر إشعار"
    )
    invoice_number = models.CharField(
        max_length=50, null=True, blank=True, verbose_name="رقم الفاتورة"
    )
    invoice_number_2 = models.CharField(
        max_length=50, null=True, blank=True, verbose_name="رقم الفاتورة الإضافي 2"
    )
    invoice_number_3 = models.CharField(
        max_length=50, null=True, blank=True, verbose_name="رقم الفاتورة الإضافي 3"
    )
    contract_number = models.CharField(
        max_length=50, null=True, blank=True, verbose_name="رقم العقد"
    )
    contract_number_2 = models.CharField(
        max_length=50, null=True, blank=True, verbose_name="رقم العقد الإضافي 2"
    )
    contract_number_3 = models.CharField(
        max_length=50, null=True, blank=True, verbose_name="رقم العقد الإضافي 3"
    )
    ...
    expected_delivery_date = models.DateField(
        null=True, blank=True, verbose_name="تاريخ التسليم المتوقع"
    )
    ...
    contract_google_drive_file_url = models.URLField(
        blank=True, null=True, verbose_name="رابط ملف العقد في Google Drive"
    )
    ...
    is_contract_uploaded_to_drive = models.BooleanField(
        default=False, verbose_name="تم رفع العقد إلى Google Drive"
    )
        ...
        try:
            ...
            for it in items_to_calc:
                try:
                    ...
                    logger.error(
                        f"Error calculating item {it.id} totals for order {self.pk}: {item_err}"
                    )
            ...
            logger = logging.getLogger(__name__)
            logger.error(
                f"Global error in calculate_final_price for order {self.pk}: {e}"
            )
        ...
        try:
            ...
            if has_services:
                ...
                self.service_types = [
                    t
                    for t in selected_types
                    if t in ["installation", "inspection", "transport", "tailoring"]
                ]
            ...
            if not self.contract_number and (
                "tailoring" in selected_types or "installation" in selected_types
            ):
            ...
            # تسجيل الخطأ
            logger.error(
                f"خطأ في حفظ الطلب {getattr(self, 'order_number', 'غير محدد')}: {str(e)}"
            )
            raise
    ...
    def generate_unique_order_number(self):
        ...
        try:
            customer_code = (
                self.customer.code
                if hasattr(self.customer, "code") and self.customer.code
                else "UNKNOWN"
            )
            ...
            last_order = (
                Order.objects.filter(
                    customer=self.customer, order_number__startswith=customer_code
                )
                .exclude(pk=self.pk)
            ...
            for attempt in range(max_attempts):
                ...
                if (
                    not Order.objects.filter(order_number=potential_order_number)
                    .exclude(pk=self.pk)
                    .exists()
                ):
    ...
    def upload_contract_to_google_drive(self):
        ...
        try:
            ...
            from orders.services.google_drive_service import (
                get_contract_google_drive_service,
            )
            ...
            # رفع الملف
            result = drive_service.upload_contract_file(self.contract_file.path, self)
    ...
    def update_installation_status(self):
        ...
        if (
            getattr(self, "_updating_installation_status", False)
            or getattr(self, "_updating_installation_status_bulk", False)
            or getattr(self, "_updating_from_signal", False)
        ):
            return
    ...
    def update_completion_status(self):
        ...
        if (
            "installation" in self.get_selected_types_list()
            or "tailoring" in self.get_selected_types_list()
        ):
    ...
    def get_display_status(self):
        ...
        if "products" in types:
            try:
                cutting_items = self.items.filter(
                    cutting_status__in=["pending", "in_progress"]
                )
    ...
    def get_display_inspection_status(self):
        ...
        if "inspection" in self.get_selected_types_list():
            ...
            if inspection:
                ...
                return {
                    "status": inspection.status,
                    "text": inspection_texts.get(
                        inspection.status, inspection.get_status_display()
                    ),
        ...
        # إذا كان طلب تفصيل أو تركيب
            ...
            "tailoring" in self.get_selected_types_list()
            or "installation" in self.get_selected_types_list()
        ):
...
def update_order_installation_status(sender, instance, **kwargs):
    ...
    try:
        ...
        if hasattr(instance, "_bulk_update") or hasattr(
            instance.order, "_updating_installation_status_bulk"
        ):
            return
...
from django.db.models.signals import post_delete as oi_post_delete
from django.db.models.signals import post_save as oi_post_save
...
class OrderItem(SoftDeleteMixin, models.Model):
    ...
    def mark_cutting_completed(
        self, cutter_name, permit_number, receiver_name, user, notes=""
    ):
        ...
        try:
            ...
            if not is_new and not force_update and self.order.paid_amount > 0:
                ...
                try:
                    ...
                    if (
                        ...
                        or old_item.discount_percentage != self.discount_percentage
                    ):
                        ...
                        logger = logging.getLogger(__name__)
                        logger.warning(
                            f"محاولة تعديل أسعار طلب مدفوع {self.order.order_number}"
                        )
            ...
            if is_new or force_update:
                try:
                    ...
                    logger = logging.getLogger(__name__)
                    logger.error(
                        f"خطأ في تحديث السعر النهائي للطلب {self.order.pk}: {str(calc_error)}"
                    )
        ...
        if self.order_id:
            try:
                ...
                try:
                    ...
                    debt = CustomerDebt.objects.filter(
                        order=self.order, is_paid=False
                    ).first()
                    if debt:
                        ...
                        if remaining <= 0:
                            ...
                            if debt.payment_receiver_name:
                                debt.payment_receiver_name = (
                                    debt.payment_receiver_name.get_full_name()
                                    or debt.payment_receiver_name.username
                                )
                            debt.save()
                        else:
...
class OrderNote(models.Model):
    ...
    is_visible_to_customer = models.BooleanField(
        default=False, verbose_name="مرئية للعميل"
    )
...
class OrderStatusLog(models.Model):
    ...
    created_at = models.DateTimeField(
        auto_now_add=True, verbose_name=_("تاريخ التغيير")
    )
    ...
    def _status_label(self, code):
        ...
        tracking_map = (
            dict(Order.TRACKING_STATUS_CHOICES) if "Order" in globals() else {}
        )
    ...
    def get_detailed_description(self):
        ...
        if self.change_type == "creation":
            ...
            return (
                self.notes
                or f"تحديث حالة التصنيع من {self.old_status_pretty} إلى {self.new_status_pretty}"
            )
        elif self.change_type == "installation":
            # عرض خاص لتحديثات التركيب
            return (
                self.notes
                or f"تحديث حالة التركيب من {self.old_status_pretty} إلى {self.new_status_pretty}"
            )
        elif self.change_type == "cutting":
            # عرض خاص لتحديثات التقطيع
            return (
                self.notes
                or f"تحديث حالة التقطيع من {self.old_status_pretty} إلى {self.new_status_pretty}"
            )
        elif self.change_type == "inspection":
            # عرض خاص لتحديثات المعاينة
            return (
                self.notes
                or f"تحديث حالة المعاينة من {self.old_status_pretty} إلى {self.new_status_pretty}"
            )
        elif self.change_type == "complaint":
            # عرض خاص لتحديثات الشكاوى
            return (
                self.notes
                or f"تحديث حالة الشكوى من {self.old_status_pretty} إلى {self.new_status_pretty}"
            )
        ...
        try:
            ...
            if self.change_type == "status" and not self.is_automatic:
                try:
                    ...
                    if self.order and self.new_status != getattr(
        ...
        try:
            ...
            change_details.update(
                {
                    ...
                    "old_value": (
                        str(old_value) if old_value not in [None, ""] else "غير محدد"
                    ),
                    "new_value": (
                        str(new_value) if new_value not in [None, ""] else "غير محدد"
                    ),
                }
            )
            ...
            if change_type == "customer":
                change_details.update(
                    {
                        "old_customer_name": (
                            getattr(old_value, "name", str(old_value))
                            if old_value
                            else "غير محدد"
                        ),
                        "new_customer_name": (
                            getattr(new_value, "name", str(new_value))
                            if new_value
                            else "غير محدد"
                        ),
                    }
...
class ManufacturingDeletionLog(models.Model):
    ...
    order_id = models.PositiveIntegerField(
        verbose_name=_("معرف الطلب المحذوف"), default=0
    )
    order_number = models.CharField(
        max_length=50, verbose_name="رقم الطلب المحذوف", blank=True, null=True
    )
    ...
    manufacturing_order_data = models.JSONField(
        default=dict, verbose_name="بيانات أمر التصنيع المحذوف"
    )
...
class DeliveryTimeSettings(models.Model):
    ...
    delivery_days = models.PositiveIntegerField(
        verbose_name="عدد أيام التسليم", help_text="عدد الأيام المتوقعة للتسليم"
    )
        ...
        try:
            ...
            if service_type and order_type:
                ...
                setting = cls.objects.filter(
                    service_type=service_type, order_type=order_type, is_active=True
                ).first()
                if setting:
            ...
            if service_type:
                setting = cls.objects.filter(
                    service_type=service_type, order_type__isnull=True, is_active=True
                ).first()
                if setting:
                    return setting.delivery_days
            ...
            if order_type:
                setting = cls.objects.filter(
                    order_type=order_type, service_type__isnull=True, is_active=True
                ).first()
                if setting:
                    return setting.delivery_days
