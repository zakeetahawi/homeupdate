...
class ContractTemplate(models.Model):
    ...
    company_phone = models.CharField(
        max_length=100, blank=True, verbose_name="هاتف الشركة"
    )
    ...
    company_tax_number = models.CharField(
        max_length=100, blank=True, verbose_name="الرقم الضريبي"
    )
    company_commercial_register = models.CharField(
        max_length=100, blank=True, verbose_name="السجل التجاري"
    )
    ...
    secondary_color = models.CharField(
        max_length=7, default="#6c757d", verbose_name="اللون الثانوي"
    )
    accent_color = models.CharField(
        max_length=7, default="#ffc107", verbose_name="لون التمييز"
    )
    font_family = models.CharField(
        max_length=100, default="Arial, sans-serif", verbose_name="نوع الخط"
    )
    font_size = models.IntegerField(default=14, verbose_name="حجم الخط (px)")
    ...
    css_styles = models.TextField(
        blank=True, verbose_name="أنماط CSS مخصصة", help_text="أنماط CSS إضافية للقالب"
    )
    ...
    show_company_logo = models.BooleanField(
        default=True, verbose_name="عرض شعار الشركة"
    )
    show_order_details = models.BooleanField(
        default=True, verbose_name="عرض تفاصيل الطلب"
    )
    show_customer_details = models.BooleanField(
        default=True, verbose_name="عرض بيانات العميل"
    )
    show_items_table = models.BooleanField(
        default=True, verbose_name="عرض جدول العناصر"
    )
    show_payment_details = models.BooleanField(
        default=True, verbose_name="عرض تفاصيل الدفع"
    )
    ...
    header_text = models.TextField(
        blank=True, verbose_name="نص الرأس", help_text="نص يظهر في أعلى العقد"
    )
    footer_text = models.TextField(
        blank=True, verbose_name="نص التذييل", help_text="نص يظهر في أسفل العقد"
    )
    terms_text = models.TextField(
        blank=True, verbose_name="الشروط والأحكام", default="شروط وأحكام العقد..."
    )
...
class ContractCurtain(models.Model):
    ...
    width = models.DecimalField(
        max_digits=10, decimal_places=2, verbose_name="العرض", help_text="بالمتر"
    )
    height = models.DecimalField(
        max_digits=10, decimal_places=2, verbose_name="الطول", help_text="بالمتر"
    )
    ...
    # الإكسسوارات
    # خشب
    wood_quantity = models.IntegerField(default=0, verbose_name="عدد الخشب")
    wood_type = models.CharField(
        max_length=100, blank=True, verbose_name="نوع الخشب", help_text="يُكتب يدوياً"
    )
    ...
    track_quantity = models.IntegerField(default=0, verbose_name="عدد المجرى")
    ...
    pipe_quantity = models.IntegerField(default=0, verbose_name="عدد المواسير")
    ...
    bracket_quantity = models.IntegerField(default=0, verbose_name="عدد الكوابيل")
    ...
    finial_quantity = models.IntegerField(default=0, verbose_name="عدد النهايات")
    ...
    ring_quantity = models.IntegerField(default=0, verbose_name="عدد الطبة")
    ...
    hanger_quantity = models.IntegerField(default=0, verbose_name="عدد الشماعات")
    ...
    valance_quantity = models.IntegerField(default=0, verbose_name="عدد الفرانشة")
    ...
    tassel_quantity = models.IntegerField(default=0, verbose_name="عدد الشرابة")
    ...
    tieback_quantity = models.IntegerField(default=0, verbose_name="عدد المرابط")
    ...
    belt_quantity = models.IntegerField(default=0, verbose_name="عدد حزام الوسط")
    ...
    def clean(self):
        ...
        if self.light_fabric and self.light_fabric_meters:
            if self.light_fabric_meters > self.light_fabric.quantity:
                errors["light_fabric_meters"] = (
                    f"الكمية المطلوبة ({self.light_fabric_meters}) أكبر من الكمية المتاحة ({self.light_fabric.quantity})"
                )

        if self.heavy_fabric and self.heavy_fabric_meters:
            if self.heavy_fabric_meters > self.heavy_fabric.quantity:
                errors["heavy_fabric_meters"] = (
                    f"الكمية المطلوبة ({self.heavy_fabric_meters}) أكبر من الكمية المتاحة ({self.heavy_fabric.quantity})"
                )

        if self.blackout_fabric and self.blackout_fabric_meters:
            if self.blackout_fabric_meters > self.blackout_fabric.quantity:
                errors["blackout_fabric_meters"] = (
                    f"الكمية المطلوبة ({self.blackout_fabric_meters}) أكبر من الكمية المتاحة ({self.blackout_fabric.quantity})"
                )
...
class CurtainFabric(models.Model):
    ...
    meters = models.DecimalField(
        max_digits=8, decimal_places=2, default=0, verbose_name="عدد الأمتار"
    )
    ...
    notes = models.TextField(
        blank=True, verbose_name="ملاحظات القماش", help_text="ملاحظات خاصة بهذا القماش"
    )
    ...
    def __str__(self):
        ...
        return (
            f"{self.get_fabric_type_display()} - {self.display_name} ({self.meters}م)"
        )
    ...
    def get_tailoring_type_display(self):
        ...
        try:
            ...
            option = WizardFieldOption.objects.filter(
                field_type="tailoring_type", value=self.tailoring_type, is_active=True
            ).first()
    ...
    def clean(self):
        ...
        if self.order_item and self.meters:
            ...
            used_total = (
                CurtainFabric.objects.filter(order_item=self.order_item)
                .exclude(pk=self.pk)
                .aggregate(total=models.Sum("meters"))["total"]
                or 0
            )
            ...
            if self.meters > available:
                errors["meters"] = (
                    f"الكمية المطلوبة ({self.meters}م) أكبر من المتاح ({available}م من {self.order_item.quantity}م)"
                )
        ...
        if self.draft_order_item and self.meters:
            ...
            ).exclude(pk=self.pk).aggregate(total=Sum("meters"))["total"] or Decimal(
                "0"
            )
...
class CurtainAccessory(models.Model):
    ...
    accessory_name = models.CharField(max_length=200, verbose_name="اسم الإكسسوار")
    ...
    count = models.IntegerField(default=1, verbose_name="العدد", help_text="عدد القطع")
    ...
    def clean(self):
        ...
        if self.order_item and self.quantity:
            # حساب إجمالي ما تم استخدامه من هذا العنصر
            used_total = (
                CurtainAccessory.objects.filter(order_item=self.order_item)
                .exclude(pk=self.pk)
            ...
            if self.quantity > available:
                errors["quantity"] = (
                    f"الكمية المطلوبة ({self.quantity}) أكبر من المتاح ({available} من {self.order_item.quantity})"
                )
        ...
        if self.draft_order_item and self.quantity:
            ...
            used_in_fabrics = CurtainFabric.objects.filter(
                draft_order_item=self.draft_order_item
            ).aggregate(total=Sum("meters"))["total"] or Decimal("0")
