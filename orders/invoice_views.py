...
def save_template(request):
    ...
    try:
        ...
        template.company_website = company_info.get('website') or (company_info_db.website if company_info_db else '')
        ...
        template.show_customer_details = content_settings.get('show_customer_details', True)
        template.show_payment_details = content_settings.get('show_payment_details', True)
...
def delete_template(request, template_id):
    ...
    try:
        ...
        if template.is_default:
            active_templates_count = InvoiceTemplate.objects.filter(is_active=True).count()
            if active_templates_count <= 1:
                return JsonResponse({
                    'success': False, 
                    'error': 'لا يمكن حذف القالب الافتراضي الوحيد'
                })
        ...
        return JsonResponse({
            'success': True,
            'message': f'تم حذف القالب "{template_name}" بنجاح'
        })
...
def set_default_template(request, template_id):
    ...
    try:
        ...
        return JsonResponse({
            'success': True,
            'message': f'تم تعيين "{template.name}" كقالب افتراضي'
        })
...
def preview_invoice(request, order_id, template_id=None):
    ...
    try:
        ...
        logger.info(f"معاينة موحدة لفاتورة الطلب {order.order_number} باستخدام القالب الافتراضي (المحرر البسيط)")
...
def print_invoice_with_template(request, order_id, template_id=None):
    ...
    try:
        order = get_object_or_404(Order, id=order_id)
        logger.info(f"طباعة موحدة لفاتورة الطلب {order.order_number} باستخدام القالب الافتراضي (المحرر البسيط)")
...
def export_template(request, template_id):
    ...
    try:
        ...
        response['Content-Disposition'] = f'attachment; filename="{template.name}_template.json"'
