...
class OrderInventoryService:
    ...
    def deduct_stock_for_order(order, user=None):
        ...
        """
        from inventory.models import StockTransaction, Product, Warehouse
    ...
    def check_stock_availability(order):
        ...
        for item in order.items.all():
            ...
            current = OrderInventoryService._get_current_stock(product, default_warehouse)
    ...
    def _send_deduction_notification(order, deducted_items, user):
        ...
        try:
            items_text = ", ".join([f"{item['product']} ({item['quantity']})" for item in deducted_items[:3]])
