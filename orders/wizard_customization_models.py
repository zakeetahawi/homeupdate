...
class WizardFieldOption(models.Model):
    ...
    def clean(self):
        ...
        if existing.exists():
            raise ValidationError({
                'value': f'القيمة "{self.value}" موجودة بالفعل'
            })
    ...
    def get_active_options(cls, field_type):
        ...
        return cls.objects.filter(
            ...
            is_active=True
        ).order_by('sequence', 'display_name')
...
class WizardStepConfiguration(models.Model):
    ...
    step_title_ar = models.CharField(
        max_length=200,
        verbose_name='عنوان الخطوة (عربي)'
    )
