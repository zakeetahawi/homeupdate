    ...
    def get_order_statistics(date_range="month"):
        ...
        if stats_data is None:
            try:
                ...
                logger.debug(
                    f"تم تحديث إحصائيات الطلبات '{date_range}' في التخزين المؤقت"
                )
    ...
    def get_salesperson_data(salesperson_id):
        ...
        if salesperson_data is None:
            try:
                ...
                salesperson = Salesperson.objects.select_related("user", "branch").get(
                    id=salesperson_id
                )
                ...
                salesperson_data = {
                    ...
                    "branch_name": (
                        salesperson.branch.name if salesperson.branch else None
                    ),
                ...
                # تخزين في الكاش
                cache.set(
                    cache_key, salesperson_data, CACHE_TIMEOUTS["salesperson_data"]
                )
                logger.debug(
                    f"تم تحديث بيانات البائع {salesperson_id} في التخزين المؤقت"
                )
