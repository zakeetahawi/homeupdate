...
def clear_user_cache(sender, instance, **kwargs):
    ...
    clear_user_related_cache(
        instance.id, instance.branch_id if instance.branch else None
    )
...
def get_cached_customer_data(customer_id):
    ...
    if customer_data is None:
        ...
        try:
            customer = Customer.objects.select_related("branch", "category").get(
                id=customer_id
            )
            customer_data = {
                "id": customer.id,
...
def get_cached_user_customers(user):
    ...
    cache_key = (
        f'user_customers_{user.id}_{user.branch_id if user.branch else "no_branch"}'
    )
...
def get_cached_customer_inspections(customer_id, limit=10):
    ...
    if cached_inspections is None:
        ...
        inspections = (
            Inspection.objects.filter(customer_id=customer_id)
            .select_related("customer")
            .only("id", "contract_number", "created_at", "customer__name")
        ...
        cached_inspections = [
            {
                ...
                "customer_name": (
                    insp.customer.name if insp.customer else "عميل غير محدد"
                ),
            }
...
def warm_up_cache():
    ...
    active_customers = Customer.objects.filter(status="active").select_related(
        "branch"
    )[:100]
...
def get_cache_stats():
    ...
    try:
        ...
        return {"type": "Error", "message": f"خطأ في جلب الإحصائيات: {str(e)}"}
