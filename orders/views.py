...
from accounts.models import Branch, Department, Salesperson, SystemSettings
...
class OrdersDashboardView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["total_sales"] = (
            orders.filter(order_status="completed").aggregate(Sum("total_amount"))[
                "total_amount__sum"
            ]
            or 0
        )
        context["monthly_sales"] = (
            orders.filter(
                order_status="completed", created_at__month=today.month
...
def order_list(request):
    ...
    if show_branch_filter and branch_filter:
        orders = Order.objects.select_related("customer", "salesperson").filter(
            branch__id=branch_filter
        )
    else:
                    ...
                    return JsonResponse(
                        {
                            ...
                            + (
                                f"?show_print=1&paid_amount={paid_amount}"
                                if paid_amount > 0
                                else ""
                            ),
                        }
...
def order_update_by_number(request, order_number):
            ...
            for i, form_errors in enumerate(formset.errors):
                if form_errors:
                    for field, errors in form_errors.items():
                        messages.error(
                            request,
                            f"خطأ في العنصر {i+1} - {field}: {', '.join(errors)}",
                        )
        ...
        if form.is_valid() and formset.is_valid():
            ...
            for field in form.changed_data:
                if field in form.fields:
                    ...
                    if field == "customer":
                        ...
                        changes.append(
                            f"تم تغيير العميل من '{old_name}' إلى '{new_name}'"
                        )
                    elif field == "branch":
                        old_name = old_value.name if old_value else "غير محدد"
                        new_name = new_value.name if new_value else "غير محدد"
                        changes.append(
                            f"تم تغيير الفرع من '{old_name}' إلى '{new_name}'"
                        )
                    elif field == "salesperson":
                        old_name = old_value.name if old_value else "غير محدد"
                        new_name = new_value.name if new_value else "غير محدد"
                        changes.append(
                            f"تم تغيير البائع من '{old_name}' إلى '{new_name}'"
                        )
                    elif field == "paid_amount":
                        changes.append(
                            f"تم تغيير المبلغ المدفوع من {old_value} ج.م إلى {new_value} ج.م"
                        )
                    elif field == "notes":
                        old_notes = old_value or "بدون ملاحظات"
                        new_notes = new_value or "بدون ملاحظات"
                        changes.append(
                            f"تم تغيير ملاحظات الطلب من '{old_notes}' إلى '{new_notes}'"
                        )
                    else:
                        changes.append(
                            f"تم تغيير {field_label} من '{old_value}' إلى '{new_value}'"
                        )
            ...
            for form_item in formset:
                if form_item.cleaned_data.get("DELETE"):
                    if form_item.instance.pk:
                        product_name = (
                            form_item.instance.product.name
                            if form_item.instance.product
                            else "غير محدد"
                        )
                        quantity = form_item.instance.quantity
                        price = form_item.instance.unit_price
                        deleted_items.append(
                            f"حذف الصنف: {product_name} (الكمية: {quantity}, السعر: {price} ج.م)"
                        )
                elif form_item.instance.pk:
                    # عنصر موجود تم تعديله
                    if (
                        form_item.changed_data
                        and form_item.instance.pk in old_items_data
                    ):
                        item_changes = []
                        ...
                        if "product" in form_item.changed_data:
                            old_product_name = (
                                old_data["product"].name
                                if old_data["product"]
                                else "غير محدد"
                            )
                            new_product_name = (
                                form_item.cleaned_data["product"].name
                                if form_item.cleaned_data.get("product")
                                else "غير محدد"
                            )
                        else:
                        ...
                        for field in form_item.changed_data:
                            if field == "product":
                                ...
                                old_name = (
                                    old_product.name if old_product else "غير محدد"
                                )
                                new_name = (
                                    new_product.name if new_product else "غير محدد"
                                )
                                item_changes.append(
                                    f"تبديل نوع الصنف من '{old_name}' إلى '{new_name}'"
                                )
                            elif field == "quantity":
                                ...
                                if "product" in form_item.changed_data:
                                    item_changes.append(
                                        f"تعديل كمية الصنف من {old_value} إلى {new_value} (المنتج: {old_product_name} → {new_product_name})"
                                    )
                                else:
                                    item_changes.append(
                                        f"تعديل كمية الصنف '{current_product_name}' من {old_value} إلى {new_value}"
                                    )
                                ...
                                if "product" in form_item.changed_data:
                                    item_changes.append(
                                        f"تعديل سعر الصنف من {old_value} ج.م إلى {new_value} ج.م (المنتج: {old_product_name} → {new_product_name})"
                                    )
                                else:
                                    item_changes.append(
                                        f"تعديل سعر الصنف '{current_product_name}' من {old_value} ج.م إلى {new_value} ج.م"
                                    )
                            elif field == "notes":
                                old_value = old_data["notes"] or "بدون ملاحظات"
                                new_value = (
                                    form_item.cleaned_data[field] or "بدون ملاحظات"
                                )
                                # استخدام الاسم المناسب حسب ما إذا تم تغيير المنتج أم لا
                                if "product" in form_item.changed_data:
                                    item_changes.append(
                                        f"تعديل ملاحظات الصنف من '{old_value}' إلى '{new_value}' (المنتج: {old_product_name} → {new_product_name})"
                                    )
                                else:
                                    item_changes.append(
                                        f"تعديل ملاحظات الصنف '{current_product_name}' من '{old_value}' إلى '{new_value}'"
                                    )
                    ...
                    if form_item.cleaned_data and not form_item.cleaned_data.get(
                        ...
                        product_name = (
                            form_item.cleaned_data.get("product").name
                            if form_item.cleaned_data.get("product")
                            else "غير محدد"
                        )
                        ...
                        notes = (
                            form_item.cleaned_data.get("notes", "") or "بدون ملاحظات"
                        )
                        added_items.append(
                            f"إضافة صنف جديد: {product_name} (الكمية: {quantity}, السعر: {price} ج.م, ملاحظات: {notes})"
                        )
            ...
            if changes or deleted_items or modified_items or added_items:
                ...
                content_parts.append(
                    f"تم تعديل الطلب بواسطة {request.user.get_full_name() or request.user.username}"
                )
                ...
                content_parts.append(
                    f"💰 إجمالي المبلغ بعد التعديل: {updated_order.total_amount} ج.م"
                )
                content_parts.append(
                    f"💳 المبلغ المدفوع: {updated_order.paid_amount} ج.م"
                )
                content_parts.append(
                    f"📊 المديونية: {updated_order.remaining_amount} ج.م"
                )
            ...
            messages.success(
                request, f"تم تحديث الطلب {order.order_number} وعناصره بنجاح."
            )
            return redirect("orders:order_detail_by_number", order_number=order_number)
        else:
            # إذا كان هناك أخطاء في النموذج
            messages.error(
                request, "حدث خطأ في حفظ التعديلات. يرجى مراجعة البيانات المدخلة."
            )
    else:
    ...
    return render(
        request,
        "orders/order_edit_form.html",
        {"form": form, "formset": formset, "order": order, "is_update": True},
    )
