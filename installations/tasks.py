...
def sync_customer_debts_task(self):
    ...
    try:
        ...
        logger.info(f"✅ تمت مزامنة المديونيات - {stats['total_debts']} مديونية بمبلغ {stats['total_amount']:.2f} ج.م")
        ...
        return {
            'status': 'error',
            'message': f'خطأ في مزامنة المديونيات: {str(e)}'
        }
...
def update_overdue_debts_task(self):
    ...
    try:
        ...
        logger.info(f"⚠️ تم العثور على {overdue_count} مديونية متأخرة بمبلغ {total_overdue_amount:.2f} ج.م")
...
def generate_debt_report_task(self):
    ...
    try:
        ...
        logger.info(f"📈 تقرير المديونيات: {total_debts} مديونية، {total_amount:.2f} ج.م، {overdue_count} متأخرة")
