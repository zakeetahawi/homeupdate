...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.display_results(
            before_stats, after_stats, new_debts, updated_debts, paid_debts
        )
    ...
    def sync_new_debts(self, dry_run, verbose):
        ...
        for order in debt_orders:
            ...
            if verbose:
                self.stdout.write(
                    f"📝 إنشاء مديونية جديدة للطلب {order.order_number} - {debt_amount:.2f} ج.م"
                )
    ...
    def update_existing_debts(self, dry_run, verbose):
        ...
        existing_debts = CustomerDebt.objects.filter(is_paid=False).select_related(
            "order"
        )
        ...
        for debt in existing_debts:
            ...
            if abs(current_debt - old_debt) > 0.01:  # تغيير في المبلغ
                ...
                if not dry_run:
                    ...
                    debt.notes += (
                        f' - تحديث تلقائي {timezone.now().strftime("%Y-%m-%d %H:%M")}'
                    )
                    debt.save()
    ...
    def mark_paid_debts(self, dry_run, verbose):
        ...
        for debt in paid_orders_debts:
            if verbose:
                self.stdout.write(
                    f"💰 تحديث مديونية الطلب {debt.order.order_number} إلى مدفوعة"
                )
    ...
    def display_results(
        self, before_stats, after_stats, new_debts, updated_debts, paid_debts
    ):
        """عرض نتائج المزامنة"""
        self.stdout.write("\n" + "=" * 60)
        ...
        self.stdout.write("\n📈 الإحصائيات:")
        self.stdout.write(
            f'قبل المزامنة: {before_stats["total_debts"]} مديونية بمبلغ {before_stats["total_amount"]:.2f} ج.م'
        )
        self.stdout.write(
            f'بعد المزامنة: {after_stats["total_debts"]} مديونية بمبلغ {after_stats["total_amount"]:.2f} ج.م'
        )
