    ...
    def queryset(self, request, queryset):
        ...
        if self.value() == "current":
            return queryset.filter(
                is_paid=False, created_at__gte=now - timezone.timedelta(days=30)
            )
            ...
            return queryset.filter(
                is_paid=False, created_at__lt=now - timezone.timedelta(days=90)
            )
        return queryset
...
class SalespersonFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        ...
        salespersons = (
            Salesperson.objects.filter(orders__isnull=False).distinct().order_by("name")
        )

        return [(salesperson.id, salesperson.name) for salesperson in salespersons]
