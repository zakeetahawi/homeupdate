...
def update_related_orders(installation, new_status, old_status, user=None):
    ...
    try:
        ...
        try:
            ...
            new_manufacturing_status = installation_to_manufacturing_mapping.get(
                new_status
            )
...
def dashboard(request):
    ...
    context = {
        ...
        "orders_in_manufacturing": stats.get(
            "orders_in_manufacturing", 0
        ),  # طلبات تحت التصنيع
        "delivered_manufacturing_orders": stats.get(
            "delivered_manufacturing_orders", 0
        ),  # تم التسليم
        # الجداول
...
def change_installation_status(request, installation_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            from .models import InstallationEventLog, InstallationStatusLog
    ...
    if request.headers.get("X-Requested-With") == "XMLHttpRequest":
        ...
        html = render_to_string(
            "installations/change_status_modal.html", context, request=request
        )
...
def installation_list(request):
    ...
    if not status_filter or status_filter in [
        ...
        scheduled_query, monthly_filter_context = apply_monthly_filter(
            scheduled_query, request, "order__order_date"
        )
        ...
        if date_from:
            try:
                ...
                scheduled_query = scheduled_query.filter(
                    scheduled_date__gte=date_from_obj
                )
            except ValueError:
                pass
        if date_to:
            try:
                ...
                scheduled_query = scheduled_query.filter(
                    scheduled_date__lte=date_to_obj
                )
            except ValueError:
                pass
        if search:
    ...
    if not status_filter or status_filter == "needs_scheduling":
        ...
        if branch_filter:
            ready_manufacturing_query = ready_manufacturing_query.filter(
...
def installation_detail(request, installation_id):
    ...
    event_logs = (
        InstallationEventLog.objects.filter(installation=installation)
        .select_related("user")
    ...
    status_logs = (
        InstallationStatusLog.objects.filter(installation=installation)
        ...
        try:
            ...
            if inspection_orders.exists():
                ...
                if latest_inspection.notes:
                    ...
                    hasattr(latest_inspection, "windows_count")
                    and latest_inspection.windows_count
                ):
...
def update_status(request, installation_id):
    ...
    try:
        ...
        if request.method == "POST":
            ...
            if not new_status:
                return JsonResponse(
                    {"success": False, "error": "لم يتم تحديد الحالة الجديدة"}
                )
            ...
            if new_status not in valid_statuses:
                return JsonResponse(
                    {"success": False, "error": f"الحالة غير صحيحة: {new_status}"}
                )
        ...
        return JsonResponse({"success": False, "error": "التركيب غير موجود"})
    except Exception as e:
        return JsonResponse(
            {"success": False, "error": f"حدث خطأ أثناء تحديث الحالة: {str(e)}"}
        )
...
def update_status_by_code(request, installation_code):
    ...
    try:
        ...
        installation = get_object_or_404(
            InstallationSchedule,
            order__order_number=installation_code.replace("-T", ""),
        )
...
def orders_modal(request):
    ...
    if order_type == "total":
        ...
        for mfg in manufacturing_orders:
            mfg.order._cached_manufacturing_order = (
                mfg  # ربط أمر التصنيع بالطلب لسهولة العرض في القالب
            )
        ...
        modifications = (
            ...
            .select_related(
                ...
                "installation__order__branch",
                "installation__order__salesperson",
            )
            .order_by("-created_at")
        )
        ...
        manufacturing_orders = ManufacturingOrder.objects.filter(
            status="delivered", order__selected_types__icontains="installation"
        ).select_related(
            "order", "order__customer", "order__branch", "order__salesperson"
        )
...
def schedule_from_needs_scheduling(request, installation_id):
    ...
    if request.method == "POST":
        ...
        form = InstallationScheduleForm(
            ...
            initial={"scheduled_date": tomorrow, "scheduled_time": default_time},
        )
...
def manufacturing_orders_list(request):
    ...
    context = {"page_obj": page_obj, "title": "أوامر التصنيع للتعديلات"}
...
def quick_schedule_installation(request, order_id):
    ...
    try:
        ...
        if manufacturing_order and manufacturing_order.status in [
            "ready_install",
            "delivered",
        ]:
            is_ready_for_installation = True
    ...
    if not is_ready_for_installation:
        messages.error(
            request,
            'لا يمكن جدولة التركيب. الطلب ليس جاهزاً للتركيب بعد. يجب أن يكون الطلب في حالة "جاهز للتركيب" أو "تم التسليم" في المصنع.',
        )
        return redirect("installations:dashboard")
            ...
            if update_customer_address and order.customer:
                try:
                    ...
                    if form.cleaned_data.get("location_address"):
                        installation.location_address = form.cleaned_data[
                            "location_address"
                        ]
                    else:
                    ...
                    try:
                        ...
                        for inspection in inspections:
                            ...
                            inspection_note += (
                                f"العنوان الجديد: {update_customer_address}\n"
                            )
                            inspection_note += (
                                f"تم التحديث بواسطة: {request.user.username}\n"
                            )
...
def daily_schedule(request):
    ...
    if form.is_valid():
        ...
        # فلترة التركيبات
        installations = (
            InstallationSchedule.objects.filter(scheduled_date=date)
            .select_related(
                "order",
                "order__customer",
                "order__salesperson",
...
def installation_stats_api(request):
    ...
    try:
        ...
        stats = installation_schedules_queryset.aggregate(
            ...
            ),  # هذا سيكون دائماً 0 لأننا استثنينا المكتملة
        ...
        try:
            teams_stats_count = (
                InstallationTeam.objects.filter(is_active=True)
                .annotate(installations_count=Count("installationschedule"))
                .count()
            )
        except Exception as e:
            teams_stats_count = 0
        ...
        try:
            total_installations_all = (
                stats["total"] + completed_count
            )  # المجدول + المكتمل
            completion_rate = (
                (completed_count / total_installations_all * 100)
                if total_installations_all > 0
                else 0
            )
        except Exception as e:
            completion_rate = 0
        ...
        return JsonResponse(
            {
                "total_scheduled": stats[
                    "total"
                ],  # إجمالي التركيبات المجدولة غير المكتملة
...
def manage_customer_debt(request, order_id):
    ...
    debt, created = CustomerDebt.objects.get_or_create(
        order=order,
        defaults={"customer": order.customer, "debt_amount": order.remaining_amount},
    )
            ...
            if debt.is_paid and (not was_paid_before or not original_payment_date):
                ...
                try:
                    ...
                    payment = Payment.objects.create(
                        ...
                        payment_method="cash",
                        reference_number=debt.payment_receipt_number
                        or "",  # رقم الإيصال المُدخل
                        notes=payment_notes,
                    ...
                    messages.success(
                        request,
                        f"✅ تم تسجيل الدفعة بمبلغ {debt.debt_amount} وتحديث الطلب بنجاح",
                    )
                except Exception as e:
                    import traceback
                    ...
                    return redirect(
                        "installations:manage_customer_debt", order_id=order.id
                    )
...
def pay_debt(request, debt_id):
    ...
    context = {"form": form, "debt": debt, "title": "دفع المديونية"}
...
def debt_list(request):
    ...
    debts = CustomerDebt.objects.select_related("customer", "order").order_by(
        "-created_at"
    )
...
def schedule_manufacturing_order(request, manufacturing_order_id):
    ...
    try:
        ...
        if manufacturing_order.status != "ready_install":
            return JsonResponse(
                {"success": False, "message": "أمر التصنيع ليس جاهزاً للتركيب"}
            )

        # التحقق من عدم وجود جدولة تركيب موجودة
        if InstallationSchedule.objects.filter(
        ...
        return JsonResponse(
            {"success": False, "message": f"حدث خطأ أثناء إنشاء الجدولة: {str(e)}"}
        )
...
def installation_analytics(request):
    ...
    # إحصائيات حسب الح��لة
    status_stats = (
        monthly_installations.values("status")
    ...
    # إحصائيات حسب الفريق
    team_stats = (
        monthly_installations.filter(team__isnull=False)
        .values("team__name")
        .annotate(count=Count("id"))
        .order_by("-count")
    )
    ...
    for day in range(1, calendar.monthrange(year, month)[1] + 1):
        ...
        daily_stats.append({"date": day_date, "count": day_installations})
    ...
    completed_installations = monthly_installations.filter(
        status="completed", completion_date__isnull=False
    )
    ...
    if completed_installations.exists():
        ...
        for installation in completed_installations:
            if installation.scheduled_date and installation.completion_date:
                days_diff = (
                    installation.completion_date.date() - installation.scheduled_date
                ).days
...
def installation_event_logs(request):
    ...
    event_logs = (
        InstallationEventLog.objects.select_related(
            "installation",
            "installation__order",
            "installation__order__customer",
            "user",
        )
        .prefetch_related("installation__order__customer")
        .order_by("-created_at")
    )
...
def view_scheduling_details(request, installation_id):
    ...
    schedule_events = InstallationEventLog.objects.filter(
        installation=installation, event_type="schedule_change"
    ).order_by("-created_at")

    # الحصول على التركيبات المجدولة في نفس اليوم
    same_day_installations = (
        InstallationSchedule.objects.filter(scheduled_date=installation.scheduled_date)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if old_settings["scheduled_date"] != installation.scheduled_date:
                changes.append(
                    f"التاريخ: {old_settings['scheduled_date']} → {installation.scheduled_date}"
                )
            if old_settings["scheduled_time"] != installation.scheduled_time:
                changes.append(
                    f"الوقت: {old_settings['scheduled_time']} → {installation.scheduled_time}"
                )
            if old_settings["team"] != installation.team:
                old_team = (
                    old_settings["team"].name if old_settings["team"] else "غير محدد"
                )
            ...
            if changes:
                InstallationEventLog.objects.create(
                    ...
                    user=request.user,
                    metadata={"old_settings": old_settings, "changes": changes},
                )
    ...
    if request.method == "POST":
        try:
            ...
            if installation.scheduled_date:
                ...
                messages.success(
                    request,
                    f"تم تحديث تاريخ التركيب بنجاح إلى {installation.scheduled_date}",
                )
                return JsonResponse(
                    {
                        "success": True,
                ...
                return JsonResponse(
                    {"success": False, "error": "لا يوجد تاريخ مجدول للتركيب"}
                )

        except Exception as e:
            return JsonResponse(
                {"success": False, "error": f"حدث خطأ أثناء تحديث التاريخ: {str(e)}"}
            )
...
def delete_installation(request, installation_id):
    ...
    return render(
        request,
        "installations/installation_confirm_delete.html",
        {"installation": installation},
    )
...
def debt_orders_list(request):
    ...
    unpaid_debts_subquery = CustomerDebt.objects.filter(
        order=OuterRef("pk"), is_paid=False
    )

    debt_orders_query = (
        Order.objects.filter(Exists(unpaid_debts_subquery))
        .select_related("customer", "salesperson", "branch")
        .annotate(calculated_debt=F("total_amount") - F("paid_amount"))
    )
    ...
    # حساب الإجماليات
    total_debt = (
        debt_orders_query.aggregate(total=models.Sum("calculated_debt"))["total"] or 0
    )

    # إحصائيات حسب الحالة
    status_stats = (
        debt_orders_query.values("order_status")
        .annotate(count=Count("id"), debt_sum=models.Sum("calculated_debt"))
        .order_by("-debt_sum")
    )
    ...
    context = {
        ...
        "has_filters": bool(
            order_status_filter or search or year_filter or month_filter
        ),
...
def delete_technician(request, technician_id):
    ...
    if request.method == "POST":
        ...
        if installation_teams.exists():
            ...
            messages.error(
                request,
                f'لا يمكن حذف الفني "{technician.name}" لأنه مرتبط بالفرق التالية: {teams_names}. يجب إزالته من الفرق أولاً.',
            )
            return redirect("installations:team_management")
...
def installation_detail_redirect(request, installation_id):
    ...
    installation = get_object_or_404(InstallationSchedule, id=installation_id)
    return redirect(
        "installations:installation_detail_by_code",
...
def daily_schedule(request):
    ...
    ).filter(scheduled_date=selected_date)
    ...
    if salesperson_filter:
        installations_query = installations_query.filter(
            order__salesperson_id=salesperson_filter
        )
...
def upcoming_installations_schedule(request):
    ...
    return render(
        request, "installations/upcoming_installations_schedule.html", context
    )
...
def api_upcoming_installations(request):
    ...
    installations = (
        InstallationSchedule.objects.filter(scheduled_date=selected_date)
    ...
    for inst in installations:
        order = inst.order
        debt_amount = (
            order.debt_amount if hasattr(order, "debt_amount") else Decimal("0")
        )
        paid_amount = (
            order.paid_amount if hasattr(order, "paid_amount") else Decimal("0")
        )
        total_amount = (
            order.total_amount if hasattr(order, "total_amount") else Decimal("0")
        )
        ...
        installations_data.append(
            {
                ...
                "customer_name": order.customer.name,
                "customer_phone": (
                    order.customer.phone if hasattr(order.customer, "phone") else ""
                ),
                "location_address": inst.location_address or "",
                "scheduled_time": (
                    inst.scheduled_time.strftime("%H:%M") if inst.scheduled_time else ""
                ),
                "invoice_number": (
                    order.invoice_number if hasattr(order, "invoice_number") else ""
                ),
                "contract_number": (
                    order.contract_number if hasattr(order, "contract_number") else ""
                ),
                "total_amount": float(total_amount),
                "paid_amount": float(paid_amount),
                "debt_amount": float(debt_amount),
                "salesperson_name": (
                    order.salesperson.get_display_name() if order.salesperson else ""
                ),
...
def api_upcoming_installations_filters(request):
    ...
    for sp in salespersons:
        salespersons_list.append({"id": sp.id, "name": sp.get_display_name()})
