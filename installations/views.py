...
def debt_orders_list(request):
    ...
    # حساب الإجماليات
    total_debt = (
        debt_orders_query.aggregate(total=models.Sum("debt_amount"))["total"] or 0
    )

    # إحصائيات حسب الحالة
    status_stats = (
        debt_orders_query.values("order_status")
        .annotate(count=Count("id"), debt_sum=models.Sum("debt_amount"))
        .order_by("-debt_sum")
    )
