...
from ..models import InstallationArchive, InstallationSchedule, InstallationTeam
...
class InstallationService:
    ...
    def get_dashboard_stats():
        ...
        return {
            ...
            "scheduled": InstallationSchedule.objects.filter(
                status="scheduled"
            ).count(),
            "in_progress": InstallationSchedule.objects.filter(
                status="in_progress"
            ).count(),
            "completed": InstallationSchedule.objects.filter(
                status="completed"
            ).count(),
    ...
    def get_today_installations():
        ...
        return (
            InstallationSchedule.objects.filter(
                scheduled_date=today, status__in=["scheduled", "in_progress"]
            )
    ...
    def get_upcoming_installations(limit=10):
        ...
        return (
            ...
            .select_related("order", "order__customer", "team")
            .order_by("scheduled_date", "scheduled_time")[:limit]
        )
    ...
    def schedule_installation(
        installation_id, team_id, scheduled_date, scheduled_time, notes=None
    ):
        """جدولة تركيب"""
        try:
    ...
    def update_installation_status(installation_id, new_status):
        ...
        try:
            ...
            if new_status == "completed":
                InstallationArchive.objects.get_or_create(installation=installation)
    ...
    def get_daily_schedule(date, team_id=None):
        ...
        installations = InstallationSchedule.objects.filter(
            scheduled_date=date, status__in=["scheduled", "in_progress"]
        ).select_related("order", "order__customer", "team")
