...
def check_secret_key():
    ...
    if len(key) >= 50:
        ...
        print_check(False, f"SECRET_KEY length: {len(key)} chars (Too short - should be 50+)")
    ...
    if unique_chars < 20:
        print_check(False, f"SECRET_KEY has only {unique_chars} unique characters (Too simple)")
    else:
...
def check_debug():
    ...
    if settings.DEBUG:
        ...
        if os.environ.get('DEVELOPMENT_MODE'):
            ...
            print(f"{Fore.RED}   ⚠️  DEVELOPMENT_MODE not set - This might be production!")
    else:
