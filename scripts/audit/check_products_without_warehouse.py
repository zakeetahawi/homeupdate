...
def check_products_without_warehouse():
    ...
    for item in recent_order_items:
        ...
        if not has_stock:
            products_without_warehouse.append(
                {"product": item.product, "order": item.order, "order_item": item}
            )
    ...
    if products_without_warehouse:
        ...
        for idx, data in enumerate(products_without_warehouse, 1):
            ...
            print(
                f"   👤 العميل: {order.customer.name if order.customer else 'غير محدد'}"
            )
            ...
            cutting_items = CuttingOrderItem.objects.filter(
                order_item__product=product, cutting_order__order=order
            ).select_related("cutting_order__warehouse")

            if cutting_items.exists():
                for ci in cutting_items:
                    print(
                        f"   🏪 أمر التقطيع: {ci.cutting_order.cutting_code} - مستودع {ci.cutting_order.warehouse.name}"
                    )
            else:
        ...
        for item in co.items.all():
            ...
            last_transaction = (
                StockTransaction.objects.filter(
                    product=item.order_item.product, warehouse=co.warehouse
                )
