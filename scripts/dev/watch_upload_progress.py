        ...
        for log in new_logs:
            ...
            while log.status == "processing":
                ...
                if log.total_rows > 0:
                    ...
                    print(
                        f"\r   التقدم: {log.processed_count}/{log.total_rows} ({progress:.1f}%) | جديد:{log.created_count} محدث:{log.updated_count} أخطاء:{log.error_count}",
                        end="",
                        flush=True,
                    )
                else:
                    print(
                        f"\r   التقدم: {log.processed_count} صف معالج...",
                        end="",
                        flush=True,
                    )
