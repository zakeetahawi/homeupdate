...
def optimize_draft_orders():
    ...
    try:
        ...
        old_drafts = DraftOrder.objects.filter(
            is_completed=False, updated_at__lt=old_date
        )
...
def create_performance_cache():
    ...
    try:
        ...
        # Cache الفروع النشطة
        branches = list(
            Branch.objects.filter(is_active=True).values("id", "name", "code")
        )
        ...
        try:
            ...
            print_status(
                f"تم تخزين {len(categories)} تصنيف في الذاكرة المؤقتة", "success"
            )
        except Exception as e:
