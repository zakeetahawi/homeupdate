...
class CuttingOrderItemInline(admin.TabularInline):
    ...
    fields = (
        'order_item', 'status', 'cutter_name', 'permit_number', 
        'receiver_name', 'cutting_date', 'delivery_date', 'bag_number',
        'additional_quantity', 'notes', 'rejection_reason'
    )
    
    def get_readonly_fields(self, request, obj=None):
        if obj and obj.status == 'completed':
            return self.readonly_fields + ('status', 'cutter_name', 'permit_number', 'receiver_name')
...
class CuttingOrderAdmin(admin.ModelAdmin):
    ...
    list_filter = [
        'status', 'warehouse', 'created_at', 'assigned_to'
    ]
    search_fields = [
        'cutting_code', 'order__contract_number', 'order__customer__name',
        'order__customer__phone', 'notes'
    ]
    ...
    def order_link(self, obj):
        ...
        if obj.order:
            ...
            return format_html('<a href="{}">{}</a>', url, obj.order.contract_number or f'طلب #{obj.order.pk}')
        return '-'
    ...
    def completion_progress(self, obj):
        ...
        return format_html(
            ...
            obj.completed_items, obj.total_items
        )
...
class CuttingOrderItemAdmin(admin.ModelAdmin):
    list_display = [
        ...
        'cutting_date', 'updated_by'
    ]
    list_filter = [
        'status', 'cutting_order__warehouse', 'cutting_date', 'updated_by'
    ]
    search_fields = [
        'cutting_order__cutting_code', 'order_item__product__name',
        'cutter_name', 'permit_number', 'receiver_name', 'notes'
    ]
    readonly_fields = ['order_item', 'updated_at', 'updated_by']
    
    fieldsets = (
        ('معلومات أساسية', {
            'fields': ('cutting_order', 'order_item', 'status')
        }),
        ('بيانات التقطيع', {
            'fields': ('cutter_name', 'permit_number', 'receiver_name')
        }),
        ('التوقيتات', {
            'fields': ('cutting_date', 'delivery_date')
        }),
        ('معلومات إضافية', {
            'fields': ('bag_number', 'additional_quantity', 'notes')
        }),
        ('الرفض', {
...
class CuttingReportAdmin(admin.ModelAdmin):
    ...
    list_filter = [
        'report_type', 'warehouse', 'generated_at', 'generated_by'
    ]
    search_fields = ['warehouse__name']
    readonly_fields = [
        'total_orders', 'completed_items', 'rejected_items', 
        'pending_items', 'generated_at', 'generated_by'
    ]
