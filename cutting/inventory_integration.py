...
from inventory.models import StockTransaction, Product, Warehouse as InventoryWarehouse
...
class InventoryIntegrationService:
    ...
    def process_cutting_completion(cutting_item, user):
        ...
        try:
            with transaction.atomic():
                ...
                required_quantity = cutting_item.order_item.quantity + cutting_item.additional_quantity
                
                # الحصول على المستودع المناسب
                warehouse = InventoryIntegrationService._get_warehouse_for_cutting(cutting_item)
                
                if not warehouse:
                    raise ValueError("لا يمكن تحديد المستودع المناسب للخصم")
                
                # التحقق من توفر الكمية
                current_stock = InventoryIntegrationService._get_current_stock(product, warehouse)
    ...
    def reverse_cutting_deduction(cutting_item, user, reason="إلغاء التقطيع"):
        ...
        try:
            with transaction.atomic():
                if not hasattr(cutting_item, 'inventory_transaction') or not cutting_item.inventory_transaction:
                    logger.warning(f"لا توجد معاملة مخزون لعكسها للعنصر {cutting_item.id}")
                    return None
    ...
    def _update_running_balance(transaction):
        ...
        try:
            # الحصول على آخر رصيد
            previous_transaction = StockTransaction.objects.filter(
                product=transaction.product,
                warehouse=transaction.warehouse,
                transaction_date__lt=transaction.transaction_date
            ).order_by('-transaction_date', '-id').first()
            
            previous_balance = previous_transaction.running_balance if previous_transaction else Decimal('0')
    ...
    @staticmethod
    def _send_stock_shortage_notification(cutting_item, current_stock, required_quantity, user):
        """إرسال إشعار نقص المخزون"""
        try:
...
def reverse_inventory_deduction(cutting_item, user, reason="إلغاء التقطيع"):
    ...
    return InventoryIntegrationService.reverse_cutting_deduction(cutting_item, user, reason)
