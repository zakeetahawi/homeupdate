...
class CuttingOrderForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            "order": forms.Select(
                attrs={"class": "form-select", "data-placeholder": "اختر الطلب"}
            ),
            "warehouse": forms.Select(
                attrs={"class": "form-select", "data-placeholder": "اختر المستودع"}
            ),
            "status": forms.Select(attrs={"class": "form-select"}),
            "assigned_to": forms.Select(
                attrs={"class": "form-select", "data-placeholder": "اختر المسؤول"}
            ),
...
class CuttingItemForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            "status": forms.Select(attrs={"class": "form-select"}),
            "cutter_name": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "اسم القصاص"}
            ),
            "permit_number": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "رقم الإذن"}
            ),
            "receiver_name": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "اسم المستلم"}
            ),
            "bag_number": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "رقم الشنطة"}
            ),
            "additional_quantity": forms.NumberInput(
                attrs={
                    "class": "form-control",
                    "step": "0.001",
                    "min": "0",
...
class BulkUpdateForm(forms.Form):
    ...
    bag_number = forms.CharField(
        ...
        required=False,
        widget=forms.TextInput(
            attrs={"class": "form-control", "placeholder": "رقم الشنطة (اختياري)"}
        ),
        label="رقم الشنطة",
    )
