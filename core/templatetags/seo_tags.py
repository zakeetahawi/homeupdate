...
def get_meta_description(context, custom_description=None):
    ...
    elif resolver_match and ('dashboard' in resolver_match.url_name or 'home' in resolver_match.url_name):
...
def _get_list_description(context):
    ...
    if page_obj and hasattr(page_obj, 'paginator'):
        ...
        try:
            count = len(object_list) if hasattr(object_list, '__len__') else object_list.count()
    ...
    elif page_obj and hasattr(page_obj, 'object_list') and hasattr(page_obj.object_list, 'model'):
