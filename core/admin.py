...
class AuditLogAdmin(admin.ModelAdmin):
    ...
    """
    list_display = [
        'timestamp', 'severity_badge', 'action_badge', 'user_link',
        'model_name', 'description_short', 'ip_address'
    ]
    list_filter = [
        'severity', 'action', 'model_name',
    ...
    readonly_fields = [
        'user', 'action', 'severity', 'model_name', 'object_id',
        'description', 'old_value', 'new_value', 'ip_address',
        'user_agent', 'timestamp', 'session_key'
    ]
...
class SecurityEventAdmin(admin.ModelAdmin):
    ...
    """
    list_display = [
        'timestamp', 'event_badge', 'ip_address', 'user_link',
        'url_short', 'method', 'blocked_badge'
    ]
    list_filter = [
        'event_type', 'blocked',
        ('timestamp', admin.DateFieldListFilter),
    ...
    search_fields = [
        'ip_address', 'user__username', 'url', 'user_agent'
    ]
    date_hierarchy = 'timestamp'
    ordering = ['-timestamp']
    list_per_page = 50
    ...
    fieldsets = (
        ('معلومات الحدث', {
            'fields': ('event_type', 'blocked', 'timestamp')
        }),
        ('معلومات المستخدم', {
            'fields': ('user', 'ip_address', 'user_agent'),
        }),
        ('تفاصيل الطلب', {
            'fields': ('url', 'method', 'details'),
            'classes': ('collapse',)
        }),
    )
    
    readonly_fields = [
        'event_type', 'ip_address', 'user', 'details',
        'user_agent', 'url', 'method', 'blocked', 'timestamp'
    ]
