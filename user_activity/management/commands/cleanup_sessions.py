...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--all-inactive", action="store_true", help="حذف جميع الجلسات غير النشطة"
        )
        parser.add_argument(
            "--django-sessions", action="store_true", help="حذف جلسات Django المنتهية"
        )
        parser.add_argument(
            "--dry-run", action="store_true", help="عرض ما سيتم حذفه دون تنفيذ الحذف"
        )
    ...
    def handle(self, *args, **options):
        ...
        if options["django_sessions"]:
            ...
            if count > 0:
                ...
                if not dry_run:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(f"✅ تم حذف {deleted:,} جلسة Django")
                    )
                    total_deleted += deleted
                else:
                    self.stdout.write(
                        self.style.WARNING(f"🔍 سيتم حذف {count:,} جلسة Django")
                    )
            else:
            ...
            if count > 0:
                ...
                if not dry_run:
                    ...
                    with connection.cursor() as cursor:
                        ...
                        deleted = cursor.rowcount
                    self.stdout.write(
                        self.style.SUCCESS(f"✅ تم حذف {deleted:,} جلسة غير نشطة")
                    )
                    total_deleted += deleted
                else:
                    self.stdout.write(
                        self.style.WARNING(f"🔍 سيتم حذف {count:,} جلسة غير نشطة")
                    )
            else:
        ...
        if count > 0:
            ...
            if not dry_run:
                ...
                with connection.cursor() as cursor:
                    ...
                    deleted = cursor.rowcount
                self.stdout.write(
                    self.style.SUCCESS(f"✅ تم حذف {deleted:,} جلسة قديمة")
                )
                total_deleted += deleted
            else:
                self.stdout.write(
                    self.style.WARNING(f"🔍 سيتم حذف {count:,} جلسة قديمة")
                )
        else:
            self.stdout.write(
                self.style.SUCCESS(f"✅ لا توجد جلسات أقدم من {hours} ساعة")
            )
        ...
        if dry_run:
            self.stdout.write(
                self.style.WARNING("🔍 وضع المحاكاة (Dry Run) - لم يتم حذف شيء")
            )
            self.stdout.write(
                self.style.WARNING("قم بإزالة --dry-run لتنفيذ الحذف فعلياً")
            )
        else:
            self.stdout.write(
                self.style.SUCCESS(f"✅ تم حذف {total_deleted:,} جلسة إجمالاً")
            )
