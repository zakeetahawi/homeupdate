...
class UserActivityLogAdmin(admin.ModelAdmin):
    ...
    actions = [
        ...
        "delete_old_logs_30_days",
        "delete_old_logs_60_days",
        "delete_old_logs_90_days",
    ]
    ...
    def bulk_delete_selected(self, request, queryset):
        ...
        if count > 0:
            ...
            self.message_user(
                request, f"تم حذف {count} سجل بنجاح بطريقة سريعة!", level="success"
            )
        else:
    ...
    def delete_old_logs_30_days(self, request, queryset):
        ...
        if count > 0:
            ...
            self.message_user(
                request, f"تم حذف {count} سجل أقدم من 30 يوم", level="success"
            )
        else:
    ...
    def delete_old_logs_60_days(self, request, queryset):
        ...
        if count > 0:
            old_logs._raw_delete(old_logs.db)
            self.message_user(
                request, f"تم حذف {count} سجل أقدم من 60 يوم", level="success"
            )
        else:
    ...
    def delete_old_logs_90_days(self, request, queryset):
        ...
        if count > 0:
            old_logs._raw_delete(old_logs.db)
            self.message_user(
                request, f"تم حذف {count} سجل أقدم من 90 يوم", level="success"
            )
        else:
...
class UserSessionAdmin(admin.ModelAdmin):
    ...
    list_filter = ["is_active", "device_type", "login_time", "last_activity"]
    search_fields = ["user__username", "user__email", "ip_address", "browser"]
    readonly_fields = ["session_key", "login_time", "last_activity", "duration_display"]
    ...
    def get_urls(self):
        ...
        custom_urls = [
            path(
                "quick-cleanup/",
                self.admin_site.admin_view(self.quick_cleanup_view),
                name="usersession_quick_cleanup",
            ),
        ]
    ...
    def quick_cleanup_view(self, request):
        ...
        if request.method == "POST":
            ...
            with connection.cursor() as cursor:
                if action == "inactive":
                    ...
                    cursor.execute(
                        "DELETE FROM user_activity_useractivitylog WHERE session_id IN (SELECT id FROM user_activity_usersession WHERE is_active = false)"
                    )
                    cursor.execute(
                        "DELETE FROM user_activity_usersession WHERE is_active = false"
                    )
                    ...
                    messages.success(
                        request,
                        f"✅ تم حذف {count} جلسة تتبع + {django_count} جلسة Django منتهية",
                    )
                    ...
                    cursor.execute(
                        "SELECT session_key FROM user_activity_usersession WHERE last_activity < %s",
                        [cutoff],
                    )
                    ...
                    cursor.execute(
                        "DELETE FROM user_activity_useractivitylog WHERE session_id IN (SELECT id FROM user_activity_usersession WHERE last_activity < %s)",
                        [cutoff],
                    )
                    cursor.execute(
                    ...
                    if session_keys:
                        ...
                        cursor.execute(
                            f"DELETE FROM django_session WHERE session_key IN ({placeholders})",
                            session_keys,
                        )
                        django_count = cursor.rowcount
                    else:
                    ...
                    messages.success(
                        request,
                        f"✅ تم حذف {count} جلسة تتبع + {django_count} جلسة Django (أقدم من يوم). تم إخراج المستخدمين!",
                    )
                    ...
                    messages.success(
                        request,
                        f"✅ تم حذف {count} جلسة تتبع + {django_count} جلسة Django (أقدم من أسبوع). تم إخراج المستخدمين!",
                    )
                    ...
                    messages.success(
                        request,
                        f"✅ تم حذف {count} جلسة تتبع + {django_count} جلسة Django (أقدم من شهر). تم إخراج المستخدمين!",
                    )

                elif action == "all_except_super":
                    # جمع session_keys للمستخدمين العاديين
                    cursor.execute(
                        "SELECT session_key FROM user_activity_usersession WHERE user_id IN (SELECT id FROM accounts_user WHERE is_superuser = false)"
                    )
                    session_keys = [row[0] for row in cursor.fetchall()]

                    cursor.execute(
                        "DELETE FROM user_activity_useractivitylog WHERE session_id IN (SELECT id FROM user_activity_usersession WHERE user_id IN (SELECT id FROM accounts_user WHERE is_superuser = false))"
                    )
                    cursor.execute(
                        "DELETE FROM user_activity_usersession WHERE user_id IN (SELECT id FROM accounts_user WHERE is_superuser = false)"
                    )
                    count = cursor.rowcount
                    ...
                    messages.warning(
                        request,
                        f"🔴 تم حذف {count} جلسة تتبع + {django_count} جلسة Django. تم إخراج جميع المستخدمين (ماعدا السوبر يوزر)!",
                    )
                    ...
                    cursor.execute(
                        "DELETE FROM user_activity_useractivitylog WHERE session_id IS NOT NULL"
                    )
                    ...
                    messages.error(
                        request,
                        f"🔴🔴 تم حذف {count} جلسة تتبع + {django_count} جلسة Django. تم إخراج الجميع!",
                    )
        ...
        with connection.cursor() as cursor:
            ...
            cursor.execute(
                "SELECT COUNT(*) FROM user_activity_usersession WHERE is_active = false"
            )
            inactive_sessions = cursor.fetchone()[0]

            cursor.execute(
                "SELECT COUNT(*) FROM user_activity_usersession WHERE last_activity < %s",
    ...
    delete_all_sessions_keep_superusers.short_description = (
        "🔴 حذف الجميع (ماعدا السوبر يوزر)"
    )
...
class UserLoginHistoryAdmin(admin.ModelAdmin):
    ...
    actions = [
        "bulk_delete_selected",
        "delete_old_history_30_days",
        "delete_old_history_60_days",
    ]
    ...
    def bulk_delete_selected(self, request, queryset):
        ...
        if count > 0:
            queryset._raw_delete(queryset.db)
            self.message_user(
                request, f"تم حذف {count} سجل تسجيل دخول", level="success"
            )
        else:
            self.message_user(request, "لم يتم تحديد أي سجلات للحذف", level="warning")
