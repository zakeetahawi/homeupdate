...
def analyze_cutting_order_items():
    ...
    for cutting_order in CuttingOrder.objects.select_related(
        "warehouse", "order"
            ...
            issues.append(
                {
                    ...
                    "order_number": (
                        cutting_order.order.order_number
                        if cutting_order.order
                        else "N/A"
                    ),
                    "invoice_number": (
                        cutting_order.order.invoice_number
                        if cutting_order.order
                        else "N/A"
                    ),
                    "current_warehouse": cutting_order.warehouse,
                    ...
                    "items_to_move": sum(
                        len(items) for items in items_by_warehouse.values()
                    ),
    ...
    for idx, issue in enumerate(issues, 1):
        ...
        if issue["items_by_warehouse"]:
            ...
            for warehouse_id, items in issue["items_by_warehouse"].items():
                ...
                print(
                    f"          أصناف حرجة (لا يوجد مخزون في الحالي): {len(critical_items)}"
                )
        ...
        if issue.get("service_items"):
            ...
            for item_data in issue["service_items"][:3]:
                print(
                    f"       ⛔ {item_data['product'].name} (كود: {item_data['product'].code or 'N/A'})"
                )
        ...
        if issue.get("duplicate_items"):
            ...
            for order_item_id, items in list(issue["duplicate_items"].items())[:3]:
                ...
                print(
                    f"       🔁 {product_name}: {len(items)} نسخ (مكتمل: {completed_count}, قيد الانتظار: {pending_count})"
                )
...
def generate_cutting_code(order, warehouse):
    ...
    existing_codes = CuttingOrder.objects.filter(order=order).values_list(
        "cutting_code", flat=True
    )
    ...
    for warehouse_id, items in issue["items_by_warehouse"].items():
        ...
        for item_data in items:
            ...
            try:
                ...
                results["errors"].append(
                    {"product": item_data["product"].name, "error": str(e)}
                )
...
def find_issue_by_invoice(issues, invoice_number):
    ...
    for issue in issues:
        if (
            ...
            and str(issue["invoice_number"]).strip() == str(invoice_number).strip()
        ):
            return issue
    return None
...
def find_issue_by_cutting_code(issues, cutting_code):
    ...
    for issue in issues:
        if (
            issue["cutting_code"]
            and str(issue["cutting_code"]).strip() == str(cutting_code).strip()
        ):
            return issue
    return None
...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            try:
                confirm = input(
                    "\n⚠️  هل أنت متأكد من إصلاح جميع الأصناف؟ (نعم/لا): "
                ).strip()
            except (EOFError, KeyboardInterrupt):
                print("\n❌ تم إلغاء العملية.")
                continue
                ...
                for issue in issues:
                    ...
                    if results["moved_items"]:
                        ...
                        for item in results["moved_items"][:3]:
                            print(
                                f"     - {item['product']}: {item['from_warehouse']} → {item['to_warehouse']}"
                            )
                        if len(results["moved_items"]) > 3:
                            print(
                                f"     ... و {len(results['moved_items']) - 3} صنف آخر"
                            )

                    if results["new_orders_created"]:
                        print(
                            f"  🆕 تم إنشاء {len(results['new_orders_created'])} أمر تقطيع جديد:"
                        )
                        for order in results["new_orders_created"]:
                            print(
                                f"     - {order['code']} (مستودع: {order['warehouse']})"
                            )
                        total_new_orders += len(results["new_orders_created"])

                    if results["moved_to_existing"]:
                        print(
                            f"  📦 تم نقل أصناف لـ {len(results['moved_to_existing'])} أمر موجود:"
                        )
                        for order in results["moved_to_existing"]:
                            print(
                                f"     - {order['code']} (مستودع: {order['warehouse']}, أصناف: {order['items_count']})"
                            )
            ...
            for i in issues:
                print(
                    f"   - {i['invoice_number']} (كود التقطيع: {i['cutting_code']}, أصناف تحتاج نقل: {i['items_to_move']})"
                )
            ...
            if confirm in ["نعم", "yes", "y", "ن"]:
                ...
                if results["moved_items"]:
                    ...
                    for warehouse, items in items_by_warehouse.items():
                        ...
                        for item in items[:5]:  # عرض أول 5 أصناف
                            action_icon = (
                                "🆕" if item["action"] == "moved_to_new" else "📥"
                            )
                            print(f"      {action_icon} {item['product']}")
                            print(
                                f"         من: {item['from_warehouse']} → إلى: {item['to_warehouse']}"
                            )
                            print(f"         أمر التقطيع: {item['to_order']}")
                            print(
                                f"         المخزون في المستودع المستهدف: {item['stock_in_target']}"
                            )
                ...
                print(
                    f"   - أوامر موجودة تم النقل إليها: {len(results['moved_to_existing'])}"
                )
            ...
            for i in issues:
                print(
                    f"   - {i['cutting_code']} (فاتورة: {i['invoice_number']}, أصناف: {i['items_to_move']})"
                )
