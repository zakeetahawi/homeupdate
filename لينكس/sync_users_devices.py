...
from accounts.models import Branch, BranchDevice, User
...
def sync_users_to_devices(branch_name=None, dry_run=False):
    ...
    for branch in branches:
        ...
        for device in devices:
            print(
                f"      • {device.device_name} (مستخدمين حاليين: {device.authorized_users.count()})"
            )
        print()
        ...
        for user in users:
            ...
            if not dry_run:
                with transaction.atomic():
                    ...
                    if new_devices > 0:
                        print(
                            f"   ✅ {user.username}: أضيف {new_devices} جهاز (الإجمالي: {user_devices_after})"
                        )
                        branch_users_updated += 1
                        branch_devices_linked += new_devices
                    else:
                        print(
                            f"   ℹ️  {user.username}: مرتبط بالفعل بكل الأجهزة ({user_devices_after})"
                        )
            else:
                # وضع المعاينة
                missing_devices = []
                ...
                if missing_devices:
                    print(
                        f"   📝 {user.username}: سيتم إضافة {len(missing_devices)} جهاز: {', '.join(missing_devices)}"
                    )
                    branch_users_updated += 1
                    branch_devices_linked += len(missing_devices)
                else:
                    print(
                        f"   ℹ️  {user.username}: مرتبط بالفعل بكل الأجهزة ({user_devices_before})"
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dry-run", "-d", action="store_true", help="معاينة فقط دون حفظ التغييرات"
    )
    parser.add_argument(
        "--status", "-s", action="store_true", help="عرض الحالة الحالية فقط"
    )
