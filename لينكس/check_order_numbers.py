...
def check_order_number_validity(order):
    ...
    if not order.order_number.startswith(customer_code):
        ...
        actual_prefix = (
            order.order_number.rsplit("-", 1)[0]
            if "-" in order.order_number
            else order.order_number
        )
        return (
            False,
            f"رقم الطلب ({order.order_number}) لا يبدأ بكود العميل ({customer_code})",
        )
...
def get_orders_to_check(days=None, all_orders=False):
    ...
    if days:
        ...
        return Order.objects.filter(created_at__gte=start_date).select_related(
            "customer"
        )
...
def main():
    ...
    parser.add_argument(
        "--days", type=int, default=7, help="عدد الأيام للفحص (افتراضي: 7)"
    )
    parser.add_argument(
        "--fix", action="store_true", help="إصلاح الطلبات التي بها مشاكل"
    )
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="عرض تفاصيل إضافية"
    )
    parser.add_argument("--all", action="store_true", help="فحص جميع الطلبات")
    parser.add_argument(
        "--dry-run", action="store_true", help="محاكاة الإصلاح بدون تطبيق التغييرات"
    )
    ...
    for order in orders:
        ...
        if is_valid:
            ...
            invalid_count += 1
            invalid_orders.append({"order": order, "message": message})
            print_error(
                f"{order.order_number} - {order.customer.name if order.customer else 'بدون عميل'}"
            )
            print(f"   └─ {message}")
    ...
    if invalid_count > 0:
        ...
        for item in invalid_orders:
            ...
            print(
                f"{order.order_number:<20} {customer_name:<30} {customer_code:<15} {item['message']}"
            )
