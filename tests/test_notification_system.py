    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if "notifications" in data:
                print(
                    f"✅ API الإشعارات الحديثة يعمل: {len(data['notifications'])} إشعار"
                )
                return True
            else:
    ...
    try:
        ...
        if not branch:
            branch = Branch.objects.create(
                name="فرع تجريبي", code="TEST", address="عنوان تجريبي"
            )
        ...
        if notifications_after > notifications_before:
            ...
            customer_notification = Notification.objects.filter(
                notification_type="customer_created", object_id=customer.id
            ).first()
