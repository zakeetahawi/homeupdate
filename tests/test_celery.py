...
from crm.celery import get_celery_stats, test_celery_connection
from orders.tasks import cleanup_failed_uploads, clear_expired_cache
...
def test_redis():
    ...
    try:
        ...
        if r.ping():
            ...
            try:
                info = r.info()
                version = info.get("redis_version") or info.get(
                    "valkey_version", "غير معروف"
                )
                print(f"📊 الإصدار: {version}")
                print(
                    f"💾 استخدام الذاكرة: {info.get('used_memory_human', 'غير معروف')}"
                )
                print(
                    f"👥 العملاء المتصلون: {info.get('connected_clients', 'غير معروف')}"
                )
            except Exception:
