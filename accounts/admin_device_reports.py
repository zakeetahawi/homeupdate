...
from .models import ActivityLog, Branch, BranchDevice, UnauthorizedDeviceAttempt
...
def device_dashboard_view(request):
    ...
    recently_used = BranchDevice.objects.filter(last_used__gte=week_ago).order_by(
        "-last_used"
    )[:10]
    ...
    inactive_long = BranchDevice.objects.filter(
        Q(last_used__lt=month_ago) | Q(last_used__isnull=True), is_active=True
    ).count()
