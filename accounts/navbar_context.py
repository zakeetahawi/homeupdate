...
def navbar_departments(request):
    ...
    all_units = Department.objects.filter(
        parent__isnull=False,
        is_active=True
    ).select_related('parent').order_by('parent__order', 'order')
    
    # تنظيم الوحدات حسب العناصر المراد عرضها
    navbar_items = {
        'customers': {'name': 'العملاء', 'icon': 'fa-users', 'url': '/customers/', 'units': []},
        'orders': {'name': 'الطلبات', 'icon': 'fa-shopping-cart', 'url': '/orders/', 'units': []},
        'inventory': {'name': 'المخزون', 'icon': 'fa-boxes', 'url': None, 'units': []},
        'inspections': {'name': 'المعاينات', 'icon': 'fa-clipboard-check', 'url': '/inspections/', 'units': []},
        'installations': {'name': 'التركيبات', 'icon': 'fa-tools', 'url': '/installations/', 'units': []},
        'manufacturing': {'name': 'المصنع', 'icon': 'fa-industry', 'url': None, 'units': []},
        'complaints': {'name': 'الشكاوى', 'icon': 'fa-exclamation-triangle', 'url': None, 'units': []},
        'reports': {'name': 'التقارير', 'icon': 'fa-chart-bar', 'url': None, 'units': []},
        'accounting': {'name': 'المحاسبة', 'icon': 'fa-calculator', 'url': None, 'units': []},
        'database': {'name': 'إدارة البيانات', 'icon': 'fa-database', 'url': '/database/', 'units': []},
    }
    ...
    navbar_items_filtered = {
        key: value for key, value in navbar_items.items()
        if value['units']
    }
