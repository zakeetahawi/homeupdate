...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"تم إضافة {all_permissions.count()} صلاحية لدور مدير النظام"
            )
        )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"تم إضافة {admin_permissions.count()} صلاحية لدور المدير"
            )
        )
        ...
        sales_permissions = Permission.objects.filter(
            Q(content_type__app_label__in=sales_apps) | Q(content_type=customer_ct)
        )
        sales_manager_role.permissions.set(sales_permissions)
        self.stdout.write(
            self.style.SUCCESS(
                f"تم إضافة {sales_permissions.count()} صلاحية لدور مدير المبيعات"
            )
        )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"تم إضافة {inspection_permissions.count()} صلاحية لدور فني المعاينة"
            )
        )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"تم إضافة {installation_permissions.count()} صلاحية لدور فني التركيب"
            )
        )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"تم إضافة {inventory_permissions.count()} صلاحية لدور مسؤول المخزون"
            )
        )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"تم إضافة {factory_permissions.count()} صلاحية لدور مسؤول المصنع"
            )
        )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"تم إضافة {sales_employee_permissions.count()} صلاحية لدور موظف المبيعات"
            )
        )
        ...
        for user in superusers:
            ...
            self.stdout.write(
                self.style.SUCCESS(f"تم إسناد دور مدير النظام للمستخدم {user.username}")
            )
