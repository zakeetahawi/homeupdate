...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--role_id", type=int, help="معرف الدور المراد تحديث صلاحياته"
        )
        parser.add_argument(
            "--user_id", type=int, help="معرف المستخدم المراد تحديث صلاحياته"
        )
    ...
    def handle(self, *args, **options):
        ...
        if role_id:
            ...
            try:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"تم تحديث صلاحيات {user_roles.count()} مستخدمين للدور {role.name}"
                    )
                )
            ...
            try:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f"تم تحديث صلاحيات المستخدم {user.username}")
                )
            ...
            self.stdout.write(
                self.style.SUCCESS(f"تم تحديث صلاحيات {count} مستخدمين بنجاح")
            )
