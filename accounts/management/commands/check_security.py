...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if (
            ...
            self.stdout.write(
                self.style.SUCCESS("✓ SECURE_PROXY_SSL_HEADER is configured")
            )
        else:
            self.stdout.write(
                self.style.WARNING("✗ SECURE_PROXY_SSL_HEADER is not configured")
            )
        ...
        if settings.SECURE_HSTS_SECONDS:
            self.stdout.write(
                self.style.SUCCESS(
                    f"✓ HSTS is enabled ({settings.SECURE_HSTS_SECONDS} seconds)"
                )
            )
        ...
        if settings.SESSION_COOKIE_SECURE:
            ...
            self.stdout.write(
                self.style.WARNING("✗ Secure session cookies not enabled")
            )
        ...
        if settings.SECURE_REFERRER_POLICY:
            self.stdout.write(
                self.style.SUCCESS(
                    f"✓ Referrer Policy is set to: {settings.SECURE_REFERRER_POLICY}"
                )
            )
        else:
        ...
        if settings.ALLOWED_HOSTS:
            self.stdout.write(
                self.style.SUCCESS(
                    f'✓ ALLOWED_HOSTS is configured: {", ".join(settings.ALLOWED_HOSTS)}'
                )
            )
        else:
        ...
        if settings.DEBUG:
            self.stdout.write(
                self.style.ERROR("✗ DEBUG is enabled - disable this in production")
            )
        else:
