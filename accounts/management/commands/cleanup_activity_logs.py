...
from accounts.models import (
    UserActivityLog, UserSession, UserLoginHistory, OnlineUser
)
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--all',
            action='store_true',
            help='تنظيف جميع البيانات القديمة'
        )
    ...
    def cleanup_online_users(self, dry_run=False):
        """تنظيف المستخدمين غير المتصلين"""
        timeout_minutes = getattr(settings, 'ACTIVITY_TRACKING', {}).get('ONLINE_TIMEOUT_MINUTES', 5)
    ...
    def cleanup_expired_sessions(self, cleanup_date, dry_run=False):
        ...
        expired_sessions = UserSession.objects.filter(
            last_activity__lt=cleanup_date
        )
        ...
        if count > 0:
            ...
            if not dry_run:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f'✅ تم حذف {count} جلسة منتهية')
                )
        else:
    ...
    def cleanup_login_history(self, cleanup_date, dry_run=False):
        """تنظيف سجلات تسجيل الدخول القديمة"""
        old_logins = UserLoginHistory.objects.filter(
            login_time__lt=cleanup_date
        )
    ...
    def cleanup_excessive_logs_per_user(self, dry_run=False):
        ...
        max_logs = getattr(settings, 'ACTIVITY_TRACKING', {}).get('MAX_ACTIVITY_LOGS_PER_USER', 1000)
        ...
        if total_deleted > 0:
            self.stdout.write(
                self.style.SUCCESS(f'✅ تم حذف {total_deleted} سجل زائد')
            )
        else:
