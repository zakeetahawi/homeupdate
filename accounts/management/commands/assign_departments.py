    ...
    def handle(self, *args, **options):
        ...
        if options["departments"]:
            ...
            existing_departments = Department.objects.filter(
                code__in=dept_codes, is_active=True
            )
            ...
            if missing_codes:
                self.stdout.write(
                    self.style.WARNING(
                        f"⚠️ أقسام غير موجودة: {', '.join(missing_codes)}"
                    )
                )
            ...
            if existing_departments.exists():
                ...
                self.stdout.write(self.style.ERROR("❌ لم يتم العثور على أقسام صالحة"))
        else:
            self.stdout.write(
                self.style.WARNING(
            ...
            self.stdout.write("\nأمثلة:")
            self.stdout.write(
                "  python manage.py assign_departments --user john --departments customers,orders"
            )
            self.stdout.write(
                "  python manage.py assign_departments --user john --show"
            )
            self.stdout.write(
                "  python manage.py assign_departments --user john --clear"
            )
            self.stdout.write(
                "  python manage.py assign_departments --user john --list-departments"
            )
