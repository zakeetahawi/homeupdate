...
from accounts.models import BranchDevice, Branch
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if total_devices > 0:
            ...
            for device in devices.select_related('branch', 'last_used_by'):
                ...
                if device.last_used:
                    ...
                    self.stdout.write(
                        self.style.WARNING("   ⚠️  لم يُستخدم أبداً")
                    )
        ...
        if never_used > 0:
            self.stdout.write(
                self.style.WARNING(
                    f"⚠️  تنبيه: يوجد {never_used} جهاز لم يُستخدم أبداً"
                )
            )
        ...
        if inactive_devices > 0:
            self.stdout.write(
                self.style.WARNING(
                    f"⚠️  تنبيه: يوجد {inactive_devices} جهاز غير نشط"
                )
            )
