...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Successfully updated inventory department URL from {dept.url_name} to inventory:dashboard"
                )
            )
            ...
            self.stdout.write(
                self.style.ERROR(f"Error updating inventory department URL: {str(e)}")
            )
