...
class Command(BaseCommand):
    ...
    def fix_sessions(self, dry_run=False):
        ...
        if not duplicate_keys:
            self.stdout.write(
                self.style.SUCCESS("لا يوجد جلسات مكررة في قاعدة البيانات.")
            )
            return
        ...
        for key in duplicate_keys:
            ...
            if sessions.count() > 1:
                ...
                self.stdout.write(
                    f"الاحتفاظ بالجلسة الأحدث (ID: {primary_session.id}) وحذف {duplicate_sessions.count()} جلسة مكررة"
                )

                if not dry_run:
                    duplicate_sessions.delete()
                    self.stdout.write(
                        self.style.SUCCESS(f"تم إصلاح الجلسة المكررة: {key}")
                    )
                else:
                    self.stdout.write(
                        self.style.WARNING("وضع المحاكاة: لم يتم تنفيذ أي تغييرات")
                    )
        ...
        if expired_count > 0:
            ...
            if not dry_run:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f"تم حذف {expired_count} جلسة منتهية الصلاحية")
                )
            else:
                self.stdout.write(
                    self.style.WARNING("وضع المحاكاة: لم يتم تنفيذ أي تغييرات")
        ...
        if not duplicate_usernames:
            self.stdout.write(
                self.style.SUCCESS("لا يوجد مستخدمين مكررين في قاعدة البيانات.")
            )
            return
        ...
        for username in duplicate_usernames:
            ...
            for i, user in enumerate(users):
                self.stdout.write(
                    f"  {i+1}. ID: {user.id}, البريد الإلكتروني: {user.email}, تاريخ الانضمام: {user.date_joined}"
                )
            ...
            self.stdout.write(
                f"الاحتفاظ بالمستخدم الأول (ID: {primary_user.id}) وحذف {duplicate_users.count()} مستخدم مكرر"
            )
            ...
            if not dry_run:
                    ...
                    for dup_user in duplicate_users:
                        ...
                        for role in UserRole.objects.filter(user=dup_user):
                            UserRole.objects.get_or_create(
                                user=primary_user, role=role.role
                            )
                ...
                self.stdout.write(
                    self.style.SUCCESS(f"تم إصلاح المستخدم المكرر: {username}")
                )
            else:
                self.stdout.write(
                    self.style.WARNING("وضع المحاكاة: لم يتم تنفيذ أي تغييرات")
        ...
        if not dry_run:
            self.stdout.write(
                self.style.SUCCESS("تم إصلاح جميع المستخدمين المكررين بنجاح.")
            )
        else:
            self.stdout.write(
                self.style.WARNING(
                    "وضع المحاكاة: لم يتم تنفيذ أي تغييرات. قم بتشغيل الأمر بدون --dry-run لتنفيذ التغييرات."
                )
            )
