...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not departments_to_delete.exists():
            self.stdout.write(self.style.SUCCESS("✅ لا توجد أقسام وهمية للحذف"))
            return
        ...
        if options["dry_run"]:
            self.stdout.write(self.style.WARNING("🔍 وضع المعاينة - لن يتم حذف أي شيء"))
            return
        ...
        self.stdout.write(
            self.style.SUCCESS(f"🎉 تم حذف {deleted_count} قسم وهمي بنجاح!")
        )
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS("✅ تم تحديث رابط قسم إدارة البيانات"))
