...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if created:
            self.stdout.write(
                self.style.SUCCESS(
                    f"Created inventory department: {inventory_dept.name}"
                )
            )
        else:
            self.stdout.write(
                self.style.SUCCESS(
                    f"Found existing inventory department: {inventory_dept.name}"
                )
            )
        ...
        for action in ["add", "change", "delete", "view"]:
            ...
            perm, created = Permission.objects.get_or_create(
                codename=codename, content_type=product_ct, defaults={"name": name}
            )
        ...
        for action in ["add", "change", "delete", "view"]:
            ...
            perm, created = Permission.objects.get_or_create(
                codename=codename, content_type=category_ct, defaults={"name": name}
            )
            permissions.append(perm)
            if created:
                self.stdout.write(self.style.SUCCESS(f"Created permission: {name}"))
        ...
        for action in ["add", "change", "delete", "view"]:
            ...
            perm, created = Permission.objects.get_or_create(
                codename=codename, content_type=supplier_ct, defaults={"name": name}
            )
            permissions.append(perm)
            if created:
                self.stdout.write(self.style.SUCCESS(f"Created permission: {name}"))
        ...
        for user in inventory_users:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Assigned inventory permissions to user: {user.username}"
                )
            )

        self.stdout.write(
            self.style.SUCCESS("Inventory permissions setup completed successfully!")
        )
