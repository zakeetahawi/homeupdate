    ...
    def handle(self, *args, **options):
        with connection.cursor() as cursor:
            try:
                ...
                self.stdout.write(
                    self.style.SUCCESS("Accounts User Sequence Information:")
                )
                ...
                if max_id is not None and current_seq is not None:
                    ...
                    if gap < 0:
                        self.stdout.write(
                            self.style.ERROR(
                                "  ⚠️  Sequence is behind the maximum ID! This can cause primary key conflicts."
                            )
                        )
                        self.stdout.write(
                            self.style.SUCCESS(
                                "  To fix, run: ALTER SEQUENCE accounts_user_id_seq RESTART WITH <new_value>;"
                            )
                        )
                        self.stdout.write(
                            f"  Suggested command: ALTER SEQUENCE accounts_user_id_seq RESTART WITH {max_id + 1};"
                        )
                    elif gap == 0:
                        self.stdout.write(
                            self.style.SUCCESS(
                                "  ✅ Sequence is in sync with the table."
                            )
                        )
                    else:
                        self.stdout.write(
                            self.style.WARNING(
                                "  ℹ️  Sequence is ahead of the maximum ID. This is normal but may indicate unused sequence values."
                            )
                        )

            except Exception as e:
                self.stdout.write(
                    self.style.ERROR(f"Error checking sequence: {str(e)}")
                )
