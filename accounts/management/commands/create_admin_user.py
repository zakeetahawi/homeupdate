...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--username", type=str, default="admin", help=_("اسم المستخدم")
        )
        parser.add_argument(
            "--email",
            type=str,
            default="admin@example.com",
            help=_("البريد الإلكتروني"),
        )
        parser.add_argument("--password", type=str, default=None, help=_("كلمة المرور"))
        parser.add_argument(
            "--force",
            action="store_true",
            help=_("إنشاء المستخدم حتى لو كان موجودًا بالفعل"),
        )
    ...
    def handle(self, *args, **options):
        ...
        try:
            if User.objects.filter(username=username).exists() and force:
                ...
                user.save()
                self.stdout.write(
                    self.style.SUCCESS(f'تم تحديث المستخدم المسؤول "{username}" بنجاح.')
                )
            else:
                user = User.objects.create_superuser(
                    ...
                    email=email,
                    password=password
                    or username,  # استخدام اسم المستخدم ككلمة مرور إذا لم يتم تحديد كلمة مرور
                )
                self.stdout.write(
                    self.style.SUCCESS(f'تم إنشاء المستخدم المسؤول "{username}" بنجاح.')
                )
                if not password:
                    self.stdout.write(
                        self.style.WARNING(
