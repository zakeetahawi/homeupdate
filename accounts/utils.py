...
def send_notification(title, message, sender, target_department=None, target_branch=None, priority='medium'):
    """
    ...
    Args:
        ...
        message: نص الإشعار
        sender: المرسل
        target_department: القسم المستهدف (اختياري)
        target_branch: الفرع المستهدف (اختياري)
        priority: أولوية الإشعار (افتراضي: medium)
    
    Returns:
        Notification: كائن الإشعار المُنشأ
    """
...
def get_user_notifications(user, limit=10, unread_only=False):
    ...
    Args:
        user: المستخدم
        limit: عدد الإشعارات (افتراضي: 10)
        unread_only: إظهار الإشعارات غير المقروءة فقط (افتراضي: False)
    
    Returns:
        QuerySet: إشعارات المستخدم
    """
    from .models import Notification
    
    # الحصول على الإشعارات المرسلة للمستخدم أو قسمه أو فرعه
    notifications = Notification.objects.filter(
        Q(target_users=user) |
        Q(target_department__in=user.departments.all()) |
        Q(target_branch=user.branch)
    ).distinct()
    
    # تصفية الإشعارات غير المقروءة إذا طُلب ذلك
    if unread_only:
