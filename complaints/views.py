...
class ComplaintCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            if self.request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                ...
                messages.success(
                    self.request,
                    f'تم إنشاء الشكوى بنجاح برقم {self.object.complaint_number}'
                )
    ...
    def form_invalid(self, form):
        ...
        print("===== انتهاء معالجة أخطاء النموذج =====")
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            ComplaintUpdate.objects.create(
                ...
                description=notes or f'تم تغيير حالة الشكوى إلى {complaint.get_status_display()}',
                created_by=request.user,
                is_visible_to_customer=True
            )
            
            messages.success(request, 'تم تحديث حالة الشكوى بنجاح')
...
def complaint_assignment(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            assignee_name = complaint.assigned_to.get_full_name() if complaint.assigned_to else 'غير محدد'
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            ComplaintUpdate.objects.create(
                ...
                title=f'تصعيد الشكوى - {escalation.get_reason_display()}',
...
def notifications_list(request):
    """قائمة إشعارات الشكاوى للمستخدم"""
    notifications = ComplaintNotification.objects.filter(
        recipient=request.user
    ).select_related(
        'complaint', 'complaint__customer'
    ).order_by('-created_at')
    ...
    return render(request, 'complaints/notifications.html', context)
