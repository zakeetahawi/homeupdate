...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS('🔍 بدء فحص الشكاوى المتأخرة...')
        )
        ...
        overdue_complaints = Complaint.objects.filter(**overdue_filter).select_related(
            'customer', 'complaint_type', 'assigned_to', 'assigned_department'
        ).order_by('deadline')
        
        if not overdue_complaints.exists():
            self.stdout.write(
                self.style.SUCCESS('✅ لا توجد شكاوى متأخرة')
            )
            return
        
        self.stdout.write(
            self.style.WARNING(f'⚠️  تم العثور على {overdue_complaints.count()} شكوى متأخرة')
        )
        ...
        for complaint in overdue_complaints:
            ...
            self.stdout.write(
                f'   📅 متأخرة منذ: {days_late} يوم'
            )
            self.stdout.write(
                f'   👤 المسؤول: {complaint.assigned_to or "غير محدد"}'
            )
            self.stdout.write(
                f'   🏢 القسم: {complaint.assigned_department or "غير محدد"}'
            )
            self.stdout.write(
                f'   📊 الحالة: {complaint.get_status_display()}'
            )
            self.stdout.write('   ' + '-' * 50)
        
        if send_notifications:
            self.stdout.write(
                self.style.SUCCESS('📧 بدء إرسال التنبيهات...')
            )
            ...
            for complaint in overdue_complaints:
                try:
                    ...
                    self.stdout.write(
                        self.style.ERROR(f'❌ خطأ في إرسال التنبيهات للشكوى {complaint.complaint_number}: {str(e)}')
                    )
            
            self.stdout.write(
                self.style.SUCCESS('✅ تم الانتهاء من إرسال التنبيهات')
            )
        else:
            self.stdout.write(
                self.style.WARNING('⚠️  لم يتم إرسال التنبيهات (استخدم --send-notifications لإرسالها)')
            )
        ...
        departments = {}
        for complaint in overdue_complaints:
            dept_name = complaint.assigned_department.name if complaint.assigned_department else 'غير محدد'
        ...
        assignees = {}
        for complaint in overdue_complaints:
            assignee_name = str(complaint.assigned_to) if complaint.assigned_to else 'غير محدد'
