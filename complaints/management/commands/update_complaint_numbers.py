...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS('🔄 بدء تحديث أرقام الشكاوى...')
        )
        
        # الحصول على جميع الشكاوى التي لا تتبع النظام الجديد
        complaints_to_update = Complaint.objects.exclude(
            complaint_number__startswith='P'
        ...
        self.stdout.write(
            f'📊 تم العثور على {total_complaints} شكوى تحتاج تحديث'
        )
        ...
        try:
                    ...
                    for index, complaint in enumerate(complaints, 1):
                        ...
                        if Complaint.objects.filter(
                            ...
                            if not force:
                                ...
                                # البحث عن رقم بديل
                                counter = index
                                while Complaint.objects.filter(
            ...
            self.stdout.write(
                self.style.ERROR(f'❌ خطأ أثناء التحديث: {str(e)}')
            )
            return
        ...
        if dry_run:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f'✅ تم تحديث {updated_count} شكوى بنجاح!'
                )
            )
            if errors_count > 0:
                self.stdout.write(
