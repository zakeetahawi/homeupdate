...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--test-form", action="store_true", help="اختبار نموذج الفلترة"
        )
        parser.add_argument(
            "--test-queries", action="store_true", help="اختبار استعلامات الفلترة"
        )
        parser.add_argument(
            "--test-all", action="store_true", help="اختبار جميع الوظائف"
        )
        parser.add_argument(
            "--create-test-data", action="store_true", help="إنشاء بيانات اختبار"
        )

    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS("🔍 بدء اختبار نظام فلاتر الشكاوى..."))
        ...
        self.stdout.write(self.style.SUCCESS("✅ انتهى اختبار نظام الفلاتر"))
    ...
    def test_filter_queries(self):
        ...
        if total_complaints == 0:
            self.stdout.write(self.style.WARNING("⚠️ لا توجد شكاوى للاختبار"))
            return
    ...
    def test_complete_filtering(self):
        ...
        self.stdout.write("\n📋 الخيارات المتاحة:")
        self.stdout.write(
            f"أنواع الشكاوى النشطة: {ComplaintType.objects.filter(is_active=True).count()}"
        )
        self.stdout.write(
            f"المستخدمون النشطون: {User.objects.filter(is_active=True).count()}"
        )
        self.stdout.write(
            f"الأقسام النشطة: {Department.objects.filter(is_active=True).count()}"
        )
