...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS('=== تشخيص نظام الشكاوى ===')
        )
    ...
    def check_complaints_status(self):
        ...
        status_stats = Complaint.objects.values('status').annotate(
            count=Count('id')
        ).order_by('-count')
    ...
    def check_specific_user(self, username):
        ...
        try:
            ...
            try:
                ...
                self.stdout.write(f'  - تعديل الكل: {permissions.can_edit_all_complaints}')
                self.stdout.write(f'  - عرض الكل: {permissions.can_view_all_complaints}')
            ...
            self.stdout.write(
                self.style.ERROR(f'المستخدم غير موجود: {username}')
            )
    ...
    def check_specific_complaint(self, complaint_id):
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(f'الشكوى غير موجودة: {complaint_id}')
            )
    ...
    def performance_test(self):
        ...
        if complaints:
            ...
            self.stdout.write(f'وقت تحميل شكوى مع التفاصيل: {end_time - start_time:.3f} ثانية')
            self.stdout.write(f'عدد الاستعلامات: {len(connection.queries)}')
