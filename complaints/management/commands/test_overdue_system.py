...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS('🧪 بدء اختبار نظام الشكاوى المتأخرة...')
        )
        ...
        self.stdout.write(
            self.style.SUCCESS('✅ تم الانتهاء من الاختبار')
        )
    ...
    def create_test_data(self):
        ...
        try:
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS(f'✅ تم إنشاء شكوى اختبار: {complaint.complaint_number}')
                )
            else:
                self.stdout.write(
                    self.style.WARNING(f'⚠️  شكوى الاختبار موجودة مسبقاً: {complaint.complaint_number}')
                )
    ...
    def test_notification_system(self):
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(f'❌ خطأ في اختبار التنبيهات: {str(e)}')
            )
    ...
    def check_current_overdue_complaints(self):
        ...
        if not overdue_complaints.exists():
            self.stdout.write(
                self.style.SUCCESS('✅ لا توجد شكاوى متأخرة حالياً')
            )
            return
