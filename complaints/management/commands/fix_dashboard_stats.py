...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS("✅ تم الانتهاء من إصلاح الإحصائيات"))
    ...
    def fix_overdue_complaints(self, dry_run=False):
        ...
        self.stdout.write(
            self.style.WARNING(
                f"⚠️  تم العثور على {overdue_complaints.count()} شكوى تحتاج تحديث"
            )
        )
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f"❌ خطأ في تحديث الشكاوى: {str(e)}"))

    def send_overdue_notifications(self, complaints):
        """إرسال تنبيهات للشكاوى المتأخرة"""
        try:
            from complaints.services.notification_service import (
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"📧 تم إرسال تنبيهات لـ {complaints.count()} شكوى متأخرة"
                )
            )

        except Exception as e:
            self.stdout.write(self.style.ERROR(f"❌ خطأ في إرسال التنبيهات: {str(e)}"))
    ...
    def validate_stats_consistency(self):
        ...
        if actually_overdue != status_overdue:
            issues.append(
                f"تضارب في الشكاوى المتأخرة: فعلياً {actually_overdue} vs حالة {status_overdue}"
            )
        ...
        if issues:
            ...
            self.stdout.write(self.style.SUCCESS("✅ الإحصائيات متناسقة"))
