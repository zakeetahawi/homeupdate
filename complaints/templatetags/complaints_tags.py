...
def order_complaints_widget(order, limit=3):
    ...
    complaints = Complaint.objects.filter(
        ...
        'complaint_type', 'assigned_to', 'customer'
    ).order_by('-created_at')[:limit]
...
def complaint_status_badge(status):
    ...
    status_config = {
        ...
        'in_progress': {'class': 'bg-warning', 'text': 'قيد الحل', 'icon': 'fas fa-cog'},
        'resolved': {'class': 'bg-success', 'text': 'محلولة', 'icon': 'fas fa-check-circle'},
        'pending_evaluation': {'class': 'bg-warning text-dark', 'text': 'بحاجة تقييم', 'icon': 'fas fa-star-half-alt'},
        'closed': {'class': 'bg-secondary', 'text': 'مغلقة', 'icon': 'fas fa-times-circle'},
        'overdue': {'class': 'bg-danger', 'text': 'متأخرة', 'icon': 'fas fa-exclamation-triangle'},
...
def get_complaint_stats_for_period(days=30):
    ...
    stats = Complaint.objects.filter(
