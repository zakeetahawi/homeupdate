...
class ComplaintForm(forms.ModelForm):
    ...
    template = forms.ModelChoiceField(
        ...
        label="نموذج جاهز",
        widget=forms.Select(
            attrs={"class": "form-select", "data-placeholder": "اختر نموذجاً جاهزاً"}
        ),
    )
        ...
        if "complaint_type" in self.data:
            try:
                ...
                self.fields["template"].queryset = ComplaintTemplate.objects.filter(
                    complaint_type_id=complaint_type_id, is_active=True
                )
    ...
    def clean(self):
        ...
        try:
            ...
            if template:
                ...
                try:
                    ...
                    cleaned_data["title"] = template.generate_title(
                        customer=customer, order=related_order
                    )
            ...
            if not customer:
                ...
                try:
                    if (
                        hasattr(customer, "has_active_contract")
                        and not customer.has_active_contract()
                    ):
            ...
            if related_order and customer:
                ...
                if hasattr(related_order, "status") and related_order.status in [
                    "cancelled",
                    "rejected",
                ]:
        ...
        if deadline:
            ...
            if complaint_type and complaint_type.default_deadline_hours:
                max_deadline = timezone.now() + timedelta(
                    hours=complaint_type.default_deadline_hours * 2
                )
    ...
    def clean_related_order(self):
        ...
        print(
            f"العميل المستخدم للتحقق: {customer} (نوع: {type(customer) if customer else 'None'})"
        )

        # إذا كان هناك عميل محدد، نتحقق من أن الطلب يخصه
        if customer and hasattr(related_order, "customer"):
            print(
                f"مقارنة: معرف عميل الطلب: {related_order.customer_id} - معرف العميل المحدد: {customer.id}"
            )
...
class ComplaintUpdateForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "description": forms.Textarea(
                attrs={"placeholder": "تفاصيل التحديث (إجباري)", "rows": 4}
            ),
        }
...
class ComplaintFilterForm(forms.Form):
    ...
    status = forms.ChoiceField(
        ...
        label="الحالة",
        widget=forms.Select(
            attrs={"class": "form-select", "data-placeholder": "اختر الحالة..."}
        ),
    )
    priority = forms.ChoiceField(
        ...
        label="الأولوية",
        widget=forms.Select(
            attrs={"class": "form-select", "data-placeholder": "اختر الأولوية..."}
        ),
    )
    complaint_type = forms.ModelChoiceField(
        ...
        label="نوع الشكوى",
        widget=forms.Select(
            attrs={"class": "form-select", "data-placeholder": "اختر النوع..."}
        ),
    )
    assigned_to = forms.ModelChoiceField(
        ...
        label="المسؤول",
        widget=forms.Select(
            attrs={"class": "form-select", "data-placeholder": "اختر الموظف..."}
        ),
    )
    assigned_department = forms.ModelChoiceField(
        queryset=Department.objects.filter(is_active=True),
        ...
        label="القسم",
        widget=forms.Select(
            attrs={"class": "form-select", "data-placeholder": "اختر القسم..."}
        ),
    )
