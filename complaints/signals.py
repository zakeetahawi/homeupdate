...
def handle_post_save_notifications(sender, instance, created, **kwargs):
    ...
    if not created:
        ...
        if hasattr(instance, "_assignee_changed") and instance._assignee_changed:
            ...
            logger.info(
                f"🔄 تغيير المسؤول للشكوى {instance.complaint_number}: من {instance._old_assignee} إلى {instance.assigned_to}"
            )
...
def handle_update_notifications(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        if (
            instance.complaint.assigned_department
            and instance.complaint.assigned_department.manager
        ):
...
def check_complaint_deadlines():
    ...
    overdue = Complaint.objects.filter(
        status__in=["new", "in_progress"], deadline__lt=timezone.now()
    )
