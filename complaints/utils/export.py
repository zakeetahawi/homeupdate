...
def export_complaints_to_csv(queryset=None, selected_ids=None):
    ...
    response['Content-Disposition'] = f'attachment; filename="complaints-{timezone.now().strftime("%Y%m%d-%H%M%S")}.csv"'
    ...
    for complaint in queryset.select_related(
        ...
        'assigned_department', 'related_order', 'branch'
    ):
        row = [
            ...
            complaint.assigned_to.get_full_name() if complaint.assigned_to else 'غير معين',
            complaint.assigned_department.name if complaint.assigned_department else 'غير محدد',
            timezone.localtime(complaint.created_at).strftime('%Y-%m-%d %H:%M'),
            timezone.localtime(complaint.deadline).strftime('%Y-%m-%d %H:%M'),
            timezone.localtime(complaint.resolved_at).strftime('%Y-%m-%d %H:%M') if complaint.resolved_at else 'لم يتم الحل',
            complaint.get_customer_rating_display() if complaint.customer_rating else 'لم يتم التقييم',
            complaint.customer_feedback or '',
            complaint.related_order.order_number if complaint.related_order else 'لا يوجد',
...
def export_complaints_to_excel(queryset=None, selected_ids=None):
    ...
    response['Content-Disposition'] = f'attachment; filename="complaints-{timezone.now().strftime("%Y%m%d-%H%M%S")}.xlsx"'
