...
def main():
    ...
    print(
        f"✅ تم العثور على {orders.count()} طلب من أصل {len(suspicious_order_numbers)}"
    )
    ...
    for order in orders.order_by("order_number"):
        ...
        print(
            f"   👤 العميل: {order.customer_name if hasattr(order, 'customer_name') else 'غير محدد'}"
        )
        ...
        for i, inspection in enumerate(inspections, 1):
            ...
            if inspection.completed_at and order.order_date:
                if inspection.completed_at.date() != order.order_date.date():
                    ...
                    print(
                        f"         - تاريخ إكمال المعاينة: {inspection.completed_at.date()}"
                    )
                    issues_found += 1
                else:
