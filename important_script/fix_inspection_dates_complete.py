...
def main():
    ...
    with transaction.atomic():
        ...
        for order in target_orders.order_by("order_number"):
            ...
            for inspection in inspections:
                ...
                if hasattr(inspection, "inspection_date"):
                    inspection_date_fields.append(
                        ("inspection_date", inspection.inspection_date)
                    )
                    print(
                        f"      📋 تاريخ المعاينة الحالي: {inspection.inspection_date}"
                    )
                ...
                if hasattr(inspection, "appointment_date"):
                    inspection_date_fields.append(
                        ("appointment_date", inspection.appointment_date)
                    )
                    print(
                ...
                if order.order_date:
                    ...
                    print(
                        f"      ✅ تحديث تاريخ الإنشاء: {old_created} → {inspection.created_at}"
                    )
                    ...
                    for field_name, old_value in inspection_date_fields:
                        if hasattr(inspection, field_name):
                            new_date = (
                                order.order_date.date() if order.order_date else None
                            )
                            setattr(inspection, field_name, new_date)
                            print(
                                f"      ✅ تحديث {field_name}: {old_value} → {new_date}"
                            )
                    ...
                    if inspection.status == "completed":
                        ...
                        print(
                            f"      ✅ تحديث تاريخ الإكمال: {old_completed} → {inspection.completed_at}"
                        )
        ...
        for order in target_orders[:5]:  # عرض أول 5 طلبات للتحقق
            ...
            for inspection in inspections:
                ...
                if (
                    hasattr(inspection, "appointment_date")
                    and inspection.appointment_date
                ):
    ...
    print(
        "📋 تم تحديث: تاريخ الإنشاء، تاريخ الإكمال، تاريخ المعاينة، وجميع التواريخ ذات الصلة"
    )
