...
def main():
    ...
    with transaction.atomic():
        ...
        if wrong_results.exists():
            print(
                f"🔧 إصلاح {wrong_results.count()} معاينة تحتوي على قيمة 'success' خاطئة..."
            )
        ...
        completed_without_result = Inspection.objects.filter(
            status="completed", result__isnull=True
        )
        if completed_without_result.exists():
            print(
                f"🔧 إضافة نتيجة لـ {completed_without_result.count()} معاينة مكتملة بدون نتيجة..."
            )
        ...
        manufacturing_orders_to_update = ManufacturingOrder.objects.exclude(
        ...
        for order in installation_orders:
            ...
            installation_schedule = InstallationSchedule.objects.create(
                ...
                completion_date=(
                    order.order_date if order.order_date else timezone.now()
                ),
                status="completed",
        ...
        installations_to_update = InstallationSchedule.objects.exclude(
            status="completed"
