...
class WhatsAppSettingsAdmin(admin.ModelAdmin):
    ...
    list_display = ["phone_number", "is_active_badge", "test_mode_badge", "updated_at"]
                ...
                try:
                    ...
                    if template == "hello_world":
                        ...
                        result = service.send_template_message(
                            to=phone, template_name="hello_world", language="en_US"
                        )
                    elif template.startswith("db_"):
                        # Template from database without meta_template_name
                        django_messages.error(
                            request,
                            "❌ هذا القالب غير معتمد من Meta بعد. أضف اسم القالب في Meta أولاً.",
                        )
                    ...
                    if result and result.get("messages"):
                        ...
                        customer, _ = Customer.objects.get_or_create(
                            phone=phone, defaults={"name": f"Test {phone[-4:]}"}
                        )
...
class WhatsAppMessageTemplateAdmin(admin.ModelAdmin):
    ...
    list_filter = ["message_type", "is_active", "language"]
    ...
    fieldsets = (
        ...
        ("ملاحظات", {"fields": ("description",), "classes": ("collapse",)}),
    )
...
class WhatsAppMessageAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("المحتوى", {"fields": ("message_text", "phone_number", "attachments")}),
        (
            "الحالة",
    ...
    def retry_failed_messages(self, request, queryset):
        ...
        self.message_user(request, f"تم إعادة محاولة إرسال {success_count} رسالة بنجاح")
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        today_messages = WhatsAppMessage.objects.filter(created_at__date=today).count()

        # رسائل الأسبوع
        week_ago = timezone.now() - timedelta(days=7)
        week_messages = WhatsAppMessage.objects.filter(created_at__gte=week_ago).count()
...
class WhatsAppEventTypeAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("معلومات الحدث", {"fields": ("code", "name", "description", "is_active")}),
    )
...
class WhatsAppNotificationRuleAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("معلومات القاعدة", {"fields": ("event_type", "is_enabled")}),
        ("القالب", {"fields": ("template",)}),
        ("الإعدادات", {"fields": ("delay_minutes", "conditions")}),
    )
    ...
    def template_link(self, obj):
        if obj.template:
            url = reverse(
                "admin:whatsapp_whatsappmessagetemplate_change", args=[obj.template.id]
            )
