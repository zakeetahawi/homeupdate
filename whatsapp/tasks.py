...
@shared_task(
    bind=True, max_retries=3, default_retry_delay=60, autoretry_for=(Exception,)
)
def send_whatsapp_notification_task(
    ...
    try:
        ...
        installation = (
            InstallationSchedule.objects.get(id=installation_id)
            if installation_id
            else None
        )
...
@shared_task(
    bind=True, max_retries=3, default_retry_delay=300, autoretry_for=(Exception,)
)
