...
from whatsapp.models import WhatsAppNotificationRule, WhatsAppMessageTemplate
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for rule_data in rules_data:
            ...
            if not template:
                self.stdout.write(
                    self.style.WARNING(f'⚠ القالب غير موجود: {rule_data["template_name"]}')
                )
                continue
            ...
            if created:
                created_count += 1
                self.stdout.write(
                    self.style.SUCCESS(f'✓ تم إنشاء القاعدة: {rule.get_event_type_display()}')
                )
            else:
                updated_count += 1
                self.stdout.write(
                    self.style.WARNING(f'⟳ تم تحديث القاعدة: {rule.get_event_type_display()}')
                )
