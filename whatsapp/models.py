...
class WhatsAppSettings(models.Model):
    ...
    test_mode = models.BooleanField(default=False, verbose_name="وضع الاختبار")
    ...
    max_retry_attempts = models.IntegerField(
        default=3, verbose_name="عدد محاولات الإعادة"
    )
    default_language = models.CharField(
        max_length=5, default="ar", verbose_name="اللغة الافتراضية"
    )
    ...
    def is_template_enabled(self, message_type):
        ...
        return self.enabled_templates.filter(
            message_type=message_type, is_active=True
        ).exists()
...
class WhatsAppMessageTemplate(models.Model):
    ...
    name = models.CharField(max_length=200, verbose_name="اسم القالب (للعرض)")
    ...
    description = models.TextField(
        blank=True, verbose_name="وصف/ملاحظات", help_text="وصف اختياري للقالب"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
...
class WhatsAppMessage(models.Model):
    ...
    message_type = models.CharField(max_length=50, verbose_name="نوع الرسالة")
    ...
    attachments = models.JSONField(default=dict, blank=True, verbose_name="المرفقات")
    ...
    delivered_at = models.DateTimeField(
        null=True, blank=True, verbose_name="وقت التسليم"
    )
...
class WhatsAppNotificationRule(models.Model):
    ...
    event_type = models.CharField(
        max_length=50, choices=EVENT_TYPES, unique=True, verbose_name="نوع الحدث"
    )
    ...
    delay_minutes = models.IntegerField(default=0, verbose_name="تأخير الإرسال (دقائق)")
    ...
    def __str__(self):
        return (
            f"{self.get_event_type_display()} - {'مفعل' if self.is_enabled else 'معطل'}"
        )
