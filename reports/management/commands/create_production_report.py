...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--user',
            type=str,
            help='اسم المستخدم الذي سيتم إنشاء التقرير باسمه'
        )
    ...
    def handle(self, *args, **kwargs):
        ...
        if existing_report:
            ...
            self.stdout.write(
                self.style.SUCCESS(f'✅ تم تحديث تقرير الإنتاج: {existing_report.title}')
            )
            self.stdout.write(
                self.style.SUCCESS(f'   - ID: {existing_report.id}')
            )
            self.stdout.write(
                self.style.SUCCESS(f'   - المالك: {user.get_full_name() or user.username}')
            )
            ...
            self.stdout.write(
                self.style.SUCCESS(f'   - ID: {report.id}')
            )
            self.stdout.write(
                self.style.SUCCESS(f'   - المالك: {user.get_full_name() or user.username}')
            )
        ...
        self.stdout.write(
            self.style.SUCCESS(f'   /reports/{report.id}/')
        )
        self.stdout.write(
