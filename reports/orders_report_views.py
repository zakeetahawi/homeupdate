...
from orders.models import DraftOrder, Order
...
def orders_report_view(request):
    ...
    traditional_orders = (
        ...
        .exclude(source_draft__isnull=False)  # استبعاد الطلبات المرتبطة بـ wizard
    ...
    wizard_orders = (
        DraftOrder.objects.filter(
            date_filter, is_completed=True  # فقط الطلبات المكتملة
        )
        .select_related(
    ...
    top_traditional_salespersons = (
        traditional_orders.values("salesperson__id", "salesperson__name")
    ...
    top_wizard_salespersons = (
        wizard_orders.values("salesperson__id", "salesperson__name")
        .annotate(order_count=Count("id"))
        .filter(salesperson__isnull=False)
        .order_by("-order_count")[:5]
    )
