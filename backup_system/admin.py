...
class BackupJobAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("أخطاء", {"fields": ("error_message",), "classes": ("collapse",)}),
    )
    ...
    def compressed_size_display(self, obj):
        ...
        if obj.compressed_size:
            if obj.compressed_size < 1024:
                ...
                return "{:.1f} جيجابايت".format(
                    obj.compressed_size / (1024 * 1024 * 1024)
                )
        return "-"
...
class RestoreJobAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("معلومات أساسية", {"fields": ("name", "description", "created_by")}),
        (
            "إعدادات الاستعادة",
...
class BackupScheduleAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("معلومات أساسية", {"fields": ("name", "description", "created_by")}),
        ("إعدادات الجدولة", {"fields": ("frequency", "hour", "minute", "is_active")}),
        (
