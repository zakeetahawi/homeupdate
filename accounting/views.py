...
from .forms import (
    AccountForm, TransactionForm, TransactionLineFormSet,
    CustomerAdvanceForm, QuickPaymentForm, QuickAdvanceForm,
    DateRangeFilterForm, AdvanceUsageForm
)
...
def account_detail(request, pk):
    ...
    ).select_related('transaction').order_by('-transaction__transaction_date')[:50]
...
def transaction_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and formset.is_valid():
            ...
            messages.success(request, f'تم إنشاء القيد {transaction.reference_number} بنجاح')
...
def transaction_detail(request, pk):
    ...
    transaction = get_object_or_404(
        Transaction.objects.prefetch_related('lines__account'),
        pk=pk
    )
...
def customer_financial_summary(request, customer_id):
    ...
    # السلف النشطة
    active_advances = CustomerAdvance.objects.filter(
        customer=customer,
        status='active'
    )
        ...
        if form.is_valid():
            ...
            try:
                ...
                if order:
                    ...
                    messages.success(request, f'تم تسجيل الدفعة بنجاح للطلب {order.order_number}')
                else:
...
def api_customer_summary(request, customer_id):
    ...
    data = {
        ...
        'status': 'debit' if summary.total_debt > 0 else ('credit' if summary.total_debt < 0 else 'clear'),
...
def api_dashboard_stats(request):
    ...
    data = {
        'total_receivables': float(
            CustomerFinancialSummary.objects.filter(
                total_debt__gt=0
