    ...
    try:
        ...
        with db_transaction.atomic():
            ...
            txn = Transaction.objects.create(
                transaction_date=payment.payment_date if hasattr(payment, 'payment_date') else timezone.now().date(),
                transaction_type='receipt',
                description=f'استلام دفعة للطلب {payment.order.order_number}',
                notes=f'رقم إيصال: {payment.receipt_number}' if hasattr(payment, 'receipt_number') else '',
                order=payment.order,
...
def create_order_transaction(order):
    ...
    try:
        ...
        with db_transaction.atomic():
            txn = Transaction.objects.create(
                transaction_date=order.order_date if hasattr(order, 'order_date') else timezone.now().date(),
