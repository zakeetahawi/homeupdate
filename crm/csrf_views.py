...
def csrf_failure(request, reason=""):
    ...
    logger.warning(
        f"CSRF failure for user {request.user} from IP {request.META.get('REMOTE_ADDR')}: {reason}"
    )
    ...
    context = {
        ...
        "csrf_cookie_value": (
            request.COOKIES.get("csrftoken", "غير موجود")[:10] + "..."
    ...
    return HttpResponseForbidden(render(request, "403_csrf.html", context).content)
...
def get_csrf_token_view(request):
    ...
    return JsonResponse(
        {"csrf_token": token, "status": "success", "message": "تم إنشاء رمز حماية جديد"}
    )
...
def csrf_debug_view(request):
    ...
    if not settings.DEBUG:
        return JsonResponse(
            {"error": "هذه الصفحة متاحة في وضع التطوير فقط"}, status=404
        )
    ...
    debug_info = {
        ...
        "csrf_info": {
            "has_csrf_cookie": "csrftoken" in request.COOKIES,
            "csrf_cookie_value": (
                request.COOKIES.get("csrftoken", "غير موجود")[:20] + "..."
                if "csrftoken" in request.COOKIES
                else "غير موجود"
            ),
            "csrf_header": (
                request.META.get("HTTP_X_CSRFTOKEN", "غير موجود")[:20] + "..."
        ...
        "session_info": {
            ...
            "user": (
                str(request.user) if request.user.is_authenticated else "غير مسجل دخول"
            ),
        },
    }

    return JsonResponse(
        debug_info, json_dumps_params={"ensure_ascii": False, "indent": 2}
    )
