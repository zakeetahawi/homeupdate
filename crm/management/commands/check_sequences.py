...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--app", type=str, help="فحص تطبيق محدد فقط")
        parser.add_argument("--table", type=str, help="فحص جدول محدد فقط")
        parser.add_argument(
            "--show-all",
            action="store_true",
            help="عرض جميع الجداول حتى التي لا تحتوي على مشاكل",
        )
        parser.add_argument("--export", type=str, help="تصدير النتائج إلى ملف JSON")
        parser.add_argument("--fix", action="store_true", help="إصلاح التسلسل تلقائياً")
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS("🔍 بدء فحص تسلسل ID..."))
    ...
    def check_all_sequences(self):
        ...
        all_results = {"problems": [], "healthy": [], "no_sequence": [], "errors": []}
    ...
    def check_app_sequences(self, app_name, show_header=True):
        ...
        if show_header:
            self.stdout.write(self.style.SUCCESS(f"🔍 فحص التطبيق: {app_name}"))

        try:
            app_config = apps.get_app_config(app_name)
        except LookupError:
            self.stdout.write(self.style.ERROR(f"❌ التطبيق {app_name} غير موجود"))
            return {"problems": [], "healthy": [], "no_sequence": [], "errors": []}

        models = app_config.get_models()
        results = {"problems": [], "healthy": [], "no_sequence": [], "errors": []}
    ...
    def check_single_table(self, table_name):
        ...
        self.stdout.write(self.style.SUCCESS(f"🔍 فحص الجدول: {table_name}"))
        ...
        try:
            ...
            error_result = {"table": table_name, "status": "errors", "error": str(e)}
            results["errors"].append(error_result)
            self.stdout.write(self.style.ERROR(f"❌ خطأ في فحص {table_name}: {str(e)}"))
    ...
    def check_sequence_health(self, table_name, column_name, sequence_name):
        ...
        logger.debug(
            f"Checking sequence health for table={table_name}, column={column_name}, sequence={sequence_name}"
        )
        ...
        try:
            ...
            with connection.cursor() as cursor:
                ...
                try:
                    ...
                    logger.debug(
                        f"Max ID for {table_name}: {max_id} (type: {type(max_id) if max_id is not None else 'NoneType'})"
                    )
                except Exception as e:
                ...
                logger.debug(
                    f"Sequence values for {table_name} - max_id: {max_id} (type: {type(max_id)}), current_seq: {current_seq} (type: {type(current_seq)}), next_value: {next_value}, gap: {gap}"
                )
            ...
            logger.error(
                f"Error checking sequence health for {table_name}: {str(e)}",
                exc_info=True,
            )
            return {
                "table": table_name,
    ...
    def display_summary(self, results):
        ...
        if problems:
            ...
            if critical:
                self.stdout.write(self.style.ERROR(f"🚨 مشاكل حرجة: {len(critical)}"))
            ...
            if warnings:
                ...
                if self.show_all:
                    for warning in warnings:
                        self.stdout.write(
                            f'   - {warning["table"]}: {warning["message"]}'
                        )

        if healthy and self.show_all:
            self.stdout.write(self.style.SUCCESS(f"✅ جداول سليمة: {len(healthy)}"))

        if no_seq and self.show_all:
            self.stdout.write(self.style.WARNING(f"ℹ️  جداول بدون تسلسل: {len(no_seq)}"))
        ...
        if problems:
            self.stdout.write("\n💡 التوصيات:")
            critical_count = len(
                [p for p in problems if p.get("severity") == "critical"]
            )
            if critical_count > 0:
                self.stdout.write(
                    self.style.ERROR(f"🚨 يجب إصلاح {critical_count} مشكلة حرجة فوراً!")
                )
    ...
    def export_results(self, results, filename):
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f"❌ فشل في تصدير النتائج: {str(e)}"))
