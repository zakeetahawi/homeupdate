...
class Command(BaseCommand):
    ...
    def _show_status(self):
        ...
        if system_stats:
            ...
            self.stdout.write(f"   العملية: {process['memory_percent']:.1f}% ذاكرة، {process['num_threads']} خيط")
    ...
    def _cleanup_connections(self, emergency=False):
        ...
        if cleaned > 0:
            self.stdout.write(
                self.style.SUCCESS(f'✅ تم تنظيف {cleaned} اتصال')
            )
        else:
            self.stdout.write(
                self.style.SUCCESS('✅ لا توجد اتصالات تحتاج تنظيف')
            )
    ...
    def _single_check(self):
        ...
        if db_stats:
            ...
            if alert_level:
                color = self.style.ERROR if alert_level == 'critical' else self.style.WARNING
                self.stdout.write(color(f"⚠️  تحذير: {alert_level}"))
            else:
                self.stdout.write(self.style.SUCCESS("✅ الحالة طبيعية"))
        else:
            self.stdout.write(self.style.ERROR('❌ فشل في الحصول على إحصائيات قاعدة البيانات'))
        ...
        try:
            while self.monitoring_active:
                ...
                if db_stats:
                    ...
                    self.stdout.write(
                        self.style.ERROR(f"[{timezone.now().strftime('%H:%M:%S')}] ❌ خطأ في البيانات")
                    )
