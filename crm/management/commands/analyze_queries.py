...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--verbose", action="store_true", help="عرض تفاصيل مفصلة")

    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS("🔍 بدء تحليل أداء الاستعلامات..."))
    ...
    def analyze_manufacturing_admin(self):
        ...
        self.stdout.write(f"  🔢 عدد الاستعلامات: {total_queries}")
        self.stdout.write(f"  📄 عدد السجلات: {len(results)}")

        if total_time > 100:
            self.stdout.write(self.style.WARNING(f"  ⚠️  الاستعلام بطيء! يحتاج تحسين"))
        else:
            self.stdout.write(self.style.SUCCESS(f"  ✅ الاستعلام سريع"))
    ...
    def analyze_customers_admin(self):
        ...
        self.stdout.write("\n" + "=" * 50)
        self.stdout.write(self.style.SUCCESS("✅ تم الانتهاء من تحليل الأداء"))
