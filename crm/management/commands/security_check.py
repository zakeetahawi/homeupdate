...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if settings.DEBUG:
            ...
            if verbose:
                self.stdout.write(
                    self.style.WARNING("  DEBUG = True وُجد في settings.py")
                )
        else:
        ...
        if hasattr(settings, "ALLOWED_HOSTS"):
            if "*" in settings.ALLOWED_HOSTS:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"✅ ALLOWED_HOSTS محدد ({len(settings.ALLOWED_HOSTS)} نطاق)"
                    )
                )
        ...
        if hasattr(settings, "DATABASES"):
            ...
            if default_db.get("PASSWORD") in ["", "password", "1234"]:
                ...
                self.stdout.write(
                    self.style.SUCCESS("✅ كلمة مرور قاعدة البيانات محمية")
                )
        ...
        if hasattr(settings, "AUTH_PASSWORD_VALIDATORS"):
            if len(settings.AUTH_PASSWORD_VALIDATORS) < 3:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"✅ محققي كلمات المرور ({len(settings.AUTH_PASSWORD_VALIDATORS)})"
                    )
                )
        ...
        for setting_name, recommended_value in file_settings.items():
            if not hasattr(settings, setting_name):
                ...
                self.stdout.write(
                    self.style.SUCCESS(f"  ✅ {setting_name} = {current}")
                )
        ...
        if not settings.DEBUG:
            ...
            if x_frame not in ["DENY", "SAMEORIGIN"]:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f"  ✅ X_FRAME_OPTIONS = {x_frame}")
                )
        ...
        if session_age and session_age > 86400 * 7:  # أكثر من أسبوع
            ...
            self.stdout.write(
                self.style.SUCCESS(f"  ✅ SESSION_COOKIE_AGE = {days:.1f} يوم")
            )
        ...
        if issues:
            self.stdout.write(
                self.style.ERROR("⚠️  يوجد مشاكل أمنية عاجلة يجب إصلاحها!\n")
            )
