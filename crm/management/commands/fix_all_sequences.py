...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--app", type=str, help="إصلاح تسلسل ID لتطبيق محدد فقط")
        parser.add_argument("--table", type=str, help="إصلاح تسلسل ID لجدول محدد فقط")
        parser.add_argument(
            "--dry-run",
            action="store_true",
    ...
    def fix_all_sequences(self):
        ...
        self.stdout.write(self.style.SUCCESS("🔧 بدء إصلاح تسلسل ID لجميع الجداول..."))
    ...
    def fix_single_table(self, table_name):
        ...
        self.stdout.write(self.style.SUCCESS(f"🔧 إصلاح تسلسل ID للجدول: {table_name}"))
            ...
            try:
                ...
                if not auto_columns:
                    if self.verbose:
                        self.stdout.write(
                            self.style.WARNING(
                                f"⚠️  لا يوجد عمود ID تلقائي في {table_name}"
                            )
                        )
                    return False
            ...
            try:
                ...
                if current_seq < next_value:
                    if not self.dry_run:
                        # إعادة تعيين التسلسل
                        cursor.execute(
                            f"SELECT setval('{sequence_name}', %s, false)", [next_value]
                        )
