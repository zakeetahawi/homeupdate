...
from crm.db_pool import cleanup_pool, get_pool_statistics
from crm.monitoring import get_monitoring_status, performance_monitor
...
def database_stats(request):
    ...
    try:
        ...
        if db_stats:
            ...
            return JsonResponse({"success": True, "data": db_stats})
        else:
            return JsonResponse(
                {
                    "success": False,
...
def system_stats(request):
    ...
    try:
        ...
        if system_stats:
            return JsonResponse({"success": True, "data": system_stats})
        else:
            return JsonResponse(
                {"success": False, "error": "فشل في الحصول على إحصائيات النظام"},
                status=500,
            )
    ...
    def post(self, request):
        try:
            ...
            if action == "cleanup":
                ...
                cleaned = performance_monitor.db_monitor.cleanup_connections(
                    force=force
                )
                ...
                return JsonResponse(
                    {
                        ...
                        "data": {"monitoring_interval": interval},
                    }
                )
                ...
                return JsonResponse(
                    {"success": False, "error": f"إجراء غير معروف: {action}"},
                    status=400,
                )
...
def health_check(request):
    ...
    try:
        ...
        system_stats = performance_monitor.system_monitor.get_system_stats()
        system_healthy = (
            system_stats is not None
            and system_stats["memory"]["percent"] < 90
            and system_stats["cpu"]["percent"] < 90
        )
        ...
        health_data = {
            ...
            "checks": {
                "database_connections": (
                    db_stats["total_connections"] if db_stats else "unknown"
                ),
                "memory_usage": (
                    f"{system_stats['memory']['percent']:.1f}%"
                    if system_stats
                    else "unknown"
                ),
                "cpu_usage": (
                    f"{system_stats['cpu']['percent']:.1f}%"
                    if system_stats
                    else "unknown"
                ),
            },
        ...
        return JsonResponse({"success": True, "data": health_data}, status=status_code)

    except Exception as e:
        logger.error(f"Error in health check: {e}")
        return JsonResponse(
            {"success": False, "error": str(e), "overall_healthy": False}, status=500
        )
