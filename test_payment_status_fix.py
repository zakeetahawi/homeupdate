...
def test_payment_status_logic():
    ...
    if inspection.order:
        ...
        print(f"   حالة الدفع في الطلب: {'مدفوع بالكامل' if inspection.order.is_fully_paid else 'غير مدفوع بالكامل'}")
        ...
        expected_status = 'paid' if inspection.order.is_fully_paid else 'collect_on_visit'
        if inspection.payment_status == expected_status:
            print("✅ تم تحديث حالة المديونية بشكل صحيح")
            return True
        else:
            print(f"❌ خطأ في تحديث حالة المديونية. متوقع: {expected_status}, فعلي: {inspection.payment_status}")
            return False
    else:
...
def test_template_display():
    ...
    try:
        ...
        if 'form-control-plaintext' in template_content and 'badge bg-success' in template_content:
